...
from backtest.sandbox_verification_certificate_ledger_audit_v19_6 import (
    GENESIS_HASH,
    SandboxVerificationCertificateLedgerAuditV196Policy,
    audit_sandbox_verification_certificate_ledger_v19_6,
    load_audit_result,
    save_audit_result,
...
NOW = datetime(2026, 7, 29, 10, 20, tzinfo=timezone.utc)
...
def create_entry(
    ...
    payload = {
        ...
        "latest_carried_final_report_hash": chr(ord(marker) + 4) * 64,
        "latest_linked_final_report_hash": chr(ord(marker) + 5) * 64,
        "latest_verification_ledger_snapshot_hash": (
            chr(ord(marker) + 6) * 64
        ),
        "latest_previous_final_report_hash": chr(ord(marker) + 7) * 64,
...
def create_source() -> Any:
    ...
    return OptimizedLedgerSource(
        ...
        ledger_result_id="v19-5-optimized-test-ledger",
        result_status="RECORDED_IN_MEMORY",
        result_status_label="Optimized V19.5 test fixture",
        ...
        latest_carried_final_report_hash=(
            second.latest_carried_final_report_hash
        ),
...
def main() -> None:
    ...
    require(source.to_dict() == source_before, "V19.5 Source 변경")
    ...
    require(
        result.latest_carried_final_report_hash
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        require(payload["version"] == "V19.6", "저장 Version 오류")
    ...
    checks = {
        "Version is V19.6": result.version == "V19.6",
        ...
        "V19.5 certificate ledger source passed": result.source_checks_passed,
        "Optimized source fixture passed": (
            source.ledger_result_id == "v19-5-optimized-test-ledger"
        ),
        ...
        "Result save and load passed": payload["version"] == "V19.6",
    ...
    print("=" * 108)
    print(
        "AI STOCK BOT V19.6 SANDBOX VERIFICATION CERTIFICATE LEDGER "
        "INTEGRITY AUDIT TEST"
    )
    print("=" * 108)
    print("V19.6 VALIDATION CHECKS")
    print("-" * 108)
    for name, passed in checks.items():
        print(f"{name:<74}: {passed}")
    print("=" * 108)
    require(checks["All checks passed"], "V19.6 Validation Check 실패")
    print()
    print(
        "V19.6 sandbox verification certificate ledger integrity audit "
        "test completed successfully."
    )
    print(
        "V19.5 Ledger Hash Chain, 연결 Hash, Entry Findings, 읽기 전용 "
