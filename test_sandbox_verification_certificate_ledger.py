...
from test_sandbox_audit_ledger_integrity_verification import (
    create_source as create_ledger_source,
)
from test_sandbox_audit_ledger_integrity_verification import verify

from backtest.sandbox_verification_certificate_ledger import (
    SandboxVerificationCertificateLedgerPolicy,
...
NOW = datetime(2026, 7, 28, 15, 50, tzinfo=timezone.utc)
...
def main() -> None:
    ...
    duplicate = record(
        first_source, existing=first.entries, now=NOW + timedelta(minutes=1)
    )
    ...
    unsafe_policy = SandboxVerificationCertificateLedgerPolicy(
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        require(payload["version"] == "V15.5", "저장 Version 오류")
    ...
    checks = {
        "Version is V15.5": second.version == "V15.5",
        "Default policy is valid": second.policy_checks_passed,
        "Policy is immutable": (
            SandboxVerificationCertificateLedgerPolicy.__dataclass_params__.frozen
        ),
        "Invalid policies are blocked": not unsafe.all_checks_passed,
        "V15.4 verification source passed": second.source_checks_passed,
        ...
        "Result save and load passed": payload["version"] == "V15.5",
    ...
    print("=" * 104)
    print("AI STOCK BOT V15.5 SANDBOX VERIFICATION CERTIFICATE LEDGER TEST")
    print("=" * 104)
    print("V15.5 VALIDATION CHECKS")
    print("-" * 104)
    for name, passed in checks.items():
        print(f"{name:<70}: {passed}")
    print("=" * 104)
    require(checks["All checks passed"], "V15.5 Validation Check 실패")
    print()
    print(
        "V15.5 sandbox verification certificate ledger test "
        "completed successfully."
    )
    print(
        "V15.4 Certificate Hash 보존, Ledger Sequence, SHA-256 Hash Chain 및 "
