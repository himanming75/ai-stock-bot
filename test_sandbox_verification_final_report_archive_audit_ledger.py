...
from test_sandbox_verification_final_report_archive_integrity_audit import (
    audit,
)
from test_sandbox_verification_final_report_archive_integrity_audit import (
...
from backtest.sandbox_verification_final_report_archive_audit_ledger import (
    ...
    record_sandbox_verification_final_report_archive_audit,
    save_audit_ledger_result,
    verify_audit_ledger_chain,
)


NOW = datetime(2026, 7, 28, 19, 10, tzinfo=timezone.utc)
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        require(payload["version"] == "V16.3", "저장 Version 오류")
    ...
    checks = {
        "Version is V16.3": second.version == "V16.3",
        ...
        "V16.2 audit source passed": second.source_checks_passed,
        ...
        "Result save and load passed": payload["version"] == "V16.3",
    ...
    print("=" * 108)
    print(
        "AI STOCK BOT V16.3 SANDBOX VERIFICATION FINAL REPORT "
        "ARCHIVE AUDIT LEDGER TEST"
    )
    print("=" * 108)
    print("V16.3 VALIDATION CHECKS")
    print("-" * 108)
    for name, passed in checks.items():
        print(f"{name:<74}: {passed}")
    print("=" * 108)
    require(checks["All checks passed"], "V16.3 Validation Check 실패")
    print()
    print(
        "V16.3 sandbox verification final report archive audit "
        "ledger test completed successfully."
    )
    print(
        "V16.2 Audit Certificate Hash, Final Report Hash, Certificate "
        "Ledger Snapshot Hash 보존 및 Ledger Chain이 검증되었습니다."
    )
    print(
