...
from test_sandbox_final_report_archive_v18_1 import archive
from test_sandbox_final_report_archive_v18_1 import (
...
from backtest.sandbox_archive_integrity_audit_v18_2 import (
    ...
    audit_sandbox_archive_integrity_v18_2,
    load_audit_result,
    save_audit_result,
...
def create_source() -> Any:
    ...
    return archive(
        ...
        now=datetime(2026, 7, 29, 3, 31, tzinfo=timezone.utc),
    )
...
def main() -> None:
    ...
    require(source.to_dict() == source_before, "V18.1 Source 변경")
    require(
    ...
    require(
        ...
        == source.entries[-1].certificate_ledger_snapshot_hash,
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        require(payload["version"] == "V18.2", "저장 Version 오류")
        require(
    ...
    checks = {
        "Version is V18.2": result.version == "V18.2",
        ...
        "V18.1 archive source passed": result.source_checks_passed,
        ...
        "Result save and load passed": payload["version"] == "V18.2",
    ...
    print("=" * 108)
    print(
        "AI STOCK BOT V18.2 SANDBOX ARCHIVE INTEGRITY AUDIT TEST"
    )
    print("=" * 108)
    print("V18.2 VALIDATION CHECKS")
    print("-" * 108)
    for name, passed in checks.items():
        print(f"{name:<74}: {passed}")
    print("=" * 108)
    require(checks["All checks passed"], "V18.2 Validation Check 실패")
    print()
    print(
        "V18.2 sandbox archive integrity audit test completed "
        "successfully."
    )
    print(
        "V18.1 Archive Chain, Entry Findings, 연결 Hash, Snapshot Hash, "
