...
from backtest.offline_paper_verify_cert_ledger_v22_1 import (
    OfflinePaperVerifyCertLedgerV221Policy,
    load_verification_certificate_ledger_result,
    record_offline_paper_verification_certificate_v22_1,
    save_verification_certificate_ledger_result,
    verify_verification_certificate_ledger_chain,
)
from test_offline_paper_verify_ledger_audit_v22_0 import (
    NOW as V22_0_NOW,
    create_source as create_v21_9_source,
    verify as verify_v22_0,
)


NOW = datetime(2026, 7, 29, 13, 50, tzinfo=timezone.utc)
...
def create_source(now: datetime = V22_0_NOW) -> Any:
    return verify_v22_0(create_v21_9_source(), now=now)


def record(
    source: Any,
    operator: Any = "operator-001",
    text: Any = "RECORD OFFLINE PAPER VERIFICATION CERTIFICATE V22.1",
    existing: Any = None,
    policy: Any = None,
    now: datetime = NOW,
) -> Any:
...
def source_has_no_external_execution_calls() -> bool:
    source_path = (
        ...
        / "backtest"
        / "offline_paper_verify_cert_ledger_v22_1.py"
    )
...
def main() -> None:
    ...
    require(source_one.to_dict() == source_before, "V22.0 Source 변경")

    source_two = create_source(V22_0_NOW + timedelta(minutes=1))
    second = record(
        source_two,
    ...
    chain_valid, chain_errors = verify_verification_certificate_ledger_chain(
        second.entries
    )
    require(chain_valid and not chain_errors, "V22.1 Ledger Hash Chain 실패")
    require(
        second.latest_verification_certificate_hash
        == source_two.verification_certificate_hash,
        "V22.0 Verification Certificate Hash 보존 실패",
    )
    require(
        second.latest_audit_verify_ledger_snapshot_hash
        == source_two.audit_verify_ledger_snapshot_hash,
    ...
    wrong_operator_policy = OfflinePaperVerifyCertLedgerV221Policy(
    ...
    tampered_source.certificate = replace(
        ...
        audit_verify_ledger_snapshot_hash="a" * 64,
    )
    ...
    tampered_existing = record(
        create_source(V22_0_NOW + timedelta(minutes=2)),
    ...
    with tempfile.TemporaryDirectory() as directory:
        report_path, latest_path = save_verification_certificate_ledger_result(
            second, Path(directory)
        )
        require(report_path.exists() and latest_path.exists(), "저장 실패")
        payload = load_verification_certificate_ledger_result(latest_path)
        require(payload["version"] == "V22.1", "저장 Version 오류")
    ...
    checks = {
        "Version is V22.1": second.version == "V22.1",
        ...
        "V22.0 verification source passed": second.source_checks_passed,
        ...
        "V22.0 verification certificate hash was preserved": (
            second.latest_verification_certificate_hash
            == source_two.verification_certificate_hash
        ),
        ...
        "Result save and load passed": payload["version"] == "V22.1",
    ...
    print("=" * 108)
    print(
        "AI STOCK BOT V22.1 OFFLINE PAPER VERIFICATION "
        "CERTIFICATE LEDGER TEST"
    )
    print("=" * 108)
    print("V22.1 VALIDATION CHECKS")
    print("-" * 108)
    for name, passed in checks.items():
        print(f"{name:<74}: {passed}")
    print("=" * 108)
    require(checks["All checks passed"], "V22.1 Validation Check 실패")
    print()
    print(
        "V22.1 offline paper verification certificate ledger "
        "test completed successfully."
    )
    print(
        "V22.0 Verification Certificate Hash, V21.9 Ledger Snapshot Hash, "
