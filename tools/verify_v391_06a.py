...
path = ROOT / "release/v391_06a/actual/symbol_concentration_guard_result.json"
...
checks = {
    "stage": result.get("stage") == "V391.06A",
    ...
    "symbol_present": isinstance(result.get("symbol"), str) and bool(result.get("symbol")),
    "evaluation_present": isinstance(evaluation, dict),
    "current_symbol_value_present": isinstance(
        evaluation.get("current_symbol_value"), (int, float)
    ),
    "projected_pct_present": isinstance(
        evaluation.get("projected_symbol_exposure_pct"), (int, float)
    ),
    "limit_present": isinstance(
        evaluation.get("maximum_symbol_exposure_pct"), (int, float)
    ),
