...
def main():
 ...
 root=Path(a.repository_root).resolve();out=root/"release/v81_60/output"
 if a.clean and out.exists():shutil.rmtree(out)
 c=BrokerAdapterConfig();r=run_engine(root,c,out);cert=build_certificate(root,out,c,r)
 print(json.dumps({"stage_range":"V81.41-V81.60","status":cert["status"],**cert["adapter_summary"],
 ...
 "broker_adapter_foundation_complete":cert["broker_adapter_foundation_complete"],"next_phase":cert["next_phase"]},indent=2,sort_keys=True))
