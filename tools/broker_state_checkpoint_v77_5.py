...
VERSION = "77.5"
SCHEMA = "v77.5.broker_state_checkpoint_verification.1"
...
def verify(root: Path, config: dict[str, Any]) -> dict[str, Any]:
    ...
    source_path = root/"release/v77_4/output/execution_event_reconciliation_verification_v77_4.json"
    add_gate(gates, "V77_4_VERIFICATION_EXISTS", source_path.is_file())
    source = load_json(source_path)
    add_gate(gates, "V77_4_STATUS_PASS", source.get("status") == "PASS")
    add_gate(gates, "V77_4_RECONCILIATION_ANCHOR_MATCH",
             source.get("execution_event_reconciliation_sha256")
             == config["expected_v77_4_reconciliation_sha256"])
    add_gate(gates, "V77_4_VERIFICATION_ANCHOR_MATCH",
             source.get("verification_sha256")
             == config["expected_v77_4_verification_sha256"])
    add_gate(gates, "V77_4_NEXT_PHASE_MATCH",
             source.get("next_phase") == "V77_5_BROKER_STATE_CHECKPOINT")

    sim = build_scenario()
    manager = BrokerStateCheckpointManager()
    checkpoint = manager.create(sim, checkpoint_id="V77-5-PRIMARY")

    add_gate(gates, "CHECKPOINT_SCHEMA_MATCH",
             checkpoint.schema_version == config["required_schema_version"])
    add_gate(gates, "SOURCE_RECONCILIATION_PASS",
             checkpoint.source_reconciliation_status == "PASS")
    add_gate(gates, "SOURCE_RECONCILIATION_ISSUES_ZERO",
             checkpoint.source_reconciliation_issue_count == 0)
    add_gate(gates, "CHECKPOINT_HASH_LENGTH", len(checkpoint.state_sha256) == 64)
    add_gate(gates, "CHECKPOINT_VERIFY_PASS", manager.verify(checkpoint))
    add_gate(gates, "CHECKPOINT_ORDER_COUNT", len(checkpoint.orders) == 2)
    add_gate(gates, "CHECKPOINT_FILL_COUNT", len(checkpoint.fills) == 3)
    add_gate(gates, "CHECKPOINT_POSITION_COUNT", len(checkpoint.positions) == 1)
    add_gate(gates, "CHECKPOINT_EVENT_COUNT", len(checkpoint.events) == 6)
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        round_trip_pass = loaded.as_dict() == checkpoint.as_dict()
    ...
    add_gate(gates, "ROUND_TRIP_VERIFIED", round_trip_pass)
    add_gate(gates, "TAMPER_DETECTED", tamper_detected)
    add_gate(gates, "MISSING_FIELD_DETECTED", missing_field_detected)
    ...
    checkpoint_framework_sha = digest(definition)
    add_gate(gates, "CHECKPOINT_FRAMEWORK_DIGEST_CREATED",
             len(checkpoint_framework_sha) == 64)
    ...
    result = {
        ...
        "version": VERSION,
        "record_type": "BROKER_STATE_CHECKPOINT_VERIFICATION",
        "issued_at_utc": datetime.now(timezone.utc).isoformat(),
        "status": status,
        "decision": (
            "broker_state_checkpoint_established"
            if status == "PASS" else "broker_state_checkpoint_rejected"
        ),
        "repository": {
        ...
        "source_anchors": {
            "v77_4_execution_event_reconciliation_sha256":
                source.get("execution_event_reconciliation_sha256"),
            "v77_4_verification_sha256": source.get("verification_sha256"),
        },
        "checkpoint_definition": definition,
        "broker_state_checkpoint_sha256": checkpoint_framework_sha,
        "sample_checkpoint": checkpoint.as_dict(),
        "verification_result": {
        ...
        "next_phase": NEXT_PHASE if status == "PASS"
            else "REPAIR_V77_5_BROKER_STATE_CHECKPOINT",
    }
...
def summary(result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "broker_state_checkpoint_sha256":
            result["broker_state_checkpoint_sha256"],
        "sample_state_sha256": result["sample_checkpoint"]["state_sha256"],
        ...
        **result["source_anchors"],
        ...
        "checkpoint_order_count": len(result["sample_checkpoint"]["orders"]),
        "checkpoint_fill_count": len(result["sample_checkpoint"]["fills"]),
        "checkpoint_event_count": len(result["sample_checkpoint"]["events"]),
...
def write_outputs(result: dict[str, Any], output_dir: Path) -> None:
    ...
    (output_dir/"broker_state_checkpoint_verification_v77_5.json").write_text(
        json.dumps(result, indent=2, sort_keys=True)+"\n", encoding="utf-8"
    )
    (output_dir/"broker_state_checkpoint_summary_v77_5.json").write_text(
        json.dumps(summary(result), indent=2, sort_keys=True)+"\n", encoding="utf-8"
    )
    (output_dir/"sample_broker_state_checkpoint_v77_5.json").write_text(
        json.dumps(result["sample_checkpoint"], indent=2, sort_keys=True)+"\n",
        encoding="utf-8",
    )
