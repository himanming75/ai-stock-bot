...
def main():
 ...
 root=Path(a.repository_root).resolve();out=root/"release/v81_80/output"
 if a.clean and out.exists():shutil.rmtree(out)
 c=ExecutionSimulationConfig();r=run_engine(root,c,out);cert=build_certificate(root,out,c,r)
 print(json.dumps({"stage_range":"V81.61-V81.80","status":cert["status"],**cert["execution_summary"],
 ...
 "execution_simulation_complete":cert["execution_simulation_complete"],"next_phase":cert["next_phase"]},indent=2,sort_keys=True))
