...
def main():
 ...
 root=Path(a.repository_root).resolve();out=root/"release/v83_80/output"
 if a.clean and out.exists():shutil.rmtree(out)
 c=PaperBrokerExecutionSimulationConfig();r=run_engine(root,c,out);cert=build_certificate(root,out,c,r)
 print(json.dumps({"stage_range":"V83.61-V83.80","status":cert["status"],**cert["paper_broker_execution_summary"],
 ...
 "paper_broker_execution_simulation_complete":cert["paper_broker_execution_simulation_complete"],
