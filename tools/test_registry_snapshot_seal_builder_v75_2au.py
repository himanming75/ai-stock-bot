...
from tools.registry_snapshot_seal_builder_v75_2au import *
...
def config():
    return {
        ...
        "require_snapshot_verification_integrity": True,
        "require_verified_snapshot_index_integrity": True,
        ...
        "create_seal_manifest": True,
        "create_sealed_snapshot_index": True,
        "create_seal_checks": True,
        "create_seal_ledger": True,
...
def source():
    ...
    value = {
        ...
        "certificate_registry_snapshot_verification_id": "CRSX-AAAAAAAAAAAAAAAA",
        ...
        "verified_snapshot_receipt_count": 1,
        "verified_snapshot_index": index,
        "verified_snapshot_index_sha256": sha256_of(index),
        ...
        "schema_version": "v75.2at.offline_paper_certificate_registry_snapshot_verification.1",
        "version": "75.2AT",
    }

    value["offline_paper_certificate_registry_snapshot_verification_sha256"] = sha256_of(value)
    return value


class TestV752AU(unittest.TestCase):
    def build(self):
        return build_seal(source(), config(), SEALED_AT)

    @staticmethod
    def rehash(value):
        value.pop("offline_paper_certificate_registry_snapshot_verification_sha256", None)
        value["offline_paper_certificate_registry_snapshot_verification_sha256"] = sha256_of(value)
    ...
    def test_state(self):
        self.assertEqual(
            self.build()["seal_state"],
            "SEALED_VERIFIED_OFFLINE_CERTIFICATE_REGISTRY_SNAPSHOT",
        )

    def test_count(self):
        self.assertEqual(self.build()["sealed_receipt_count"], 1)

    def test_manifest(self):
        self.assertEqual(
            self.build()["seal_manifest"]["seal_effect"],
            "OFFLINE_FINAL_EVIDENCE_CERTIFICATE_REGISTRY_SNAPSHOT_SEAL_ONLY",
        )

    def test_deterministic_id(self):
        self.assertEqual(
            self.build()["certificate_registry_snapshot_seal_id"],
            self.build()["certificate_registry_snapshot_seal_id"],
        )

    def test_index_state(self):
        self.assertEqual(
            self.build()["sealed_snapshot_index"][0]["seal_state"],
            "SEALED_VERIFIED_SNAPSHOTTED_REGISTERED_CERTIFIED_ARCHIVED_OFFLINE_RECEIPT",
        )

    def test_hashes(self):
        output = self.build()
        self.assertEqual(output["seal_manifest_sha256"], sha256_of(output["seal_manifest"]))
        self.assertEqual(
            output["sealed_snapshot_index_sha256"],
            sha256_of(output["sealed_snapshot_index"]),
        )
        self.assertEqual(output["seal_checks_sha256"], sha256_of(output["seal_checks"]))
        self.assertEqual(output["seal_ledger_sha256"], sha256_of(output["seal_ledger"]))

    def test_output_hash(self):
        output = self.build()
        observed = output.pop("offline_paper_certificate_registry_snapshot_seal_sha256")
    ...
    def test_source_not_mutated(self):
        ...
        build_seal(value, config(), SEALED_AT)
    ...
    def test_tampered_index(self):
        value = source()
        value["verified_snapshot_index"][0]["notional_value"] = 1
        value["verified_snapshot_index_sha256"] = sha256_of(value["verified_snapshot_index"])
        self.rehash(value)
        self.assertRaises(
            RegistrySnapshotSealError,
            build_seal,
    ...
    def test_checks_ledger(self):
        output = self.build()
        self.assertEqual(len(output["seal_checks"]), 12)
        self.assertEqual(len(output["seal_ledger"]), 6)
    ...
    def test_main_outputs(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue((path / "out/registry_snapshot_seal_v75_2au.json").exists())
