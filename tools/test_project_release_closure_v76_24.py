...
from tools.project_release_closure_v76_24 import create_closure, digest, validate_config, write_outputs
from tools.verify_project_release_closure_v76_24 import verify_output

class V7624Tests(unittest.TestCase):
    def setUp(self):
        self.config = {
            "closure_scope":"PROJECT_RELEASE_CLOSURE",
            ...
            "require_v76_23_zero_failed_gates":True,
    ...
    def source(self):
        s = {
            "schema_version":"v76.23.release_archive_completion_certificate_verification.1",
            "verification_type":"RELEASE_ARCHIVE_COMPLETION_CERTIFICATE_VERIFICATION",
            "status":"PASS",
            "decision":"release_archive_completion_certificate_independently_verified",
            "repository":{"framework_commit_sha":"c"*40},
            "verification_chain":{"sample":"chain"},
            "verification_result":{"failed_gate_count":0,"failed_gate_ids":[]},
            "release_archive_completion_certificate_independently_verified":True,
        ...
        s["verification_chain_sha256"] = digest(s["verification_chain"])
        s["verification_sha256"] = digest(s)
        return s
    ...
    def test_pass_and_output_verifier(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.config["expected_verification_chain_sha256"] = source["verification_chain_sha256"]
            d = root/"release/v76_23/output"
            d.mkdir(parents=True)
            (d/"release_archive_completion_certificate_verification_v76_23.json").write_text(json.dumps(source), encoding="utf-8")
            git = {"head_sha":"d"*40,"head_short_sha":"d"*7,"origin_main_sha":"d"*40,
                   "branch":"main","tracked_status_short":[]}
            with patch("tools.project_release_closure_v76_24.git_state", return_value=git):
                result = create_closure(root, self.config)
            self.assertEqual(result["status"], "PASS")
            out = root/"release/v76_24/output"
