...
from tools.offline_paper_fill_receipt_archive_certificate_registry_snapshot_seal_certificate_registry_builder_v75_2aq import *

REGISTERED_AT="2026-07-31T00:00:00-07:00"

def cfg():
    return {"registry_scope":"OFFLINE_PAPER_FILL_RECEIPT_ARCHIVE_CERTIFICATE_REGISTRY_SNAPSHOT_SEAL_CERTIFICATE_REGISTRY_ONLY",
      "require_certificate_verification_integrity":True,"require_verified_certified_index_integrity":True,
      "require_verification_checks_integrity":True,"require_verification_ledger_integrity":True,
      "require_zero_settlement_and_account_mutations":True,"create_registry_manifest":True,
      "create_registered_index":True,"create_registry_checks":True,"create_registry_ledger":True,
...
def src():
    index=[{"certificate_record_index":1,"seal_index":1,"snapshot_index":1,"registry_index":1,
      "certificate_index":1,"archive_index":1,"receipt_id":"FRC-AAAAAAAAAAAAAAAA",
      ...
      "verification_state":"VERIFIED_CERTIFIED_SEALED_SNAPSHOTTED_REGISTERED_CERTIFIED_ARCHIVED_OFFLINE_RECEIPT"}]
    checks=[{"check_index":i,"check":f"CHECK_{i}","state":"PASS" if i<9 else ("LOCKED" if i==9 else "ENFORCED")} for i in range(1,13)]
    ledger=[{"ledger_index":i,"event":f"EVENT_{i}","state":"PASS",
      "certificate_verification_id":"FCX-AAAAAAAAAAAAAAAA"} for i in range(1,7)]
    s={"status":"PASS",
      "decision":"offline_paper_fill_receipt_archive_certificate_registry_snapshot_seal_certificate_verified",
      "fill_receipt_archive_certificate_registry_snapshot_seal_certificate_verification_id":"FCX-AAAAAAAAAAAAAAAA",
      "fill_receipt_archive_certificate_registry_snapshot_seal_certificate_id":"FSC-AAAAAAAAAAAAAAAA",
      "fill_receipt_archive_certificate_registry_snapshot_seal_verification_id":"FSX-AAAAAAAAAAAAAAAA",
      ...
      "verified_certified_receipt_count":1,"verified_certified_index":index,
      "verified_certified_index_sha256":sha256_of(index),"verification_checks":checks,
      "verification_checks_sha256":sha256_of(checks),"verification_ledger":ledger,
      ...
      "source_certificate_sha256":"b"*64,"source_certificate_manifest_sha256":"c"*64,
      "source_certified_index_sha256":"d"*64,"source_certificate_checks_sha256":"e"*64,
      "source_certificate_ledger_sha256":"f"*64,"session_id":"PAPER-A","cycle_id":"PCS-A",
      "cycle_sequence":1,"champion_candidate_id":"CAND-A","settlements_created":0,
      "positions_updated":0,"cash_updates_created":0,"portfolio_updates_created":0,
      ...
      "schema_version":"v75.2ap.offline_paper_fill_receipt_archive_certificate_registry_snapshot_seal_certificate_verification.1",
      "version":"75.2AP"}
    s["offline_paper_fill_receipt_archive_certificate_registry_snapshot_seal_certificate_verification_sha256"]=sha256_of(s)
    return s

class TestV752AQ(unittest.TestCase):
    def build(self): return build_registry(src(),cfg(),REGISTERED_AT)
    def rehash(self,s):
        s.pop("offline_paper_fill_receipt_archive_certificate_registry_snapshot_seal_certificate_verification_sha256",None)
        s["offline_paper_fill_receipt_archive_certificate_registry_snapshot_seal_certificate_verification_sha256"]=sha256_of(s)
    def test_pass(self): self.assertEqual(self.build()["status"],"PASS")
    def test_state(self): self.assertEqual(self.build()["registry_state"],"REGISTERED_VERIFIED_OFFLINE_FILL_RECEIPT_ARCHIVE_CERTIFICATE_REGISTRY_SNAPSHOT_SEAL_CERTIFICATE")
    def test_count(self): self.assertEqual(self.build()["registered_receipt_count"],1)
    def test_manifest(self): self.assertEqual(self.build()["registry_manifest"]["registry_effect"],"OFFLINE_FINAL_EVIDENCE_CERTIFICATE_REGISTRATION_ONLY")
    def test_index_state(self): self.assertEqual(self.build()["registered_index"][0]["registry_state"],"REGISTERED_VERIFIED_CERTIFIED_SEALED_SNAPSHOTTED_REGISTERED_CERTIFIED_ARCHIVED_OFFLINE_RECEIPT")
    def test_hashes(self):
        o=self.build()
        self.assertEqual(o["registry_manifest_sha256"],sha256_of(o["registry_manifest"]))
        self.assertEqual(o["registered_index_sha256"],sha256_of(o["registered_index"]))
    def test_output_hash(self):
        o=self.build(); h=o.pop("offline_paper_fill_receipt_archive_certificate_registry_snapshot_seal_certificate_registry_sha256")
        self.assertEqual(h,sha256_of(o))
    def test_deterministic_id(self):
        self.assertEqual(self.build()["fill_receipt_archive_certificate_registry_snapshot_seal_certificate_registry_id"],
                         self.build()["fill_receipt_archive_certificate_registry_snapshot_seal_certificate_registry_id"])
    ...
    def test_no_live(self):
        o=self.build()
        for k in ("broker_routing_allowed","network_allowed","approved_for_live","network_used"):
            self.assertFalse(o[k])
    ...
    def test_tampered_source(self):
        ...
        self.assertRaises(OfflinePaperFillReceiptArchiveCertificateRegistrySnapshotSealCertificateRegistryError,
                          build_registry,s,cfg(),REGISTERED_AT)
    def test_tampered_index(self):
        s=src(); s["verified_certified_index"][0]["notional_value"]=1
        s["verified_certified_index_sha256"]=sha256_of(s["verified_certified_index"]); self.rehash(s)
        self.assertRaises(OfflinePaperFillReceiptArchiveCertificateRegistrySnapshotSealCertificateRegistryError,
                          build_registry,s,cfg(),REGISTERED_AT)
    ...
    def test_unsafe_config(self):
        ...
        self.assertRaises(OfflinePaperFillReceiptArchiveCertificateRegistrySnapshotSealCertificateRegistryError,
                          build_registry,src(),c,REGISTERED_AT)
    ...
    def test_main_outputs(self):
        with tempfile.TemporaryDirectory() as td:
            p=Path(td)
            (p/"s.json").write_text(json.dumps(src()),encoding="utf-8")
            (p/"c.json").write_text(json.dumps(cfg()),encoding="utf-8")
            rc=main(["--input",str(p/"s.json"),"--config",str(p/"c.json"),
                     "--output-dir",str(p/"out"),"--registered-at",REGISTERED_AT])
            self.assertEqual(rc,0)
            self.assertTrue((p/"out/offline_paper_fill_receipt_archive_certificate_registry_snapshot_seal_certificate_registry_v75_2aq.json").exists())
