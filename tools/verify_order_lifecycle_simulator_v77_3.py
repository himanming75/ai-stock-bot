...
from tools.order_lifecycle_simulator_v77_3 import digest, load_json, summary


def verify_output(output_dir: Path) -> dict:
    result = load_json(output_dir/"order_lifecycle_simulator_verification_v77_3.json")
    saved_summary = load_json(output_dir/"order_lifecycle_simulator_summary_v77_3.json")
    errors: list[str] = []
    ...
    checks = (
        ...
        (result.get("decision") == "order_lifecycle_simulator_established", "decision mismatch"),
        ...
        (result.get("live_trading_authorized") is False, "live trading must remain unauthorized"),
        (result.get("next_phase") == "V77_4_EXECUTION_EVENT_RECONCILIATION", "next phase mismatch"),
    )
    ...
    return {
        ...
        "errors": errors,
        "order_lifecycle_simulator_sha256": result.get("order_lifecycle_simulator_sha256"),
