...
def main():
 ...
 root=Path(a.repository_root).resolve();out=root/"release/v85_20/output"
 if a.clean and out.exists():shutil.rmtree(out)
 c=PaperBrokerNetworkFoundationConfig();r=run_engine(root,c,out);cert=build_certificate(root,out,c,r)
 print(json.dumps({"stage_range":"V85.01-V85.20","status":cert["status"],**cert["paper_network_foundation_summary"],
 ...
 "live_trading_authorized":False,"paper_network_foundation_complete":cert["paper_network_foundation_complete"],
