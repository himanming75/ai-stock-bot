...
class Tests(unittest.TestCase):
    def test_test_fixture_placeholder_is_ignored(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "fixture.json").write_text(
                '{"api_key":"raw-key-value","secret_key":"plaintext-value"}',
                encoding="utf-8",
            )
    ...
    def test_short_teaching_values_are_ignored(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "teaching.py").write_text(
