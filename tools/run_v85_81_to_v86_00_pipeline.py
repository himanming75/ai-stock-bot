...
def main():
 ...
 root=Path(a.repository_root).resolve();out=root/"release/v86_00/output"
 if a.clean and out.exists():shutil.rmtree(out)
 c=PaperBrokerNetworkEnablementConfig();r=run_engine(root,c,out);cert=build_certificate(root,out,c,r)
 print(json.dumps({"stage_range":"V85.81-V86.00","status":cert["status"],**cert["paper_network_enablement_summary"],
 "network_requests_executed":0,"credentials_used":0,"trading_client_created":False,
 "actual_orders_submitted":0,"paper_network_enabled":False,
 "paper_order_submission_authorized":False,"live_trading_authorized":False,
 "paper_network_enablement_foundation_complete":cert["paper_network_enablement_foundation_complete"],
