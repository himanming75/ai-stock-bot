...
from tools.offline_paper_order_generation_executor_v75_2u import *

TS="2026-07-30T21:55:00+00:00"

def source_fixture():
    manifest=[{"order_created":False,"order_intent_id":"INT-A",
               "order_object_creation_authorized":True,"order_submitted":False,
               "quantity":1,"reference_price":633.5,"side":"BUY","symbol":"SPY"}]
    token_material={"authorization_id":"OGA-A","validation_id":"OIV-A",
                    "issued_at":"2026-07-30T21:50:00+00:00",
                    "expires_at":"2026-07-30T22:05:00+00:00","nonce":"n",
                    "scope":"OFFLINE_PAPER_ORDER_OBJECT_CREATION_ONLY",
                    "authorized_order_intent_ids":["INT-A"]}
    ...
    s={"status":"PASS","authorization_id":"OGA-A",
       ...
       "validation_id":"OIV-A","execution_id":"OIE-A","session_id":"PAPER-A",
       "cycle_id":"PCS-A","cycle_sequence":1,"champion_candidate_id":"CAND-A",
       "source_order_intent_validation_sha256":"a"*64,
       "source_order_intent_execution_sha256":"b"*64,
       "source_order_intent_package_sha256":"c"*64,
       "authorization_gate":{"order_generation_authorized":True,
         ...
         "live_orders_allowed":False,"network_allowed":False,"next_version":"75.2U"},
       ...
       "schema_version":"v75.2t.offline_paper_order_generation_authorization.1","version":"75.2T"}
    s["offline_paper_order_generation_authorization_sha256"]=sha256_of(s)
    return s

def config_fixture():
    return {"execution_scope":"OFFLINE_PAPER_ORDER_OBJECT_CREATION_ONLY",
      "order_type":"MARKET_REFERENCE_ONLY","time_in_force":"DAY",
      "require_authorization_integrity":True,"require_single_use_token":True,
      "require_token_unconsumed":True,"require_token_unexpired":True,
      "require_manifest_integrity":True,"prevent_output_overwrite":True,
      "order_submission_allowed":False,"fill_simulation_allowed":False,
      "paper_broker_allowed":False,"live_orders_allowed":False,
      "network_allowed":False,"broker_connection_allowed":False,
      "external_side_effects_allowed":False}

class TestV752U(unittest.TestCase):
    def build(self): return build_execution(source_fixture(),config_fixture(),TS)
    def rehash(self,s):
        s.pop("offline_paper_order_generation_authorization_sha256",None)
        s["offline_paper_order_generation_authorization_sha256"]=sha256_of(s)
    def test_pass(self): self.assertEqual(self.build()["status"],"PASS")
    def test_state(self): self.assertEqual(self.build()["execution_state"],"READY_FOR_ORDER_OBJECT_VALIDATION")
    def test_order_created(self): self.assertEqual(self.build()["order_objects_created"],1)
    def test_order(self):
        x=self.build()["paper_order_package"]["paper_orders"][0]
        self.assertEqual(x["symbol"],"SPY"); self.assertEqual(x["side"],"BUY")
        self.assertEqual(x["quantity"],1); self.assertEqual(x["order_state"],"CREATED_NOT_SUBMITTED")
    def test_token_consumed(self): self.assertTrue(self.build()["token_consumed"])
    def test_no_submission(self): self.assertEqual(self.build()["orders_submitted"],0)
    def test_no_fill(self): self.assertEqual(self.build()["fills_created"],0)
    def test_gate(self): self.assertTrue(self.build()["execution_gate"]["order_object_validation_allowed"])
    def test_hash(self):
        x=self.build(); h=x.pop("offline_paper_order_generation_execution_sha256")
        self.assertEqual(h,sha256_of(x))
    def test_package_hash(self):
        x=self.build(); self.assertEqual(x["paper_order_package_sha256"],sha256_of(x["paper_order_package"]))
    ...
    def test_expired(self):
        self.assertRaises(OrderGenerationExecutionError,build_execution,source_fixture(),config_fixture(),"2026-07-30T22:06:00+00:00")
    def test_before_issued(self):
        self.assertRaises(OrderGenerationExecutionError,build_execution,source_fixture(),config_fixture(),"2026-07-30T21:49:00+00:00")
    def test_consumed(self):
        s=source_fixture(); s["authorization_token"]["consumed"]=True
        s["authorization_token"]["token_state"]="CONSUMED"
        s["authorization_token_sha256"]=sha256_of(s["authorization_token"]); self.rehash(s)
        self.assertRaises(OrderGenerationExecutionError,build_execution,s,config_fixture(),TS)
    def test_bad_integrity(self):
        s=source_fixture(); s["cycle_id"]="BAD"
        self.assertRaises(OrderGenerationExecutionError,build_execution,s,config_fixture(),TS)
    def test_bad_side(self):
        s=source_fixture(); s["authorized_order_intent_manifest"][0]["side"]="HOLD"
        s["authorized_order_intent_manifest_sha256"]=sha256_of(s["authorized_order_intent_manifest"]); self.rehash(s)
        self.assertRaises(OrderGenerationExecutionError,build_execution,s,config_fixture(),TS)
    def test_unsafe_config(self):
        c=config_fixture(); c["order_submission_allowed"]=True
        self.assertRaises(OrderGenerationExecutionError,build_execution,source_fixture(),c,TS)
    def test_main(self):
        with tempfile.TemporaryDirectory() as td:
            p=Path(td); (p/"s.json").write_text(json.dumps(source_fixture()))
