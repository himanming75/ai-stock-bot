...
def main():
 ...
 root=Path(a.repository_root).resolve();out=root/"release/v84_80/output"
 if a.clean and out.exists():shutil.rmtree(out)
 c=LiveOrderSubmissionSimulationConfig();r=run_engine(root,c,out);cert=build_certificate(root,out,c,r)
 print(json.dumps({"stage_range":"V84.61-V84.80","status":cert["status"],**cert["live_order_submission_summary"],
 ...
 "live_order_submission_simulation_complete":cert["live_order_submission_simulation_complete"],
