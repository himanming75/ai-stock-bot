...
from runtime_scheduler.runtime_scheduler_pipeline_v78_21_25 import load_json,digest_json
def main():
    p=argparse.ArgumentParser();p.add_argument("--repository-root",default=".")
    a=p.parse_args();r=Path(a.repository_root).resolve()
    path=r/"release/v78_25/output/runtime_scheduler_pipeline_summary_v78_21_to_v78_25.json"
    errors=[];doc={}
    if not path.is_file():errors.append("summary_missing")
    else:doc=load_json(path)
    if doc:
        if doc.get("pipeline_sha256")!=digest_json({k:v for k,v in doc.items() if k!="pipeline_sha256"}):errors.append("pipeline_sha256")
    ...
    result={"verified":not errors,"status":"PASS" if not errors else "FAIL","error_count":len(errors),"errors":errors,
      "pipeline_sha256":doc.get("pipeline_sha256"),"champion_candidate_id":doc.get("champion_candidate_id"),
