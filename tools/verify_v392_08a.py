...
path = ROOT / "release/v392_08a/actual/local_dispatch_release_gate_result.json"

with path.open("r", encoding="utf-8-sig") as handle:
    result = json.load(handle)

checks = {
    "stage": result.get("stage") == "V392.08A",
    "status": result.get("status") == "PASS",
    "state_valid": result.get("state") in {
        "LOCAL_DISPATCH_RELEASE_GATE_READY",
        "LOCAL_DISPATCH_RELEASE_GATE_BLOCKED",
    },
    "evaluation_present": isinstance(result.get("evaluation"), dict),
    "release_record_present": isinstance(
        result.get("evaluation", {}).get("release_record"), dict
    ),
    "release_record_hash_present": isinstance(
        result.get("evaluation", {}).get("release_record_hash"), str
    ),
