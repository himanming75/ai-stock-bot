...
def main():
 ...
 root=Path(a.repository_root).resolve();out=root/"release/v81_00/output"
 if a.clean and out.exists():shutil.rmtree(out)
 c=StrategySelectionConfig();r=run_engine(root,c,out);cert=build_certificate(root,out,c,r)
 print(json.dumps({"stage_range":"V80.81-V81.00","status":cert["status"],**cert["selection_summary"],
 ...
 "strategy_selection_complete":cert["strategy_selection_complete"],"next_phase":cert["next_phase"]},indent=2,sort_keys=True))
