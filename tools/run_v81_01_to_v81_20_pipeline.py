...
def main():
 ...
 root=Path(a.repository_root).resolve();out=root/"release/v81_20/output"
 if a.clean and out.exists():shutil.rmtree(out)
 c=PortfolioOptimizationConfig();r=run_engine(root,c,out);cert=build_certificate(root,out,c,r)
 print(json.dumps({"stage_range":"V81.01-V81.20","status":cert["status"],**cert["optimization_summary"],
 ...
 "portfolio_optimization_complete":cert["portfolio_optimization_complete"],"next_phase":cert["next_phase"]},indent=2,sort_keys=True))
