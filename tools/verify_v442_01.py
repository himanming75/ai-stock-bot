...
path = ROOT / "release/v442_01/actual/kelly_position_sizing_result.json"
...
checks = {
    "stage": result.get("stage") == "V442.01",
    "state": result.get("state") == "KELLY_POSITION_SIZING_READY",
    "status": result.get("status") == "PASS",
    "positions_present": len(positions) > 0,
    "kelly_cap_respected": all(
        float(item["effective_weight"]) <= float(result["maximum_kelly_pct"]) + 0.000001
        or item["binding_constraint"] != "KELLY_LIMIT"
