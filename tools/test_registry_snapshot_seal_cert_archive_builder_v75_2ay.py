...
from tools.registry_snapshot_seal_cert_archive_builder_v75_2ay import *
...
def config():
    return {
        ...
        "require_certificate_verification_integrity": True,
        "require_verified_certified_sealed_snapshot_index_integrity": True,
        ...
        "create_archived_certified_sealed_snapshot_index": True,
        "create_archive_checks": True,
        "create_archive_ledger": True,
...
def source():
    ...
    value = {
        ...
        "verified_certified_receipt_count": 1,
        "verified_certified_sealed_snapshot_index": index,
        "verified_certified_sealed_snapshot_index_sha256": sha256_of(index),
        ...
        "source_certificate_manifest_sha256": "c" * 64,
        "source_certified_sealed_snapshot_index_sha256": "d" * 64,
        "source_certificate_checks_sha256": "e" * 64,
        "source_certificate_ledger_sha256": "f" * 64,
        "session_id": "PAPER-A",
        "cycle_id": "PCS-A",
        "cycle_sequence": 1,
        ...
        "schema_version": (
            "v75.2ax.offline_paper_certificate_registry_snapshot_seal_"
...
class TestV752AY(unittest.TestCase):
    def build(self):
        return build_archive(source(), config(), ARCHIVED_AT)
    ...
    def test_state(self):
        self.assertEqual(
            self.build()["archive_state"],
            (
                "ARCHIVED_VERIFIED_OFFLINE_CERTIFICATE_REGISTRY_SNAPSHOT_"
                "SEAL_CERTIFICATE"
            ),
        )
    ...
    def test_manifest(self):
        self.assertEqual(
            self.build()["archive_manifest"]["archive_effect"],
            (
                "OFFLINE_FINAL_EVIDENCE_CERTIFICATE_REGISTRY_SNAPSHOT_SEAL_"
    ...
    def test_index_state(self):
        self.assertEqual(
            self.build()["archived_certified_sealed_snapshot_index"][0][
                "archive_state"
            ],
            (
    ...
    def test_hashes(self):
        output = self.build()
        self.assertEqual(
            output["archive_manifest_sha256"],
            sha256_of(output["archive_manifest"]),
        )
        self.assertEqual(
            output["archived_certified_sealed_snapshot_index_sha256"],
            sha256_of(output["archived_certified_sealed_snapshot_index"]),
        )
        self.assertEqual(
            output["archive_checks_sha256"],
            sha256_of(output["archive_checks"]),
        )
        self.assertEqual(
            output["archive_ledger_sha256"],
            sha256_of(output["archive_ledger"]),
        )
    ...
    def test_source_not_mutated(self):
        ...
        build_archive(value, config(), ARCHIVED_AT)
    ...
    def test_tampered_index(self):
        value = source()
        value["verified_certified_sealed_snapshot_index"][0][
            "notional_value"
        ] = 1
        value["verified_certified_sealed_snapshot_index_sha256"] = sha256_of(
            value["verified_certified_sealed_snapshot_index"]
        )
        self.rehash(value)
        self.assertRaises(
            RegistrySnapshotSealCertificateArchiveError,
    ...
    def test_checks_ledger(self):
        output = self.build()
        self.assertEqual(len(output["archive_checks"]), 12)
        self.assertEqual(len(output["archive_ledger"]), 6)
