...
def main():
 ...
 root=Path(a.repository_root).resolve();out=root/"release/v87_20/output"
 if a.clean and out.exists():shutil.rmtree(out)
 c=StrategyExecutionOperationsConfig();r=run_engine(root,c,out);cert=certificate(root,out,c,r)
 print(json.dumps({"stage_range":"V87.01-V87.20","status":cert["status"],**cert["strategy_execution_summary"],
 "paper_strategy_execution_operations_complete":cert["paper_strategy_execution_operations_complete"],
 "strategy_execution_preview_ready":cert["strategy_execution_preview_ready"],
