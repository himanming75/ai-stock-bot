...
from optimization.strategy_optimization_pipeline_v77_71_75 import load_json,digest_json
def main():
 p=argparse.ArgumentParser();p.add_argument("--repository-root",default=".");a=p.parse_args();r=Path(a.repository_root)
 s=load_json(r/"release/v77_75/output/strategy_optimization_pipeline_summary_v77_71_to_v77_75.json");e=[]
 ...
 if not s.get("champion_candidate_id"):e.append("champion")
 ...
 out={"verified":not e,"status":"PASS" if not e else "FAIL","error_count":len(e),"errors":e,
      "pipeline_sha256":s.get("pipeline_sha256"),"champion_candidate_id":s.get("champion_candidate_id"),
      "next_phase":s.get("next_phase")}
