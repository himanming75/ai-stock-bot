...
def main():
 ...
 root=Path(a.repository_root).resolve();out=root/"release/v83_40/output"
 if a.clean and out.exists():shutil.rmtree(out)
 c=PaperOrderAuthorizationConfig();r=run_engine(root,c,out);cert=build_certificate(root,out,c,r)
 print(json.dumps({"stage_range":"V83.21-V83.40","status":cert["status"],**cert["paper_order_authorization_summary"],
 "network_requests_executed":0,"credentials_used":0,"trading_client_created":False,"actual_orders_submitted":0,
 "paper_session_authorized":True,"paper_order_authorization_ready":cert["paper_order_authorization_ready"],
 "paper_order_submission_authorized":False,"paper_trading_authorized":False,"live_trading_authorized":False,
 "paper_order_authorization_foundation_complete":cert["paper_order_authorization_foundation_complete"],
