...
def main():
 ...
 root=Path(a.repository_root).resolve();out=root/"release/v82_60/output"
 if a.clean and out.exists():shutil.rmtree(out)
 c=BrokerConnectionValidationConfig();r=run_engine(root,c,out);cert=build_certificate(root,out,c,r)
 print(json.dumps({"stage_range":"V82.41-V82.60","status":cert["status"],**cert["broker_connection_summary"],
 ...
 "broker_connection_validation_complete":cert["broker_connection_validation_complete"],"next_phase":cert["next_phase"]},indent=2,sort_keys=True))
