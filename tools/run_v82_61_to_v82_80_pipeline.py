...
def main():
 ...
 root=Path(a.repository_root).resolve();out=root/"release/v82_80/output"
 if a.clean and out.exists():shutil.rmtree(out)
 c=DryRunBrokerConfig();r=run_engine(root,c,out);cert=build_certificate(root,out,c,r)
 print(json.dumps({"stage_range":"V82.61-V82.80","status":cert["status"],**cert["dry_run_summary"],
 ...
 "dry_run_broker_validation_complete":cert["dry_run_broker_validation_complete"],"next_phase":cert["next_phase"]},indent=2,sort_keys=True))
