...
from test_sandbox_verification_audit_ledger_integrity_verification import (
    create_source as create_verification_source,
)
from test_sandbox_verification_audit_ledger_integrity_verification import verify

from backtest.sandbox_verification_audit_certificate_ledger import (
    SandboxVerificationAuditCertificateLedgerPolicy,
    load_certificate_ledger_result,
    record_sandbox_verification_certificate,
    save_certificate_ledger_result,
    verify_certificate_ledger_chain,
)


NOW = datetime(2026, 7, 28, 17, 10, tzinfo=timezone.utc)
...
def create_source() -> Any:
    return verify(create_verification_source())


def record(
    source: Any,
    operator: str = "operator-001",
    text: str = "RECORD IN MEMORY SANDBOX VERIFICATION CERTIFICATE",
    existing: Any = None,
    policy: Any = None,
    now: datetime = NOW,
) -> Any:
...
def main() -> None:
    ...
    require(
        ...
        "V15.8 Verification Certificate Hash 보존 실패",
    )
    require(
        first.entries[0].ledger_snapshot_hash
        == first_source.ledger_snapshot_hash,
        "V15.8 Ledger Snapshot Hash 보존 실패",
    )
    require_safe(first)
    ...
    valid_chain, chain_errors = verify_certificate_ledger_chain(second.entries)
    ...
    unsafe_policy = SandboxVerificationAuditCertificateLedgerPolicy(
    ...
    tampered_source.certificate = replace(
        tampered_source.certificate, verification_status="FAILED"
    )
    ...
    changed_entry = replace(
        second.entries[0], ledger_snapshot_hash="e" * 64
    )
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        require(payload["version"] == "V15.9", "저장 Version 오류")
    ...
    checks = {
        "Version is V15.9": second.version == "V15.9",
        ...
        "V15.8 verification source passed": second.source_checks_passed,
        ...
        "Verification certificate ledger hash chain passed": valid_chain,
        "V15.8 certificate hash was preserved": (
            first.entries[0].verification_certificate_hash
            == first_source.verification_certificate_hash
        ),
        "V15.8 ledger snapshot hash was preserved": (
            first.entries[0].ledger_snapshot_hash
            == first_source.ledger_snapshot_hash
        ),
        ...
        "Result save and load passed": payload["version"] == "V15.9",
    ...
    print("=" * 108)
    print("AI STOCK BOT V15.9 SANDBOX VERIFICATION CERTIFICATE LEDGER TEST")
    print("=" * 108)
    print("V15.9 VALIDATION CHECKS")
    print("-" * 108)
    for name, passed in checks.items():
        print(f"{name:<74}: {passed}")
    print("=" * 108)
    require(checks["All checks passed"], "V15.9 Validation Check 실패")
    print()
    print(
        "V15.9 sandbox verification certificate ledger test "
        "completed successfully."
    )
    print(
        "V15.8 Verification Certificate Hash와 Ledger Snapshot Hash 보존, "
