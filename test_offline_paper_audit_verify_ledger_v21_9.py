...
from backtest.offline_paper_audit_verify_ledger_v21_9 import (
    OfflinePaperAuditVerifyLedgerV219Policy,
    load_audit_verification_ledger_result,
    record_offline_paper_audit_verification_v21_9,
    save_audit_verification_ledger_result,
    verify_audit_verify_ledger_chain,
)
from test_offline_paper_audit_integrity_verify_v21_8 import (
    NOW as V21_8_NOW,
    create_source as create_v21_7_source,
    verify as verify_v21_8,
)
...
def create_source(now: datetime = V21_8_NOW) -> Any:
    return verify_v21_8(create_v21_7_source(), now=now)


def record(
    source: Any,
    operator: Any = "operator-001",
    text: Any = "RECORD OFFLINE PAPER AUDIT INTEGRITY VERIFICATION V21.9",
    existing: Any = None,
    policy: Any = None,
    now: datetime = NOW,
) -> Any:
...
def source_has_no_external_execution_calls() -> bool:
    source_path = (
        ...
        / "backtest"
        / "offline_paper_audit_verify_ledger_v21_9.py"
    )
...
def main() -> None:
    ...
    require(source_one.to_dict() == source_before, "V21.8 Source 변경")

    source_two = create_source(V21_8_NOW + timedelta(minutes=1))
    second = record(
        source_two,
    ...
    chain_valid, chain_errors = verify_audit_verify_ledger_chain(
        second.entries
    )
    require(chain_valid and not chain_errors, "V21.9 Ledger Hash Chain 실패")
    require(
    ...
    require(
        second.latest_audit_integrity_ledger_snapshot_hash
        == source_two.audit_integrity_ledger_snapshot_hash,
        "V21.7 Audit Integrity Ledger Snapshot Hash 보존 실패",
    )
    require(
    ...
    require(
        ...
        == source_two.latest_audit_certificate_ledger_snapshot_hash,
    ...
    wrong_operator_policy = OfflinePaperAuditVerifyLedgerV219Policy(
    ...
    tampered_source.certificate = replace(
        ...
        audit_integrity_ledger_snapshot_hash="a" * 64,
    )
    ...
    tampered_existing = record(
        create_source(V21_8_NOW + timedelta(minutes=2)),
    ...
    with tempfile.TemporaryDirectory() as directory:
        report_path, latest_path = save_audit_verification_ledger_result(
            second, Path(directory)
        )
        require(report_path.exists() and latest_path.exists(), "저장 실패")
        payload = load_audit_verification_ledger_result(latest_path)
        require(payload["version"] == "V21.9", "저장 Version 오류")
    ...
    checks = {
        "Version is V21.9": second.version == "V21.9",
        ...
        "V21.8 verification source passed": second.source_checks_passed,
        ...
        "V21.8 verification certificate hash was preserved": (
            second.latest_verification_certificate_hash
            == source_two.verification_certificate_hash
        ),
        "V21.7 ledger snapshot hash was preserved": (
            second.latest_audit_integrity_ledger_snapshot_hash
            == source_two.audit_integrity_ledger_snapshot_hash
        ),
        ...
        "Result save and load passed": payload["version"] == "V21.9",
    ...
    print("=" * 108)
    print(
        "AI STOCK BOT V21.9 OFFLINE PAPER AUDIT INTEGRITY "
        "VERIFICATION LEDGER TEST"
    )
    print("=" * 108)
    print("V21.9 VALIDATION CHECKS")
    print("-" * 108)
    for name, passed in checks.items():
        print(f"{name:<74}: {passed}")
    print("=" * 108)
    require(checks["All checks passed"], "V21.9 Validation Check 실패")
    print()
    print(
        "V21.9 offline paper audit integrity verification ledger "
        "test completed successfully."
    )
    print(
        "V21.8 Verification Certificate Hash, V21.7 Ledger Snapshot Hash, "
