...
from backtest.offline_paper_verify_cert_audit_ledger_verify_v22_4 import (
    OfflinePaperVerifyCertAuditLedgerVerifyV224Policy,
    ledger_snapshot_hash,
    load_audit_integrity_verification_result,
    save_audit_integrity_verification_result,
    verify_offline_paper_verification_certificate_audit_ledger_v22_4,
    verify_verification_certificate,
)
from test_offline_paper_verify_cert_audit_ledger_v22_3 import (
    NOW as V22_3_NOW,
    create_source as create_v22_2_source,
    record as record_v22_3,
)
...
def create_source() -> Any:
    first_source = create_v22_2_source()
    first = record_v22_3(first_source)
    second_source = create_v22_2_source(V22_3_NOW + timedelta(minutes=1))
    return record_v22_3(
        second_source,
        existing=first.entries,
        now=V22_3_NOW + timedelta(minutes=1),
    )
...
def source_has_no_external_execution_calls() -> bool:
    source_path = (
        ...
        / "backtest"
        / "offline_paper_verify_cert_audit_ledger_verify_v22_4.py"
    )
...
def main() -> None:
    ...
    require(result.version == "V22.4", "Version 오류")
    ...
    require(source.to_dict() == source_before, "V22.3 Source가 변경됨")
    require(
        result.verification_certificate_ledger_snapshot_hash
        == ledger_snapshot_hash(source.entries),
        "V22.3 Ledger Snapshot Hash 오류",
    )
    require(
        result.latest_ledger_entry_hash == source.latest_ledger_entry_hash,
        "V22.3 Latest Ledger Hash 보존 실패",
    )
    require(
    ...
    require(
        ...
        "V22.0 Audit Verification Certificate Audit Ledger Snapshot Hash 보존 실패",
    )
    require(
        result.latest_source_verification_certificate_hash
        == source.latest_source_verification_certificate_hash,
        "V21.8 Source Verification Certificate Hash 보존 실패",
    )
    require(
        result.latest_source_audit_verify_ledger_snapshot_hash
        == source.latest_source_audit_verify_ledger_snapshot_hash,
        "V21.9 Source Audit Verification Certificate Audit Ledger Snapshot Hash 보존 실패",
    )
    require(
    ...
    require(
        ...
        "V22.4 Verification Certificate Hash 실패",
    )
    require_safe(result)
    ...
    try:
        ...
        immutable = True
    require(immutable, "V22.4 Policy가 immutable이 아닙니다.")
    ...
    tampered_certificate = replace(
        result.certificate,
        verification_certificate_ledger_snapshot_hash="c" * 64,
    )
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        require(loaded["version"] == "V22.4", "저장 Version 오류")
        require(
    ...
    checks = {
        "Version is V22.4": result.version == "V22.4",
        ...
        "V22.3 ledger source passed": result.source_checks_passed,
        ...
        "V22.3 ledger remained unchanged": result.ledger_unchanged_checks_passed,
        ...
        "Result save and load passed": loaded["version"] == "V22.4",
    ...
    print("=" * 78)
    print(
        "AI STOCK BOT V22.4 OFFLINE PAPER VERIFICATION "
        "CERTIFICATE AUDIT LEDGER INTEGRITY AUDIT TEST"
    )
    print("=" * 78)
    print("V22.4 VALIDATION CHECKS")
    print("-" * 78)
    for label, passed in checks.items():
        print(f"{label:<{width}} : {passed}")
    print("=" * 78)
    require(all(checks.values()), "V22.4 표시 검증 중 실패가 있습니다.")
    print()
    print("V22.4 offline paper verification certificate audit ledger integrity audit test completed successfully.")
    print(
        "V22.3 Ledger Chain, Entry Findings, Snapshot Hash, 읽기 전용 검증 및 "
