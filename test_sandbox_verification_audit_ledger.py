...
from test_sandbox_verification_ledger_integrity_audit import (
    audit as audit_verification_ledger,
)
from test_sandbox_verification_ledger_integrity_audit import (
    create_source as create_verification_ledger_source,
)

from backtest.sandbox_verification_audit_ledger import (
    SandboxVerificationAuditLedgerPolicy,
    load_verification_audit_ledger_result,
    record_sandbox_verification_audit,
    save_verification_audit_ledger_result,
...
def create_source() -> Any:
    return audit_verification_ledger(create_verification_ledger_source())


def record(
    source: Any,
    operator: str = "operator-001",
    text: str = "RECORD IN MEMORY SANDBOX VERIFICATION AUDIT",
    existing: Any = None,
    policy: Any = None,
    now: datetime = NOW,
) -> Any:
...
def main() -> None:
    ...
    valid_chain, chain_errors = verify_verification_audit_ledger_chain(
        second.entries
    )
    ...
    unsafe_policy = SandboxVerificationAuditLedgerPolicy(
    ...
    changed_entry = replace(
        second.entries[0], verification_ledger_snapshot_hash="e" * 64
    )
    changed_entries = (changed_entry, second.entries[1])
    changed_valid, changed_errors = verify_verification_audit_ledger_chain(
        changed_entries
    )
    ...
    with tempfile.TemporaryDirectory() as directory:
        report_path, latest_path = save_verification_audit_ledger_result(
            second, Path(directory)
        )
        require(report_path.exists() and latest_path.exists(), "저장 실패")
        payload = load_verification_audit_ledger_result(latest_path)
        require(payload["version"] == "V15.7", "저장 Version 오류")
    ...
    checks = {
        "Version is V15.7": second.version == "V15.7",
        "Default policy is valid": second.policy_checks_passed,
        "Policy is immutable": (
            SandboxVerificationAuditLedgerPolicy.__dataclass_params__.frozen
        ),
        "Invalid policies are blocked": not unsafe.all_checks_passed,
        "V15.6 audit source passed": second.source_checks_passed,
        ...
        "Verification audit ledger hash chain passed": valid_chain,
        "V15.6 audit certificate hash was preserved": (
            first.entries[0].audit_certificate_hash
            == first_source.audit_certificate_hash
        ),
        ...
        "Result save and load passed": payload["version"] == "V15.7",
    ...
    print("=" * 104)
    print("AI STOCK BOT V15.7 SANDBOX VERIFICATION AUDIT LEDGER TEST")
    print("=" * 104)
    print("V15.7 VALIDATION CHECKS")
    print("-" * 104)
    for name, passed in checks.items():
        print(f"{name:<70}: {passed}")
    print("=" * 104)
    require(checks["All checks passed"], "V15.7 Validation Check 실패")
    print()
    print(
        "V15.7 sandbox verification audit ledger test "
        "completed successfully."
    )
    print(
        "V15.6 Audit Certificate Hash와 Verification Ledger Snapshot Hash 보존, "
        "Ledger Sequence, SHA-256 Hash Chain 및 중복·역순·변조 차단이 "
        "검증되었습니다."
    )
    print(
