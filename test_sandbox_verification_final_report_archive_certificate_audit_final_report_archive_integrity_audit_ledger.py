...
from test_sandbox_verification_final_report_archive_certificate_audit_final_report_archive_integrity_audit import (
    audit,
)
from test_sandbox_verification_final_report_archive_certificate_audit_final_report_archive_integrity_audit import (
    create_source as create_archive_source,
)
...
from backtest.sandbox_verification_final_report_archive_certificate_audit_final_report_archive_integrity_audit_ledger import (
    ...
    load_audit_ledger_result,
    record_sandbox_verification_final_report_archive_certificate_audit_final_report_archive_integrity_audit,
    save_audit_ledger_result,
    verify_audit_ledger_chain,
)


NOW = datetime(2026, 7, 28, 23, 50, tzinfo=timezone.utc)
...
def create_source() -> Any:
    return audit(create_archive_source())
...
def main() -> None:
    ...
    valid_chain, chain_errors = verify_audit_ledger_chain(
        second.entries
    )
    require(valid_chain and not chain_errors, "Audit Ledger Chain 실패")
    ...
    changed_valid, changed_errors = verify_audit_ledger_chain(
        changed_entries
    )
    require(
    ...
    with tempfile.TemporaryDirectory() as directory:
        report_path, latest_path = save_audit_ledger_result(
            second,
            Path(directory),
        )
        require(
        ...
        payload = load_audit_ledger_result(latest_path)
        require(payload["version"] == "V17.3", "저장 Version 오류")
    ...
    checks = {
        "Version is V17.3": second.version == "V17.3",
        ...
        "V17.2 audit source passed": second.source_checks_passed,
        ...
        "Audit ledger hash chain passed": valid_chain,
        ...
        "Result save and load passed": payload["version"] == "V17.3",
    ...
    print("=" * 108)
    print(
        "AI STOCK BOT V17.3 SANDBOX VERIFICATION FINAL REPORT "
        "ARCHIVE CERTIFICATE AUDIT FINAL REPORT ARCHIVE INTEGRITY "
        "AUDIT LEDGER TEST"
    )
    print("=" * 108)
    print("V17.3 VALIDATION CHECKS")
    print("-" * 108)
    for name, passed in checks.items():
        print(f"{name:<74}: {passed}")
    print("=" * 108)
    require(checks["All checks passed"], "V17.3 Validation Check 실패")
    print()
    print(
        "V17.3 sandbox verification final report archive audit "
        "ledger test completed successfully."
    )
    print(
        "V17.2 Audit Certificate Hash, Final Report Hash, 연결 Hash, "
        "Certificate Ledger Snapshot Hash 보존 및 Ledger Chain이 검증되었습니다."
    )
    print(
