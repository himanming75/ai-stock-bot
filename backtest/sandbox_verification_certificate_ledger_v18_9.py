...
from backtest.sandbox_verification_audit_ledger_integrity_v18_8 import (
    SandboxVerificationAuditLedgerIntegrityV188Result as SourceVerificationResult,
...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "sandbox_verification_certificate_ledger_v18_9"
)
REQUIRED_LEDGER_TEXT = (
    "RECORD IN MEMORY SANDBOX VERIFICATION CERTIFICATE V18.8"
)
GENESIS_HASH = "0" * 64


@dataclass(frozen=True)
class SandboxVerificationCertificateLedgerV189Policy:
    required_source_version: str = "V18.8"
...
@dataclass(frozen=True)
class SandboxVerificationCertificateLedgerV189Entry:
    ledger_entry_id: str
    sequence: int
    recorded_at: str
...
@dataclass
class SandboxVerificationCertificateLedgerV189Result:
    version: str
    created_at: str
    ...
    live_execution_authorized: bool
    ledger_policy: (
        SandboxVerificationCertificateLedgerV189Policy
    )
    entries: tuple[
        SandboxVerificationCertificateLedgerV189Entry, ...
    ]
...
def validate_policy(
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V18.9 기준과 다릅니다.")
    ...
    for name in (
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V18.9에서 True여야 합니다.")
    return errors
...
def _result(
    ...
    reasons: list[str],
    **checks: bool,
) -> SandboxVerificationCertificateLedgerV189Result:
    latest = entries[-1] if entries else None
    return SandboxVerificationCertificateLedgerV189Result(
        version="V18.9",
        ...
        reasons=reasons,
        warnings=[
            "V18.9는 V18.8 Verification Certificate를 기록만 합니다.",
...
def record_sandbox_verification_certificate_v18_9(
    source: Any,
    operator: str,
    ...
    policy = (
        policy
        or SandboxVerificationCertificateLedgerV189Policy()
    )
    ...
    if policy_errors or input_errors:
        safe_policy = (
            ...
            else SandboxVerificationCertificateLedgerV189Policy()
        )
        return _result(
            safe_policy,
            now,
    ...
    if not isinstance(
        ...
        return _result(
            ...
            "FAILED",
            entries,
            ["Source는 V18.8 Verification Archive Ledger Integrity Result여야 합니다."],
            policy=True,
            input=True,
            existing=True,
            chain=True,
    ...
    if not (
        ...
        source_errors.append("정상 V18.8 Verification Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V18.8 Source 실행 안전장치 오류입니다.")
...
def save_certificate_ledger_result(
    result: SandboxVerificationCertificateLedgerV189Result,
