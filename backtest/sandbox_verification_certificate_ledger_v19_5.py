...
from backtest.sandbox_archive_audit_ledger_integrity_v19_4 import (
    SandboxArchiveAuditLedgerIntegrityV194Result as SourceVerificationResult,
...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "sandbox_verification_certificate_ledger_v19_5"
)
REQUIRED_LEDGER_TEXT = (
    "RECORD IN MEMORY SANDBOX VERIFICATION CERTIFICATE V19.4"
)
GENESIS_HASH = "0" * 64


@dataclass(frozen=True)
class SandboxVerificationCertificateLedgerV195Policy:
    required_source_version: str = "V19.4"
...
@dataclass(frozen=True)
class SandboxVerificationCertificateLedgerV195Entry:
    ledger_entry_id: str
    sequence: int
    recorded_at: str
...
@dataclass
class SandboxVerificationCertificateLedgerV195Result:
    version: str
    created_at: str
    ...
    live_execution_authorized: bool
    ledger_policy: (
        SandboxVerificationCertificateLedgerV195Policy
    )
    entries: tuple[
        SandboxVerificationCertificateLedgerV195Entry, ...
    ]
...
def validate_policy(
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V19.5 기준과 다릅니다.")
    ...
    for name in (
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V19.5에서 True여야 합니다.")
    return errors
...
def _result(
    ...
    reasons: list[str],
    **checks: bool,
) -> SandboxVerificationCertificateLedgerV195Result:
    latest = entries[-1] if entries else None
    return SandboxVerificationCertificateLedgerV195Result(
        version="V19.5",
        ...
        reasons=reasons,
        warnings=[
            "V19.5는 V19.4 Verification Certificate를 기록만 합니다.",
...
def record_sandbox_verification_certificate_v19_5(
    source: Any,
    operator: str,
    ...
    policy = (
        policy
        or SandboxVerificationCertificateLedgerV195Policy()
    )
    ...
    if policy_errors or input_errors:
        safe_policy = (
            ...
            else SandboxVerificationCertificateLedgerV195Policy()
        )
        return _result(
            safe_policy,
            now,
    ...
    if not isinstance(
        ...
        return _result(
            ...
            "FAILED",
            entries,
            ["Source는 V19.4 Verification Archive Ledger Integrity Result여야 합니다."],
            policy=True,
            input=True,
            existing=True,
            chain=True,
    ...
    if not (
        ...
        source_errors.append("정상 V19.4 Verification Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V19.4 Source 실행 안전장치 오류입니다.")
...
def save_certificate_ledger_result(
    result: SandboxVerificationCertificateLedgerV195Result,
