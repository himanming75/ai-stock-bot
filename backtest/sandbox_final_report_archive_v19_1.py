...
from backtest.sandbox_verification_final_report_v19_0 import (
    SandboxVerificationFinalReportV190Result,
    verify_final_report,
)
...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "sandbox_final_report_archive_v19_1"
)
REQUIRED_ARCHIVE_TEXT = (
    "ARCHIVE IN MEMORY SANDBOX VERIFICATION FINAL REPORT V19.0"
)
GENESIS_HASH = "0" * 64


@dataclass(frozen=True)
class SandboxFinalReportArchiveV191Policy:
    required_source_version: str = "V19.0"
...
@dataclass(frozen=True)
class SandboxFinalReportArchiveV191Entry:
...
@dataclass
class SandboxFinalReportArchiveV191Result:
    version: str
    created_at: str
    ...
    live_execution_authorized: bool
    archive_policy: SandboxFinalReportArchiveV191Policy
    entries: tuple[SandboxFinalReportArchiveV191Entry, ...]
...
def validate_policy(
    policy: SandboxFinalReportArchiveV191Policy,
) -> list[str]:
    if not isinstance(policy, SandboxFinalReportArchiveV191Policy):
        return ["Sandbox Verification Final Report Archive Policy 형식 오류입니다."]
    errors: list[str] = []
    expected = {
        "required_source_version": "V19.0",
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V19.1 기준과 다릅니다.")
    ...
    for name in (
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V19.1에서 True여야 합니다.")
    return errors


def normalize_entries(
    existing: Any,
) -> tuple[SandboxFinalReportArchiveV191Entry, ...]:
    if existing is None:
        return ()
    if not isinstance(existing, (tuple, list)):
        raise TypeError("Existing Archive는 tuple/list여야 합니다.")
    if not all(
        isinstance(entry, SandboxFinalReportArchiveV191Entry)
...
def _safe_source(source: SandboxVerificationFinalReportV190Result) -> bool:
    return not any(
        (
...
def _result(
    ...
    reasons: list[str],
    **checks: bool,
) -> SandboxFinalReportArchiveV191Result:
    latest = entries[-1] if entries else None
    return SandboxFinalReportArchiveV191Result(
        version="V19.1",
        ...
        latest_carried_final_report_hash=(
            latest.carried_final_report_hash if latest else None
        ),
        latest_linked_final_report_hash=(
            latest.linked_final_report_hash if latest else None
        ),
        ...
        reasons=reasons,
        warnings=[
            "V19.1은 V19.0 Final Report를 In-Memory Archive에 기록만 합니다.",
...
def archive_sandbox_final_report_v19_1(
    source: Any,
    operator: str,
    confirmation_text: str,
    existing: Any = None,
    policy: SandboxFinalReportArchiveV191Policy | None = None,
    now: datetime | None = None,
) -> SandboxFinalReportArchiveV191Result:
    policy = policy or SandboxFinalReportArchiveV191Policy()
    ...
    if policy_errors or input_errors:
        safe_policy = (
            policy
            if isinstance(policy, SandboxFinalReportArchiveV191Policy)
            else SandboxFinalReportArchiveV191Policy()
        )
        return _result(
    ...
    if not isinstance(source, SandboxVerificationFinalReportV190Result):
        return _result(
            policy, now, "FAILED", entries,
            ["Source는 V19.0 Sandbox Verification Final Report Result여야 합니다."],
    ...
    if not (
        ...
        source_errors.append("정상 V19.0 Verification Final Report가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V19.0 Source 실행 안전장치 오류입니다.")
    ...
    for actual, expected, label in (
        ...
        (
            ...
            report.latest_final_report_hash,
            "Carried Final Report Hash",
        ),
        (
            source.latest_linked_final_report_hash,
            report.latest_linked_final_report_hash,
        ...
        (
            ...
            report.latest_source_certificate_ledger_snapshot_hash,
        ...
        (
            ...
            report.latest_archived_certificate_ledger_snapshot_hash,
    ...
    if operator != report.operator:
        return _result(
            ...
            ["Operator가 V19.0 Final Report와 다릅니다."],
            policy=True, input=True, source=True, report_hash=True,
    ...
    payload = {
        ...
        "carried_final_report_hash": report.latest_final_report_hash,
        "linked_final_report_hash": (
            report.latest_linked_final_report_hash
        ),
        ...
        "archived_certificate_ledger_snapshot_hash": (
            report.latest_archived_certificate_ledger_snapshot_hash
        ),
    ...
    combined = entries + (
        SandboxFinalReportArchiveV191Entry(
    ...
    if trimmed_count:
        ...
        rebuilt: list[SandboxFinalReportArchiveV191Entry] = []
        previous_hash = GENESIS_HASH
        for sequence, item in enumerate(kept, start=1):
            ...
            item = SandboxFinalReportArchiveV191Entry(
    ...
    result = _result(
        ...
        combined,
        [
            f"V19.0 Final Report {report.final_report_id}를 보관했습니다.",
...
def save_archive_result(
    result: SandboxFinalReportArchiveV191Result,
    output_directory: Path = OUTPUT_DIRECTORY,
) -> tuple[Path, Path]:
    output_directory.mkdir(parents=True, exist_ok=True)
    stamp = result.created_at.replace(":", "").replace("-", "").replace("+", "_")
    report_path = output_directory / (
        f"sandbox_final_report_archive_v19_1_{stamp}.json"
    )
    latest_path = output_directory / (
        "latest_sandbox_final_report_archive_v19_1.json"
    )
