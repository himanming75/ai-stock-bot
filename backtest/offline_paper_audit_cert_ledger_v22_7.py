...
from backtest.offline_paper_audit_verify_cert_ledger_audit_v22_6 import (
...
OUTPUT_DIRECTORY = PROJECT_ROOT / "output" / "trading_engine" / "offline_paper_audit_cert_ledger_v22_7"
CONFIRMATION_TEXT = "RECORD OFFLINE PAPER AUDIT CERTIFICATE V22.7"
GENESIS_HASH = "0" * 64
...
class OfflinePaperAuditCertLedgerV227Policy:
    required_source_version: str = "V22.6"
...
@dataclass(frozen=True)
class OfflinePaperAuditCertLedgerV227Entry:
    ledger_entry_id: str
    sequence: int
    recorded_at: str
    ...
    source_verification_ledger_snapshot_hash: str
...
@dataclass
class OfflinePaperAuditCertLedgerV227Result:
    version: str
    created_at: str
    ...
    latest_source_audit_result_id: str | None
    latest_source_audit_certificate_id: str | None
    latest_source_audit_certificate_hash: str | None
    latest_source_verification_ledger_snapshot_hash: str | None
    ...
    order_submitted: bool
    live_execution_authorized: bool
    ledger_policy: OfflinePaperAuditCertLedgerV227Policy
...
def validate_policy(policy: OfflinePaperAuditCertLedgerV227Policy) -> list[str]:
    if not isinstance(policy, OfflinePaperAuditCertLedgerV227Policy):
        return ["V22.7 Policy 형식 오류입니다."]
    errors: list[str] = []
    if policy.required_source_version != "V22.6":
        errors.append("V22.7 Source Version은 V22.6이어야 합니다.")
    if policy.required_source_status != "AUDITED_IN_MEMORY":
        errors.append("V22.7 Source Status 정책 오류입니다.")
    if policy.required_audit_status != "PASSED":
        errors.append("V22.7 Audit Status 정책 오류입니다.")
    if policy.required_confirmation_text != CONFIRMATION_TEXT:
        errors.append("V22.7 확인 문구 정책 오류입니다.")
    if (
        ...
        errors.append("V22.7 Ledger 보관 한도는 양의 정수여야 합니다.")
    for name, value in policy.to_dict().items():
        if name.startswith(("require_", "reject_", "verify_", "ledger_")) or name.endswith("_disabled"):
            if value is not True:
                errors.append(f"{name}는 V22.7에서 True여야 합니다.")
    return errors


def normalize_entries(existing: Any) -> tuple[OfflinePaperAuditCertLedgerV227Entry, ...]:
    if existing is None:
        return ()
    if not isinstance(existing, (tuple, list)):
        raise TypeError("V22.7 Existing Ledger는 tuple/list여야 합니다.")
    if not all(isinstance(item, OfflinePaperAuditCertLedgerV227Entry) for item in existing):
        raise TypeError("V22.7 Existing Ledger Entry 형식 오류입니다.")
...
def _safe_source(source: OfflinePaperAuditVerifyCertLedgerAuditV226Result) -> bool:
    return bool(
...
def verify_ledger_chain(
    ...
    for expected, entry in enumerate(entries, start=1):
        if entry.sequence != expected:
            errors.append(f"V22.7 Sequence {expected} 오류입니다.")
        if entry.previous_entry_hash != previous_hash:
            errors.append(f"V22.7 Sequence {entry.sequence} Previous Hash 오류입니다.")
        if entry.entry_hash != sha256_payload(entry.payload_without_hash()):
            errors.append(f"V22.7 Sequence {entry.sequence} Entry Hash 오류입니다.")
        if entry.source_audit_certificate_id in certificate_ids:
            errors.append("V22.7 중복 Audit Certificate ID입니다.")
        ...
        try:
            ...
            if previous_time and current < previous_time:
                errors.append("V22.7 Ledger 시간이 역순입니다.")
            previous_time = current
        except (TypeError, ValueError):
            errors.append("V22.7 Ledger 시간이 올바르지 않습니다.")
        if entry.audit_status != "PASSED" or not _safe_entry(entry):
            errors.append("V22.7 Ledger Entry 안전성 오류입니다.")
        previous_hash = entry.entry_hash
    return not errors, errors


def ledger_snapshot_hash(entries: tuple[OfflinePaperAuditCertLedgerV227Entry, ...]) -> str:
    return sha256_payload([entry.to_dict() for entry in entries])


def _result(
    policy: OfflinePaperAuditCertLedgerV227Policy,
    now: datetime,
    status: str,
    entries: tuple[OfflinePaperAuditCertLedgerV227Entry, ...],
    reasons: list[str],
    ...
    **checks: bool,
    ...
    return OfflinePaperAuditCertLedgerV227Result(
        version="V22.7", created_at=now.isoformat(), ledger_result_id=str(uuid.uuid4()),
        ...
        latest_source_audit_result_id=latest.source_audit_result_id if latest else None,
        latest_source_audit_certificate_id=latest.source_audit_certificate_id if latest else None,
        latest_source_audit_certificate_hash=latest.source_audit_certificate_hash if latest else None,
        latest_source_verification_ledger_snapshot_hash=(
            latest.source_verification_ledger_snapshot_hash if latest else None
        ),
...
def _rebuild(entries: tuple[OfflinePaperAuditCertLedgerV227Entry, ...]) -> tuple[OfflinePaperAuditCertLedgerV227Entry, ...]:
    rebuilt = []
    ...
    for sequence, old in enumerate(entries, start=1):
        ...
        entry = OfflinePaperAuditCertLedgerV227Entry(**payload, entry_hash=sha256_payload(payload))
        rebuilt.append(entry)
        previous = entry.entry_hash
    return tuple(rebuilt)


def record_offline_paper_audit_certificate_v22_7(
    source: OfflinePaperAuditVerifyCertLedgerAuditV226Result,
    *,
    operator: str,
    confirmation_text: str,
    existing: Any = None,
    policy: OfflinePaperAuditCertLedgerV227Policy | None = None,
    now: datetime | None = None,
) -> OfflinePaperAuditCertLedgerV227Result:
    selected = policy or OfflinePaperAuditCertLedgerV227Policy()
    ...
    if not isinstance(operator, str) or not operator.strip():
        errors.append("V22.7 Operator가 필요합니다.")
    if confirmation_text != CONFIRMATION_TEXT:
        errors.append("V22.7 확인 문구가 일치하지 않습니다.")
    try:
    ...
    if not isinstance(source, OfflinePaperAuditVerifyCertLedgerAuditV226Result):
        return _result(selected, current, "FAILED", entries, errors + ["Source는 V22.6 Result여야 합니다."])
    before = source.to_dict()
    certificate_ok, certificate_errors = verify_audit_certificate(source.certificate)
    if source.version != "V22.6" or source.result_status != "AUDITED_IN_MEMORY" or not source.all_checks_passed:
        errors.append("V22.6 Source 상태 오류입니다.")
    if not certificate_ok:
        errors.extend(certificate_errors)
    source_safe = _safe_source(source)
    if not source_safe:
        errors.append("V22.6 Source 실행 안전성 오류입니다.")
    linkage_ok = bool(
        source.certificate
        and source.audit_certificate_id == source.certificate.audit_certificate_id
        and source.audit_certificate_hash == source.certificate.certificate_hash
    ...
    if not linkage_ok:
        errors.append("V22.6 Source 연결 Hash 오류입니다.")
    duplicate_ok = bool(
        source.audit_certificate_id
        and all(entry.source_audit_certificate_id != source.audit_certificate_id for entry in entries)
    )
    if not duplicate_ok:
        errors.append("V22.6 Audit Certificate가 이미 기록되었습니다.")
    chronology_ok = not entries or current >= _parse_time(entries[-1].recorded_at)
    if not chronology_ok:
        errors.append("V22.7 기록 시간이 역순입니다.")
    if source.certificate and current < _parse_time(source.certificate.audited_at):
        errors.append("V22.7 기록 시간이 감사 시간보다 빠릅니다.")
        chronology_ok = False
    if errors:
    ...
    payload = {
        ...
        "source_audit_certificate_id": source.audit_certificate_id,
        "source_audit_certificate_hash": source.audit_certificate_hash,
        "source_verification_ledger_result_id": source.source_ledger_result_id,
        "source_verification_ledger_snapshot_hash": source.source_ledger_snapshot_hash,
        "latest_source_verification_certificate_hash": (
            source.certificate.latest_source_verification_certificate_hash
        ),
    ...
    return _result(
        ...
        ["V22.6 Audit Certificate가 Offline Ledger에 기록되었습니다."], trimmed,
...
def save_result(
    result: OfflinePaperAuditCertLedgerV227Result,
...
def load_result(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("V22.7 Ledger JSON은 object여야 합니다.")
    return payload
