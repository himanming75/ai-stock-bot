...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "sandbox_verification_final_report_archive_integrity_audit"
)
REQUIRED_AUDIT_TEXT = (
...
class SandboxVerificationFinalReportArchiveIntegrityAuditPolicy:
    required_source_version: str = "V16.1"
...
@dataclass(frozen=True)
class SandboxVerificationFinalReportArchiveIntegrityFinding:
    sequence: int
...
@dataclass(frozen=True)
class SandboxVerificationFinalReportArchiveIntegrityCertificate:
    ...
    findings: tuple[
        SandboxVerificationFinalReportArchiveIntegrityFinding, ...
    ]
...
@dataclass
class SandboxVerificationFinalReportArchiveIntegrityAuditResult:
    version: str
    created_at: str
    audit_result_id: str
    result_status: str
    ...
    certificate: (
        SandboxVerificationFinalReportArchiveIntegrityCertificate | None
    )
...
def validate_policy(
    policy: SandboxVerificationFinalReportArchiveIntegrityAuditPolicy,
) -> list[str]:
    if not isinstance(
        policy,
        SandboxVerificationFinalReportArchiveIntegrityAuditPolicy,
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V16.2 기준과 다릅니다.")
    for name in (
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V16.2에서 True여야 합니다.")
    return errors


def _safe_entry(
    entry: SandboxVerificationFinalReportArchiveEntry,
) -> bool:
    return not any(
        (
...
def _safe_source(
    source: SandboxVerificationFinalReportArchiveResult,
) -> bool:
    return not any(
        (
...
def verify_audit_certificate(
    certificate: (
        SandboxVerificationFinalReportArchiveIntegrityCertificate
    ),
) -> tuple[bool, list[str]]:
    if not isinstance(
        certificate,
...
def _empty_result(
    ...
    **checks: bool,
    ...
    return SandboxVerificationFinalReportArchiveIntegrityAuditResult(
        version="V16.2",
        ...
        reasons=reasons,
        warnings=[
            "V16.2는 V16.1 Archive를 읽기 전용으로 감사합니다.",
            "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
        ],
        next_actions=["V16.1 Archive Source와 입력을 수동 확인합니다."],
    )


def audit_sandbox_verification_final_report_archive_integrity(
    source: Any,
    operator: str,
    confirmation_text: str,
    policy: (
        SandboxVerificationFinalReportArchiveIntegrityAuditPolicy | None
    ) = None,
    now: datetime | None = None,
) -> SandboxVerificationFinalReportArchiveIntegrityAuditResult:
    policy = (
        policy
        or SandboxVerificationFinalReportArchiveIntegrityAuditPolicy()
    )
    ...
    if policy_errors or input_errors:
        safe_policy = (
            ...
            else SandboxVerificationFinalReportArchiveIntegrityAuditPolicy()
        )
    ...
    if not isinstance(
        ...
        return _empty_result(
            ...
            now,
            "FAILED",
            ["Source는 V16.1 Verification Final Report Archive여야 합니다."],
            policy=True,
            input=True,
        )
    ...
    if not (
        ...
        source_errors.append("정상 V16.1 Archive Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V16.1 Source 실행 안전장치 오류입니다.")
    entries = tuple(source.entries)
    if not entries:
        source_errors.append("V16.1 Archive Entry가 비어 있습니다.")
    if source_errors:
        return _empty_result(
            policy,
            now,
    ...
    if operator != latest.operator:
        return _empty_result(
            ...
            now,
            "BLOCKED",
            ["Operator가 V16.1 Latest Archive Entry와 다릅니다."],
            policy=True,
            input=True,
            source=True,
            chain=True,
...
def save_audit_result(
    result: SandboxVerificationFinalReportArchiveIntegrityAuditResult,
    ...
    report_path = output_directory / (
        f"verification_final_report_archive_integrity_audit_{stamp}.json"
    )
    latest_path = output_directory / (
        "latest_verification_final_report_archive_integrity_audit.json"
    )
