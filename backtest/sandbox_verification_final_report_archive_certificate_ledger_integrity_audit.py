...
from backtest.sandbox_verification_final_report_archive_verification_certificate_ledger import (
    GENESIS_HASH,
...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "sandbox_verification_final_report_archive_certificate_ledger_audit"
)
REQUIRED_AUDIT_TEXT = (
    "AUDIT IN MEMORY SANDBOX VERIFICATION FINAL REPORT ARCHIVE "
    "CERTIFICATE LEDGER INTEGRITY"
)


@dataclass(frozen=True)
class SandboxVerificationFinalReportArchiveCertificateLedgerAuditPolicy:
    required_source_version: str = "V16.5"
...
@dataclass(frozen=True)
class SandboxVerificationFinalReportArchiveCertificateLedgerFinding:
    sequence: int
    ledger_entry_id: str
...
@dataclass(frozen=True)
class SandboxVerificationFinalReportArchiveCertificateLedgerAuditCertificate:
    ...
    findings: tuple[
        SandboxVerificationFinalReportArchiveCertificateLedgerFinding, ...
    ]
...
@dataclass
class SandboxVerificationFinalReportArchiveCertificateLedgerAuditResult:
    version: str
    created_at: str
    audit_result_id: str
    result_status: str
    ...
    live_execution_authorized: bool
    audit_policy: (
        SandboxVerificationFinalReportArchiveCertificateLedgerAuditPolicy
    )
    certificate: (
        SandboxVerificationFinalReportArchiveCertificateLedgerAuditCertificate
        | None
    )
...
def validate_policy(
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V16.6 기준과 다릅니다.")
    for name in (
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V16.6에서 True여야 합니다.")
    return errors


def _safe_entry(
    entry: SandboxVerificationFinalReportArchiveCertificateLedgerEntry,
) -> bool:
    return not any(
        (
...
def _safe_source(
    source: SandboxVerificationFinalReportArchiveCertificateLedgerResult,
) -> bool:
    return not any(
        (
...
def _empty_result(
    ...
    reasons: list[str],
    **checks: bool,
) -> SandboxVerificationFinalReportArchiveCertificateLedgerAuditResult:
    return SandboxVerificationFinalReportArchiveCertificateLedgerAuditResult(
        version="V16.6",
        ...
        reasons=reasons,
        warnings=[
            "V16.6은 V16.5 Certificate Ledger를 읽기 전용으로 감사합니다.",
            "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
        ],
        next_actions=["V16.5 Certificate Ledger Source와 입력을 확인합니다."],
    )


def audit_sandbox_verification_final_report_archive_certificate_ledger(
    source: Any,
    operator: str,
    ...
    policy = (
        policy
        or SandboxVerificationFinalReportArchiveCertificateLedgerAuditPolicy()
    )
    ...
    if policy_errors or input_errors:
        safe_policy = (
            ...
            else SandboxVerificationFinalReportArchiveCertificateLedgerAuditPolicy()
        )
    ...
    if not isinstance(
        ...
        return _empty_result(
            policy, now, "FAILED",
            ["Source는 V16.5 Verification Certificate Ledger여야 합니다."],
    ...
    if not (
        ...
        source_errors.append("정상 V16.5 Certificate Ledger Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V16.5 Source 실행 안전장치 오류입니다.")
    if source_errors:
        return _empty_result(
    ...
    if operator != latest.operator:
        return _empty_result(
            policy, now, "BLOCKED",
            ["Operator가 V16.5 Latest Ledger Entry와 다릅니다."],
    ...
    return SandboxVerificationFinalReportArchiveCertificateLedgerAuditResult(
        ...
        reasons=[
            f"V16.5 Certificate Ledger Entry {len(entries)}개를 감사했습니다.",
            "Hash Chain, Source Linkage 및 Snapshot Hash가 유효합니다.",
        ],
        warnings=[
            "V16.6은 V16.5 Certificate Ledger를 변경하지 않습니다.",
            "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
        ],
        next_actions=[
...
def save_audit_result(
    result: SandboxVerificationFinalReportArchiveCertificateLedgerAuditResult,
    output_directory: Path = OUTPUT_DIRECTORY,
) -> tuple[Path, Path]:
    output_directory.mkdir(parents=True, exist_ok=True)
    stamp = result.created_at.replace(":", "").replace("-", "").replace("+", "_")
    report = output_directory / f"certificate_ledger_audit_{stamp}.json"
    latest = output_directory / "latest_certificate_ledger_audit.json"
