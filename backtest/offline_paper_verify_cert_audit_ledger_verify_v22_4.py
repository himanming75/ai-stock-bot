...
from backtest.offline_paper_verify_cert_audit_ledger_v22_3 import (
    GENESIS_HASH,
    OfflinePaperVerifyCertAuditLedgerV223Entry,
...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "offline_paper_verify_cert_audit_ledger_verify_v22_4"
)
REQUIRED_VERIFICATION_TEXT = (
    "AUDIT OFFLINE PAPER VERIFICATION CERTIFICATE AUDIT LEDGER V22.4"
)


@dataclass(frozen=True)
class OfflinePaperVerifyCertAuditLedgerVerifyV224Policy:
    required_source_version: str = "V22.3"
    ...
    require_passed_verification_status: bool = True
...
@dataclass(frozen=True)
class OfflinePaperVerifyCertAuditLedgerVerifyFindingV224:
    sequence: int
    ledger_entry_id: str
...
@dataclass(frozen=True)
class OfflinePaperVerifyCertAuditLedgerVerifyCertificateV224:
    verification_certificate_id: str
    verified_at: str
    ...
    latest_verification_result_id: str
    latest_verification_certificate_id: str
    latest_verification_certificate_hash: str
    latest_source_audit_verify_ledger_result_id: str
    latest_audit_verify_ledger_snapshot_hash: str
    ...
    verification_certificate_ledger_snapshot_hash: str
    ...
    findings: tuple[OfflinePaperVerifyCertAuditLedgerVerifyFindingV224, ...]
...
@dataclass
class OfflinePaperVerifyCertAuditLedgerVerifyV224Result:
    version: str
    created_at: str
    ...
    verification_certificate_ledger_snapshot_hash: str | None
    ...
    live_order_created: bool
    live_execution_authorized: bool
    verification_policy: OfflinePaperVerifyCertAuditLedgerVerifyV224Policy
    certificate: OfflinePaperVerifyCertAuditLedgerVerifyCertificateV224 | None
...
def validate_policy(
    policy: OfflinePaperVerifyCertAuditLedgerVerifyV224Policy,
) -> list[str]:
    if not isinstance(policy, OfflinePaperVerifyCertAuditLedgerVerifyV224Policy):
        return ["V22.4 Verification Policy 형식 오류입니다."]
    errors: list[str] = []
    expected = {
        "required_source_version": "V22.3",
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V22.4 기준과 다릅니다.")
    for name in (
        ...
        "require_passed_verification_status",
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V22.4에서 True여야 합니다.")
    return errors
...
def _safe_source(source: OfflinePaperVerifyCertAuditLedgerV223Result) -> bool:
    return not any(
        (
...
def _findings(
    entries: tuple[OfflinePaperVerifyCertAuditLedgerV223Entry, ...],
) -> tuple[OfflinePaperVerifyCertAuditLedgerVerifyFindingV224, ...]:
    output: list[OfflinePaperVerifyCertAuditLedgerVerifyFindingV224] = []
    ...
    for expected, entry in enumerate(entries, start=1):
        ...
        unique = entry.verification_certificate_id not in seen
        seen.add(entry.verification_certificate_id)
        try:
        ...
        output.append(
            OfflinePaperVerifyCertAuditLedgerVerifyFindingV224(
...
def verify_verification_certificate(
    ...
    if not isinstance(certificate, OfflinePaperVerifyCertAuditLedgerVerifyCertificateV224):
        return False, ["V22.4 Verification Certificate 형식 오류입니다."]
    errors: list[str] = []
    ...
    if certificate.verification_status != "PASSED":
        errors.append("V22.4 Verification Certificate Status 오류입니다.")
    if not all(
        (
        ...
        errors.append("V22.4 Verification Certificate 검증 항목 오류입니다.")
    if any(
        (
        ...
        errors.append("V22.4 Verification Certificate 안전장치 오류입니다.")
...
def _empty_result(
    ...
    **checks: bool,
    ...
    return OfflinePaperVerifyCertAuditLedgerVerifyV224Result(
        version="V22.4",
        created_at=now.isoformat(),
        verification_result_id=str(uuid.uuid4()),
        result_status=status,
        result_status_label=(
            "Offline Paper Verification Certificate Audit Ledger 감사 차단"
            if status == "BLOCKED"
            else "Offline Paper Verification Certificate Audit Ledger 감사 실패"
        ),
        ...
        latest_verification_certificate_hash=None,
        latest_audit_verify_ledger_snapshot_hash=None,
        latest_source_verification_certificate_hash=None,
        latest_source_audit_verify_ledger_snapshot_hash=None,
        ...
        verification_certificate_ledger_snapshot_hash=None,
        ...
        verification_status_checks_passed=checks.get("status_ok", False),
        ...
        reasons=reasons,
        warnings=[
            "V22.4은 V22.3 Ledger를 읽기 전용으로 검증합니다.",
            "검증은 주문 체결 또는 Broker 전송 승인을 의미하지 않습니다.",
            "잔액·보유수량 변경, API, Network 및 Live Execution은 차단됩니다.",
        ],
        next_actions=[
            "차단 또는 실패 원인을 수정한 뒤 V22.4 검증을 다시 실행합니다."
        ],
    )


def _linkage_errors(
    source: OfflinePaperVerifyCertAuditLedgerV223Result,
    latest: OfflinePaperVerifyCertAuditLedgerV223Entry,
) -> list[str]:
    checks = (
        ...
        (
            source.latest_verification_result_id,
            latest.verification_result_id,
        ...
        (
            source.latest_source_audit_verify_ledger_result_id,
            latest.source_audit_verify_ledger_result_id,
            "V22.0 Audit Verification Result ID",
        ),
        (
            source.latest_audit_verify_ledger_snapshot_hash,
            latest.audit_verify_ledger_snapshot_hash,
            "V22.0 Audit Verification Certificate Audit Ledger Snapshot Hash",
        ),
        (
            source.latest_source_verification_certificate_hash,
            latest.latest_source_verification_certificate_hash,
            "V22.0 Source Verification Certificate Hash",
        ),
        (
            source.latest_source_audit_verify_ledger_snapshot_hash,
            latest.latest_source_audit_verify_ledger_snapshot_hash,
            "V21.9 Source Verification Certificate Audit Ledger Snapshot Hash",
        ),
        (
    ...
    return [
        f"V22.3 Source {label} 연결이 다릅니다."
        for actual, expected, label in checks
        if actual != expected
    ]


def verify_offline_paper_verification_certificate_audit_ledger_v22_4(
    source: Any,
    operator: Any,
    confirmation_text: Any,
    policy: OfflinePaperVerifyCertAuditLedgerVerifyV224Policy | None = None,
    now: datetime | None = None,
) -> OfflinePaperVerifyCertAuditLedgerVerifyV224Result:
    policy = policy or OfflinePaperVerifyCertAuditLedgerVerifyV224Policy()
    ...
    if confirmation_text != getattr(policy, "required_confirmation_text", None):
        input_errors.append("V22.4 확인 문구가 일치하지 않습니다.")
    safe_policy = (
        policy
        if isinstance(policy, OfflinePaperVerifyCertAuditLedgerVerifyV224Policy)
        else OfflinePaperVerifyCertAuditLedgerVerifyV224Policy()
    )
    ...
    if not isinstance(source, OfflinePaperVerifyCertAuditLedgerV223Result):
        return _empty_result(
            policy,
            now,
            "FAILED",
            ["Source는 V22.3 Verification Certificate Ledger Result여야 합니다."],
            policy_ok=True,
            input=True,
        )
    ...
    if not (
        ...
        source_errors.append("정상 V22.3 Verification Certificate Ledger Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V22.3 Source 실행 안전장치 오류입니다.")
    if source_errors:
        return _empty_result(
            policy,
            now,
    ...
    if latest.operator != operator.strip():
        linkage_errors.append("Operator가 V22.3 Latest Entry와 다릅니다.")
    try:
        ...
        return _empty_result(
            ...
            now,
            "BLOCKED",
            ["역순 또는 잘못된 V22.4 Verification 시간이 차단되었습니다."],
            policy_ok=True,
            input=True,
            source=True,
        )

    unique_valid = len({item.verification_certificate_id for item in entries}) == len(entries)
    chronology_valid = all(item.chronology_valid for item in findings)
    status_valid = all(item.verification_status_valid for item in findings)
    ...
    if not unique_valid:
        errors.append("중복 V21.8 Verification Certificate ID가 발견되었습니다.")
    if not chronology_valid:
        errors.append("V22.3 Ledger 시간 순서 오류가 발견되었습니다.")
    if not status_valid:
        errors.append("V22.3 Ledger Verification Status 오류가 발견되었습니다.")
    if not safety_valid:
        errors.append("V22.3 Ledger Entry 안전장치 오류가 발견되었습니다.")
    if not unchanged:
        errors.append("V22.4 검증 중 V22.3 Source가 변경되었습니다.")
    if errors:
        return _empty_result(
            policy,
            now,
    ...
    certificate_payload = {
        ...
        "latest_verification_result_id": latest.verification_result_id,
        "latest_verification_certificate_id": latest.verification_certificate_id,
        ...
        "verification_certificate_ledger_snapshot_hash": snapshot_hash,
    ...
    certificate = OfflinePaperVerifyCertAuditLedgerVerifyCertificateV224(
        **certificate_payload,
    ...
    return OfflinePaperVerifyCertAuditLedgerVerifyV224Result(
        ...
        result_status_label="Offline Paper Verification Certificate Audit Ledger 무결성 감사 완료",
        verification_status="PASSED",
        source_ledger_result_id=source.ledger_result_id,
        ...
        reasons=[
            f"V22.3 Ledger Entry {len(entries)}개의 무결성을 검증했습니다.",
            "V22.4 Verification Certificate를 생성했습니다.",
        ],
        warnings=[
            "V22.4은 V22.3 Ledger를 읽기 전용으로 검증했습니다.",
            "검증은 주문 체결 또는 Broker 전송 승인을 의미하지 않습니다.",
            "잔액·보유수량 변경, API, Network 및 Live Execution은 차단됩니다.",
        ],
        next_actions=[
            "V22.3 단계에서 V22.4 Verification Certificate를 "
...
def save_audit_integrity_verification_result(
    result: OfflinePaperVerifyCertAuditLedgerVerifyV224Result,
...
def load_audit_integrity_verification_result(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("V22.4 Verification Certificate Audit Ledger Audit JSON은 object여야 합니다.")
    return payload
