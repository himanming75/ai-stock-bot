...
from backtest.offline_paper_verify_ledger_audit_v22_0 import (
    OfflinePaperVerifyLedgerAuditV220Result,
...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "offline_paper_verify_cert_ledger_v22_1"
)
REQUIRED_CERT_LEDGER_TEXT = (
    "RECORD OFFLINE PAPER VERIFICATION CERTIFICATE V22.1"
)
GENESIS_HASH = "0" * 64


@dataclass(frozen=True)
class OfflinePaperVerifyCertLedgerV221Policy:
    required_source_version: str = "V22.0"
    required_source_status: str = "VERIFIED_IN_MEMORY"
    required_verification_status: str = "PASSED"
    required_confirmation_text: str = REQUIRED_CERT_LEDGER_TEXT
...
@dataclass(frozen=True)
class OfflinePaperVerifyCertLedgerV221Entry:
    ledger_entry_id: str
    sequence: int
    recorded_at: str
...
@dataclass
class OfflinePaperVerifyCertLedgerV221Result:
    version: str
    created_at: str
    ...
    live_order_created: bool
    live_execution_authorized: bool
    ledger_policy: OfflinePaperVerifyCertLedgerV221Policy
    entries: tuple[OfflinePaperVerifyCertLedgerV221Entry, ...]
...
def validate_policy(
    policy: OfflinePaperVerifyCertLedgerV221Policy,
) -> list[str]:
    if not isinstance(policy, OfflinePaperVerifyCertLedgerV221Policy):
        return ["V22.1 Verification Certificate Ledger Policy 형식 오류입니다."]
    errors: list[str] = []
    expected = {
        "required_source_version": "V22.0",
        "required_source_status": "VERIFIED_IN_MEMORY",
        "required_verification_status": "PASSED",
        "required_confirmation_text": REQUIRED_CERT_LEDGER_TEXT,
    }
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V22.1 기준과 다릅니다.")
    if (
        ...
        errors.append("V22.1 Ledger 보관 한도는 양의 정수여야 합니다.")
    for name in (
        "require_operator",
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V22.1에서 True여야 합니다.")
    return errors


def normalize_entries(
    existing: Any,
) -> tuple[OfflinePaperVerifyCertLedgerV221Entry, ...]:
    if existing is None:
        return ()
    if not isinstance(existing, (tuple, list)):
        raise TypeError("Existing V22.1 Ledger는 tuple/list여야 합니다.")
    if not all(
        isinstance(entry, OfflinePaperVerifyCertLedgerV221Entry)
        for entry in existing
    ):
        raise TypeError("Existing V22.1 Ledger Entry 형식 오류입니다.")
    return tuple(existing)


def _safe_entry(entry: OfflinePaperVerifyCertLedgerV221Entry) -> bool:
    return not any(
        (
...
def _safe_source(source: OfflinePaperVerifyLedgerAuditV220Result) -> bool:
    return not any(
        (
...
def verify_verification_certificate_ledger_chain(
    entries: tuple[OfflinePaperVerifyCertLedgerV221Entry, ...],
) -> tuple[bool, list[str]]:
    errors: list[str] = []
    ...
    for expected_sequence, entry in enumerate(entries, start=1):
        if entry.sequence != expected_sequence:
            errors.append(f"V22.1 Ledger Sequence {expected_sequence} 오류입니다.")
        ...
        if entry.verification_certificate_id in certificate_ids:
            errors.append(
                "중복 V22.0 Verification Certificate ID: "
        ...
        try:
            ...
            if previous_time and recorded_at < previous_time:
                errors.append("V22.1 Ledger 기록 시간이 역순입니다.")
            previous_time = recorded_at
        except (TypeError, ValueError):
            errors.append("V22.1 Ledger 기록 시간이 올바르지 않습니다.")
        if entry.verification_status != "PASSED":
            errors.append("V22.1 Ledger Verification Status 오류입니다.")
        if not _safe_entry(entry):
            errors.append("V22.1 Ledger Entry 실행 안전장치 오류입니다.")
...
def _result(
    ...
    reasons: list[str],
    **checks: bool,
) -> OfflinePaperVerifyCertLedgerV221Result:
    latest = entries[-1] if entries else None
    recorded = status == "RECORDED_IN_MEMORY"
    return OfflinePaperVerifyCertLedgerV221Result(
        version="V22.1",
        ...
        reasons=reasons,
        warnings=[
            "V22.1은 V22.0 Verification Certificate를 In-Memory Ledger에 "
            "기록합니다.",
        ...
        next_actions=[
            ...
            "단계에서 V22.1 Ledger를 읽기 전용으로 검증합니다."
        ],
    )


def _source_linkage(
    source: OfflinePaperVerifyLedgerAuditV220Result,
) -> list[str]:
    certificate = source.certificate
    if certificate is None:
        return ["V22.0 Verification Certificate가 없습니다."]
    errors: list[str] = []
    for actual, expected, label in (
        (
            source.verification_certificate_id,
            certificate.verification_certificate_id,
            "V22.0 Verification Certificate ID",
        ),
        (
            source.verification_certificate_hash,
            certificate.certificate_hash,
            "V22.0 Verification Certificate Hash",
        ),
        (
        ...
        (
            source.audit_verify_ledger_snapshot_hash,
            certificate.audit_verify_ledger_snapshot_hash,
...
def record_offline_paper_verification_certificate_v22_1(
    source: Any,
    operator: Any,
    confirmation_text: Any,
    existing: Any = None,
    policy: OfflinePaperVerifyCertLedgerV221Policy | None = None,
    now: datetime | None = None,
) -> OfflinePaperVerifyCertLedgerV221Result:
    policy = policy or OfflinePaperVerifyCertLedgerV221Policy()
    ...
    if confirmation_text != getattr(policy, "required_confirmation_text", None):
        input_errors.append("V22.1 확인 문구가 일치하지 않습니다.")
    if policy_errors or input_errors:
        safe_policy = (
            policy
            if isinstance(policy, OfflinePaperVerifyCertLedgerV221Policy)
            else OfflinePaperVerifyCertLedgerV221Policy()
        )
        return _result(
            safe_policy,
            now,
    ...
    chain_valid, chain_errors = verify_verification_certificate_ledger_chain(entries)
    if not chain_valid:
        return _result(
            policy,
            now,
    ...
    if not isinstance(source, OfflinePaperVerifyLedgerAuditV220Result):
        return _result(
            policy,
            now,
            "FAILED",
            entries,
            ["Source는 V22.0 Verification Ledger Audit Result여야 합니다."],
            policy=True,
            input=True,
            existing=True,
            chain=True,
    ...
    if not (
        ...
        source_errors.append("정상 V22.0 Integrity Verification Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V22.0 Source 실행 안전장치 오류입니다.")
    ...
    if certificate.verification_status != policy.required_verification_status:
        linkage_errors.append(
            "V22.0 Verification Status가 V22.1 기준과 다릅니다."
        )
    if linkage_errors:
        return _result(
            policy,
    ...
    if any(
        ...
        return _result(
            ...
            "BLOCKED",
            entries,
            ["중복 V22.0 Verification Certificate ID가 차단되었습니다."],
            policy=True,
            input=True,
            source=True,
            certificate=True,
    ...
    try:
        ...
        return _result(
            ...
            "BLOCKED",
            entries,
            ["역순 또는 잘못된 V22.1 Ledger 시간이 차단되었습니다."],
            policy=True,
            input=True,
            source=True,
            certificate=True,
    ...
    if source_before != canonical_json(source.to_dict()):
        return _result(
            ...
            "FAILED",
            entries,
            ["V22.1 기록 중 V22.0 Source가 변경되었습니다."],
            policy=True,
            input=True,
            source=True,
            certificate=True,
    ...
    new_entry = OfflinePaperVerifyCertLedgerV221Entry(
        **payload,
    ...
    if trimmed_count:
        ...
        rebuilt: list[OfflinePaperVerifyCertLedgerV221Entry] = []
        previous_hash = GENESIS_HASH
    ...
    valid_chain, final_errors = verify_verification_certificate_ledger_chain(combined)
    result = _result(
        policy,
        now,
        ...
        combined,
        [
            "V22.0 Verification Certificate "
            f"{certificate.verification_certificate_id}를 기록했습니다.",
            f"현재 V22.1 Ledger Entry는 {len(combined)}개입니다.",
        ]
        + final_errors,
        policy=True,
        input=True,
...
def save_verification_certificate_ledger_result(
    result: OfflinePaperVerifyCertLedgerV221Result,
...
def load_verification_certificate_ledger_result(path: Path) -> dict[str, Any]:
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("V22.1 Verification Certificate Ledger JSON은 object여야 합니다.")
    return payload
