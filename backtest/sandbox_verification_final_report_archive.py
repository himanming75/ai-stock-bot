...
from backtest.sandbox_verification_final_report import (
...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "sandbox_verification_final_report_archive"
)
REQUIRED_ARCHIVE_TEXT = (
    "ARCHIVE IN MEMORY SANDBOX VERIFICATION FINAL REPORT"
)
GENESIS_HASH = "0" * 64


@dataclass(frozen=True)
class SandboxVerificationFinalReportArchivePolicy:
    required_source_version: str = "V16.0"
...
@dataclass(frozen=True)
class SandboxVerificationFinalReportArchiveEntry:
...
@dataclass
class SandboxVerificationFinalReportArchiveResult:
    version: str
    created_at: str
    ...
    live_execution_authorized: bool
    archive_policy: SandboxVerificationFinalReportArchivePolicy
    entries: tuple[SandboxVerificationFinalReportArchiveEntry, ...]
...
def validate_policy(
    policy: SandboxVerificationFinalReportArchivePolicy,
) -> list[str]:
    if not isinstance(policy, SandboxVerificationFinalReportArchivePolicy):
        return ["Sandbox Verification Final Report Archive Policy 형식 오류입니다."]
    errors: list[str] = []
    expected = {
        "required_source_version": "V16.0",
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V16.1 기준과 다릅니다.")
    ...
    for name in (
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V16.1에서 True여야 합니다.")
    return errors


def normalize_entries(
    existing: Any,
) -> tuple[SandboxVerificationFinalReportArchiveEntry, ...]:
    if existing is None:
        return ()
    if not isinstance(existing, (tuple, list)):
        raise TypeError("Existing Archive는 tuple/list여야 합니다.")
    if not all(
        isinstance(entry, SandboxVerificationFinalReportArchiveEntry)
...
def _safe_source(source: SandboxVerificationFinalReportResult) -> bool:
    return not any(
        (
...
def _result(
    ...
    reasons: list[str],
    **checks: bool,
) -> SandboxVerificationFinalReportArchiveResult:
    latest = entries[-1] if entries else None
    return SandboxVerificationFinalReportArchiveResult(
        version="V16.1",
        ...
        reasons=reasons,
        warnings=[
            "V16.1은 V16.0 Final Report를 In-Memory Archive에 기록만 합니다.",
...
def archive_sandbox_verification_final_report(
    source: Any,
    operator: str,
    confirmation_text: str,
    existing: Any = None,
    policy: SandboxVerificationFinalReportArchivePolicy | None = None,
    now: datetime | None = None,
) -> SandboxVerificationFinalReportArchiveResult:
    policy = policy or SandboxVerificationFinalReportArchivePolicy()
    ...
    if policy_errors or input_errors:
        safe_policy = (
            policy
            if isinstance(policy, SandboxVerificationFinalReportArchivePolicy)
            else SandboxVerificationFinalReportArchivePolicy()
        )
        return _result(
    ...
    if not isinstance(source, SandboxVerificationFinalReportResult):
        return _result(
            policy, now, "FAILED", entries,
            ["Source는 V16.0 Sandbox Verification Final Report Result여야 합니다."],
    ...
    if not (
        ...
        source_errors.append("정상 V16.0 Verification Final Report가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V16.0 Source 실행 안전장치 오류입니다.")
    ...
    if operator != report.operator:
        return _result(
            ...
            ["Operator가 V16.0 Final Report와 다릅니다."],
            policy=True, input=True, source=True, report_hash=True,
    ...
    combined = entries + (
        SandboxVerificationFinalReportArchiveEntry(
    ...
    if trimmed_count:
        ...
        rebuilt: list[SandboxVerificationFinalReportArchiveEntry] = []
        previous_hash = GENESIS_HASH
        for sequence, item in enumerate(kept, start=1):
            ...
            item = SandboxVerificationFinalReportArchiveEntry(
    ...
    result = _result(
        ...
        combined,
        [
            f"V16.0 Final Report {report.final_report_id}를 보관했습니다.",
...
def save_archive_result(
    result: SandboxVerificationFinalReportArchiveResult,
    output_directory: Path = OUTPUT_DIRECTORY,
) -> tuple[Path, Path]:
    output_directory.mkdir(parents=True, exist_ok=True)
    stamp = result.created_at.replace(":", "").replace("-", "").replace("+", "_")
    report_path = output_directory / f"verification_final_report_archive_{stamp}.json"
    latest_path = output_directory / "latest_verification_final_report_archive.json"
