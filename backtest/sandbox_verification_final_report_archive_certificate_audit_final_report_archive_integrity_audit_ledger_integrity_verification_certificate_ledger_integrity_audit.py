...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "sandbox_verification_certificate_ledger_integrity_audit"
)
REQUIRED_AUDIT_TEXT = (
    "AUDIT IN MEMORY SANDBOX VERIFICATION FINAL REPORT ARCHIVE "
    "CERTIFICATE AUDIT FINAL REPORT ARCHIVE INTEGRITY AUDIT LEDGER "
    "INTEGRITY VERIFICATION CERTIFICATE LEDGER INTEGRITY"
)
GENESIS_HASH = "0" * 64


@dataclass(frozen=True)
class SandboxVerificationFinalReportArchiveCertificateAuditFinalReportArchiveIntegrityAuditLedgerIntegrityVerificationCertificateLedgerAuditPolicy:
    required_source_version: str = "V17.5"
...
@dataclass(frozen=True)
class SandboxVerificationFinalReportArchiveCertificateAuditFinalReportArchiveIntegrityAuditLedgerIntegrityVerificationCertificateLedgerFinding:
    sequence: int
    ledger_entry_id: str
...
@dataclass(frozen=True)
class SandboxVerificationFinalReportArchiveCertificateAuditFinalReportArchiveIntegrityAuditLedgerIntegrityVerificationCertificateLedgerAuditCertificate:
    ...
    findings: tuple[
        SandboxVerificationFinalReportArchiveCertificateAuditFinalReportArchiveIntegrityAuditLedgerIntegrityVerificationCertificateLedgerFinding, ...
    ]
...
@dataclass
class SandboxVerificationFinalReportArchiveCertificateAuditFinalReportArchiveIntegrityAuditLedgerIntegrityVerificationCertificateLedgerAuditResult:
    version: str
    created_at: str
    audit_result_id: str
    result_status: str
    ...
    live_execution_authorized: bool
    audit_policy: (
        SandboxVerificationFinalReportArchiveCertificateAuditFinalReportArchiveIntegrityAuditLedgerIntegrityVerificationCertificateLedgerAuditPolicy
    )
    certificate: (
        SandboxVerificationFinalReportArchiveCertificateAuditFinalReportArchiveIntegrityAuditLedgerIntegrityVerificationCertificateLedgerAuditCertificate
        | None
    )
...
def verify_certificate_ledger_chain(
    ...
    for expected_sequence, entry in enumerate(entries, start=1):
        ...
        if any(not hasattr(entry, name) for name in required):
            errors.append(
                f"Entry {expected_sequence}의 V17.5 구조가 올바르지 않습니다."
            )
            continue
...
def validate_policy(
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V17.6 기준과 다릅니다.")
    for name in (
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V17.6에서 True여야 합니다.")
    return errors
...
def _empty_result(
    ...
    reasons: list[str],
    **checks: bool,
) -> SandboxVerificationFinalReportArchiveCertificateAuditFinalReportArchiveIntegrityAuditLedgerIntegrityVerificationCertificateLedgerAuditResult:
    return SandboxVerificationFinalReportArchiveCertificateAuditFinalReportArchiveIntegrityAuditLedgerIntegrityVerificationCertificateLedgerAuditResult(
        version="V17.6",
        ...
        reasons=reasons,
        warnings=[
            "V17.6은 V17.5 Certificate Ledger를 읽기 전용으로 감사합니다.",
            "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
        ],
        next_actions=["V17.5 Certificate Ledger Source와 입력을 확인합니다."],
    )


def audit_sandbox_verification_final_report_archive_certificate_audit_final_report_archive_integrity_audit_ledger_integrity_verification_certificate_ledger(
    source: Any,
    operator: str,
    ...
    policy = (
        policy
        or SandboxVerificationFinalReportArchiveCertificateAuditFinalReportArchiveIntegrityAuditLedgerIntegrityVerificationCertificateLedgerAuditPolicy()
    )
    ...
    if policy_errors or input_errors:
        safe_policy = (
            ...
            else SandboxVerificationFinalReportArchiveCertificateAuditFinalReportArchiveIntegrityAuditLedgerIntegrityVerificationCertificateLedgerAuditPolicy()
        )
    ...
    if (
        ...
        return _empty_result(
            policy, now, "FAILED",
            ["Source는 V17.5 Verification Certificate Ledger여야 합니다."],
    ...
    try:
        ...
        return _empty_result(
            ...
            now,
            "FAILED",
            ["V17.5 Source Entries 형식이 올바르지 않습니다."],
            policy=True,
            input=True,
        )
    source_errors = []
    if not (
        ...
        source_errors.append("정상 V17.5 Certificate Ledger Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V17.5 Source 실행 안전장치 오류입니다.")
    if source_errors:
        return _empty_result(
    ...
    if operator != latest.operator:
        return _empty_result(
            policy, now, "BLOCKED",
            ["Operator가 V17.5 Latest Ledger Entry와 다릅니다."],
    ...
    return SandboxVerificationFinalReportArchiveCertificateAuditFinalReportArchiveIntegrityAuditLedgerIntegrityVerificationCertificateLedgerAuditResult(
        ...
        reasons=[
            f"V17.5 Certificate Ledger Entry {len(entries)}개를 감사했습니다.",
            "Hash Chain, Source Linkage 및 Snapshot Hash가 유효합니다.",
        ],
        warnings=[
            "V17.6은 V17.5 Certificate Ledger를 변경하지 않습니다.",
            "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
        ],
        next_actions=[
...
def save_audit_result(
    result: SandboxVerificationFinalReportArchiveCertificateAuditFinalReportArchiveIntegrityAuditLedgerIntegrityVerificationCertificateLedgerAuditResult,
    output_directory: Path = OUTPUT_DIRECTORY,
) -> tuple[Path, Path]:
    output_directory.mkdir(parents=True, exist_ok=True)
    stamp = result.created_at.replace(":", "").replace("-", "").replace("+", "_")
    report = output_directory / (
        f"verification_certificate_ledger_integrity_audit_{stamp}.json"
    )
    latest = output_directory / (
        "latest_verification_certificate_ledger_integrity_audit.json"
    )
