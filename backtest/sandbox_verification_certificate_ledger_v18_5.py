...
from backtest.sandbox_archive_audit_ledger_integrity_v18_4 import (
    SandboxArchiveAuditLedgerIntegrityV184Result as SourceVerificationResult,
...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "sandbox_verification_certificate_ledger_v18_5"
)
REQUIRED_LEDGER_TEXT = (
    "RECORD IN MEMORY SANDBOX VERIFICATION CERTIFICATE V18.4"
)
GENESIS_HASH = "0" * 64


@dataclass(frozen=True)
class SandboxVerificationCertificateLedgerV185Policy:
    required_source_version: str = "V18.4"
...
@dataclass(frozen=True)
class SandboxVerificationCertificateLedgerV185Entry:
    ledger_entry_id: str
    sequence: int
    recorded_at: str
...
@dataclass
class SandboxVerificationCertificateLedgerV185Result:
    version: str
    created_at: str
    ...
    live_execution_authorized: bool
    ledger_policy: (
        SandboxVerificationCertificateLedgerV185Policy
    )
    entries: tuple[
        SandboxVerificationCertificateLedgerV185Entry, ...
    ]
...
def validate_policy(
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V18.5 기준과 다릅니다.")
    ...
    for name in (
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V18.5에서 True여야 합니다.")
    return errors
...
def _result(
    ...
    reasons: list[str],
    **checks: bool,
) -> SandboxVerificationCertificateLedgerV185Result:
    latest = entries[-1] if entries else None
    return SandboxVerificationCertificateLedgerV185Result(
        version="V18.5",
        ...
        reasons=reasons,
        warnings=[
            "V18.5는 V18.4 Verification Certificate를 기록만 합니다.",
...
def record_sandbox_verification_certificate_v18_5(
    source: Any,
    operator: str,
    ...
    policy = (
        policy
        or SandboxVerificationCertificateLedgerV185Policy()
    )
    ...
    if policy_errors or input_errors:
        safe_policy = (
            ...
            else SandboxVerificationCertificateLedgerV185Policy()
        )
        return _result(
            safe_policy,
            now,
    ...
    if not isinstance(
        ...
        return _result(
            ...
            "FAILED",
            entries,
            ["Source는 V18.4 Verification Archive Ledger Integrity Result여야 합니다."],
            policy=True,
            input=True,
            existing=True,
            chain=True,
    ...
    if not (
        ...
        source_errors.append("정상 V18.4 Verification Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V18.4 Source 실행 안전장치 오류입니다.")
...
def save_certificate_ledger_result(
    result: SandboxVerificationCertificateLedgerV185Result,
