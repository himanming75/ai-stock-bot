...
from backtest.sandbox_archive_audit_ledger import (
    GENESIS_HASH,
    SandboxArchiveAuditLedgerEntry,
...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "sandbox_audit_ledger_integrity_verification"
)
REQUIRED_VERIFICATION_TEXT = (
    "VERIFY IN MEMORY SANDBOX AUDIT LEDGER INTEGRITY"
)


@dataclass(frozen=True)
class SandboxAuditLedgerIntegrityPolicy:
    required_source_version: str = "V15.3"
...
@dataclass(frozen=True)
class SandboxAuditLedgerFinding:
    sequence: int
    ledger_entry_id: str
...
@dataclass(frozen=True)
class SandboxAuditLedgerVerificationCertificate:
    ...
    findings: tuple[SandboxAuditLedgerFinding, ...]
...
@dataclass
class SandboxAuditLedgerIntegrityResult:
    version: str
    created_at: str
    ...
    live_execution_authorized: bool
    verification_policy: SandboxAuditLedgerIntegrityPolicy
    certificate: SandboxAuditLedgerVerificationCertificate | None
...
def validate_policy(policy: SandboxAuditLedgerIntegrityPolicy) -> list[str]:
    if not isinstance(policy, SandboxAuditLedgerIntegrityPolicy):
        return ["Sandbox Audit Ledger Integrity Policy 형식이 올바르지 않습니다."]
    errors: list[str] = []
    expected = {
        "required_source_version": "V15.3",
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V15.4 기준과 다릅니다.")
    for name in (
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V15.4에서 True여야 합니다.")
    return errors


def _safe_entry(entry: SandboxArchiveAuditLedgerEntry) -> bool:
    return not any(
        (
...
def _safe_source(source: SandboxArchiveAuditLedgerResult) -> bool:
    return not any(
        (
...
def build_findings(
    entries: tuple[SandboxArchiveAuditLedgerEntry, ...],
) -> tuple[SandboxAuditLedgerFinding, ...]:
    findings: list[SandboxAuditLedgerFinding] = []
...
def verify_verification_certificate(
    certificate: SandboxAuditLedgerVerificationCertificate,
) -> tuple[bool, list[str]]:
    if not isinstance(certificate, SandboxAuditLedgerVerificationCertificate):
        return False, ["Verification Certificate 형식이 올바르지 않습니다."]
    errors: list[str] = []
...
def _empty_result(
    ...
    **checks: bool,
    ...
    return SandboxAuditLedgerIntegrityResult(
        version="V15.4",
        created_at=now.isoformat(),
        verification_result_id=str(uuid.uuid4()),
        result_status=status,
        result_status_label=(
            "Integrity Verification 차단"
            if status == "BLOCKED"
            else "Integrity Verification 실패"
        ),
        ...
        reasons=reasons,
        warnings=[
            "V15.4는 V15.3 Audit Ledger를 읽기 전용으로 검증합니다.",
            "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
        ],
        next_actions=["V15.3 Audit Ledger Source와 입력을 확인합니다."],
    )


def verify_sandbox_audit_ledger_integrity(
    source: Any,
    operator: str,
    confirmation_text: str,
    policy: SandboxAuditLedgerIntegrityPolicy | None = None,
    now: datetime | None = None,
) -> SandboxAuditLedgerIntegrityResult:
    policy = policy or SandboxAuditLedgerIntegrityPolicy()
    ...
    if not isinstance(source, SandboxArchiveAuditLedgerResult):
        return _empty_result(
            policy, now, "FAILED",
            ["Source는 V15.3 Sandbox Archive Audit Ledger Result여야 합니다."],
    ...
    if not (
        ...
        source_errors.append("정상 V15.3 Audit Ledger Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V15.3 Source 실행 안전장치가 올바르지 않습니다.")
    if source_errors:
        return _empty_result(
    ...
    if operator != latest.operator:
        return _empty_result(
            policy, now, "BLOCKED",
            ["Operator가 V15.3 Latest Ledger Entry와 다릅니다."],
    ...
    if not unchanged:
        return _empty_result(
            policy, now, "FAILED", ["읽기 전용 검증 중 Ledger가 변경되었습니다."],
            policy=True, input=True, source=True, chain=True, linkage=True,
    ...
    return SandboxAuditLedgerIntegrityResult(
        ...
        result_status_label="Sandbox Audit Ledger Integrity Verification 완료",
        verification_status="PASSED",
        source_ledger_result_id=source.ledger_result_id,
        ...
        reasons=[
            f"V15.3 Audit Ledger Entry {len(entries)}개를 검증했습니다.",
            "Ledger Hash Chain, Source Linkage 및 Snapshot Hash가 유효합니다.",
        ],
        warnings=[
            "V15.4는 V15.3 Audit Ledger를 변경하지 않습니다.",
            "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
        ],
        next_actions=[
...
def save_verification_result(
    result: SandboxAuditLedgerIntegrityResult,
    output_directory: Path = OUTPUT_DIRECTORY,
) -> tuple[Path, Path]:
    output_directory.mkdir(parents=True, exist_ok=True)
    stamp = result.created_at.replace(":", "").replace("-", "").replace("+", "_")
    report = output_directory / f"sandbox_audit_ledger_verification_{stamp}.json"
    latest = output_directory / "latest_sandbox_audit_ledger_verification.json"
