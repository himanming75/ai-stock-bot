...
from backtest.sandbox_verification_final_report_archive_audit_ledger import (
    GENESIS_HASH,
...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "sandbox_verification_final_report_archive_audit_ledger_integrity"
)
REQUIRED_VERIFICATION_TEXT = (
    "VERIFY IN MEMORY SANDBOX VERIFICATION FINAL REPORT "
    "ARCHIVE AUDIT LEDGER INTEGRITY"
)


@dataclass(frozen=True)
class SandboxVerificationFinalReportArchiveAuditLedgerIntegrityPolicy:
    required_source_version: str = "V16.3"
...
@dataclass(frozen=True)
class SandboxVerificationFinalReportArchiveAuditLedgerFinding:
    sequence: int
    ledger_entry_id: str
...
@dataclass(frozen=True)
class SandboxVerificationFinalReportArchiveAuditLedgerCertificate:
    ...
    findings: tuple[
        SandboxVerificationFinalReportArchiveAuditLedgerFinding, ...
    ]
...
@dataclass
class SandboxVerificationFinalReportArchiveAuditLedgerIntegrityResult:
    version: str
    created_at: str
    ...
    live_execution_authorized: bool
    verification_policy: (
        SandboxVerificationFinalReportArchiveAuditLedgerIntegrityPolicy
    )
    certificate: (
        SandboxVerificationFinalReportArchiveAuditLedgerCertificate | None
    )
...
def validate_policy(
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V16.4 기준과 다릅니다.")
    for name in (
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V16.4에서 True여야 합니다.")
    return errors


def _safe_entry(
    entry: SandboxVerificationFinalReportArchiveAuditLedgerEntry,
) -> bool:
    return not any(
        (
...
def _safe_source(
    source: SandboxVerificationFinalReportArchiveAuditLedgerResult,
) -> bool:
    return not any(
        (
...
def verify_verification_certificate(
    certificate: (
        SandboxVerificationFinalReportArchiveAuditLedgerCertificate
    ),
) -> tuple[bool, list[str]]:
    if not isinstance(
        certificate,
...
def _empty_result(
    ...
    **checks: bool,
    ...
    return SandboxVerificationFinalReportArchiveAuditLedgerIntegrityResult(
        version="V16.4",
        ...
        reasons=reasons,
        warnings=[
            "V16.4는 V16.3 Audit Ledger를 읽기 전용으로 검증합니다.",
            "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
        ],
        next_actions=["V16.3 Audit Ledger Source와 입력을 확인합니다."],
    )


def verify_sandbox_verification_final_report_archive_audit_ledger_integrity(
    source: Any,
    operator: str,
    ...
    policy = (
        policy
        or SandboxVerificationFinalReportArchiveAuditLedgerIntegrityPolicy()
    )
    ...
    if policy_errors or input_errors:
        safe_policy = (
            ...
            else SandboxVerificationFinalReportArchiveAuditLedgerIntegrityPolicy()
        )
    ...
    if not isinstance(
        ...
        return _empty_result(
            ...
            now,
            "FAILED",
            ["Source는 V16.3 Verification Archive Audit Ledger여야 합니다."],
            policy=True,
            input=True,
        )
    ...
    if not (
        ...
        source_errors.append("정상 V16.3 Audit Ledger Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V16.3 Source 실행 안전장치 오류입니다.")
    if source_errors:
        return _empty_result(
            policy,
            now,
    ...
    if operator != latest.operator:
        return _empty_result(
            ...
            now,
            "BLOCKED",
            ["Operator가 V16.3 Latest Ledger Entry와 다릅니다."],
            policy=True,
            input=True,
            source=True,
            chain=True,
    ...
    return (
        SandboxVerificationFinalReportArchiveAuditLedgerIntegrityResult(
            ...
            reasons=[
                f"V16.3 Audit Ledger Entry {len(entries)}개를 검증했습니다.",
                "Ledger Hash Chain, Source Linkage 및 Snapshot Hash가 유효합니다.",
            ],
            warnings=[
                "V16.4는 V16.3 Audit Ledger를 변경하지 않습니다.",
                "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
            ],
            next_actions=[
...
def save_verification_result(
    ...
    report_path = output_directory / (
        f"verification_final_report_archive_audit_ledger_integrity_{stamp}.json"
    )
    latest_path = output_directory / (
        "latest_verification_final_report_archive_audit_ledger_integrity.json"
    )
