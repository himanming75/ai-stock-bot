...
from backtest.offline_paper_verify_cert_ledger_v22_1 import (
    GENESIS_HASH,
    OfflinePaperVerifyCertLedgerV221Entry,
    OfflinePaperVerifyCertLedgerV221Result,
    verify_verification_certificate_ledger_chain,
)
...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "offline_paper_verify_cert_audit_v22_2"
)
REQUIRED_VERIFICATION_TEXT = (
    "AUDIT OFFLINE PAPER VERIFICATION CERTIFICATE LEDGER V22.2"
)


@dataclass(frozen=True)
class OfflinePaperVerifyCertAuditV222Policy:
    required_source_version: str = "V22.1"
...
@dataclass(frozen=True)
class OfflinePaperVerifyCertAuditFindingV222:
    sequence: int
    ledger_entry_id: str
...
@dataclass(frozen=True)
class OfflinePaperVerifyCertAuditCertificateV222:
    verification_certificate_id: str
    verified_at: str
    ...
    findings: tuple[OfflinePaperVerifyCertAuditFindingV222, ...]
...
@dataclass
class OfflinePaperVerifyCertAuditV222Result:
    version: str
    created_at: str
    ...
    live_order_created: bool
    live_execution_authorized: bool
    verification_policy: OfflinePaperVerifyCertAuditV222Policy
    certificate: OfflinePaperVerifyCertAuditCertificateV222 | None
...
def validate_policy(
    policy: OfflinePaperVerifyCertAuditV222Policy,
) -> list[str]:
    if not isinstance(policy, OfflinePaperVerifyCertAuditV222Policy):
        return ["V22.2 Verification Policy 형식 오류입니다."]
    errors: list[str] = []
    expected = {
        "required_source_version": "V22.1",
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V22.2 기준과 다릅니다.")
    for name in (
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V22.2에서 True여야 합니다.")
    return errors
...
def _safe_source(source: OfflinePaperVerifyCertLedgerV221Result) -> bool:
    return not any(
        (
...
def _findings(
    entries: tuple[OfflinePaperVerifyCertLedgerV221Entry, ...],
) -> tuple[OfflinePaperVerifyCertAuditFindingV222, ...]:
    output: list[OfflinePaperVerifyCertAuditFindingV222] = []
    ...
    for expected, entry in enumerate(entries, start=1):
        ...
        output.append(
            OfflinePaperVerifyCertAuditFindingV222(
...
def verify_verification_certificate(
    ...
    if not isinstance(certificate, OfflinePaperVerifyCertAuditCertificateV222):
        return False, ["V22.2 Verification Certificate 형식 오류입니다."]
    errors: list[str] = []
    if certificate.certificate_hash != sha256_payload(
        certificate.payload_without_hash()
    ):
        errors.append("V22.2 Verification Certificate Hash 오류입니다.")
    if certificate.verification_status != "PASSED":
        errors.append("V22.2 Verification Certificate Status 오류입니다.")
    if not all(
        (
        ...
        errors.append("V22.2 Verification Certificate 검증 항목 오류입니다.")
    if any(
        (
        ...
        errors.append("V22.2 Verification Certificate 안전장치 오류입니다.")
...
def _empty_result(
    ...
    **checks: bool,
    ...
    return OfflinePaperVerifyCertAuditV222Result(
        version="V22.2",
        created_at=now.isoformat(),
        verification_result_id=str(uuid.uuid4()),
        result_status=status,
        result_status_label=(
            "Offline Paper Verification Ledger 감사 차단"
            if status == "BLOCKED"
            else "Offline Paper Verification Ledger 감사 실패"
        ),
        ...
        reasons=reasons,
        warnings=[
            "V22.2은 V22.1 Ledger를 읽기 전용으로 검증합니다.",
            "검증은 주문 체결 또는 Broker 전송 승인을 의미하지 않습니다.",
            "잔액·보유수량 변경, API, Network 및 Live Execution은 차단됩니다.",
        ],
        next_actions=[
            "차단 또는 실패 원인을 수정한 뒤 V22.2 검증을 다시 실행합니다."
        ],
    )


def _linkage_errors(
    source: OfflinePaperVerifyCertLedgerV221Result,
    latest: OfflinePaperVerifyCertLedgerV221Entry,
) -> list[str]:
    checks = (
        ...
        (
            ...
            "V22.0 Audit Verification Ledger Snapshot Hash",
        ),
        (
        ...
        (
            ...
            "V21.9 Source Verification Ledger Snapshot Hash",
        ),
        (
    ...
    return [
        f"V22.1 Source {label} 연결이 다릅니다."
        for actual, expected, label in checks
        if actual != expected
    ]


def verify_offline_paper_verification_certificate_ledger_v22_2(
    source: Any,
    operator: Any,
    confirmation_text: Any,
    policy: OfflinePaperVerifyCertAuditV222Policy | None = None,
    now: datetime | None = None,
) -> OfflinePaperVerifyCertAuditV222Result:
    policy = policy or OfflinePaperVerifyCertAuditV222Policy()
    ...
    if confirmation_text != getattr(policy, "required_confirmation_text", None):
        input_errors.append("V22.2 확인 문구가 일치하지 않습니다.")
    safe_policy = (
        policy
        if isinstance(policy, OfflinePaperVerifyCertAuditV222Policy)
        else OfflinePaperVerifyCertAuditV222Policy()
    )
    ...
    if not isinstance(source, OfflinePaperVerifyCertLedgerV221Result):
        return _empty_result(
            policy,
            now,
            "FAILED",
            ["Source는 V22.1 Verification Certificate Ledger Result여야 합니다."],
            policy_ok=True,
            input=True,
        )
    ...
    if not (
        ...
        source_errors.append("정상 V22.1 Verification Certificate Ledger Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V22.1 Source 실행 안전장치 오류입니다.")
    if source_errors:
        return _empty_result(
            policy,
            now,
    ...
    if latest.operator != operator.strip():
        linkage_errors.append("Operator가 V22.1 Latest Entry와 다릅니다.")
    try:
        ...
        return _empty_result(
            ...
            now,
            "BLOCKED",
            ["역순 또는 잘못된 V22.2 Verification 시간이 차단되었습니다."],
            policy_ok=True,
            input=True,
            source=True,
        )
    ...
    if not chronology_valid:
        errors.append("V22.1 Ledger 시간 순서 오류가 발견되었습니다.")
    if not status_valid:
        errors.append("V22.1 Ledger Verification Status 오류가 발견되었습니다.")
    if not safety_valid:
        errors.append("V22.1 Ledger Entry 안전장치 오류가 발견되었습니다.")
    if not unchanged:
        errors.append("V22.2 검증 중 V22.1 Source가 변경되었습니다.")
    if errors:
        return _empty_result(
            policy,
            now,
    ...
    certificate = OfflinePaperVerifyCertAuditCertificateV222(
        **certificate_payload,
    ...
    return OfflinePaperVerifyCertAuditV222Result(
        ...
        result_status_label="Offline Paper Verification Ledger 무결성 감사 완료",
        verification_status="PASSED",
        source_ledger_result_id=source.ledger_result_id,
        ...
        reasons=[
            f"V22.1 Ledger Entry {len(entries)}개의 무결성을 검증했습니다.",
            "V22.2 Verification Certificate를 생성했습니다.",
        ],
        warnings=[
            "V22.2은 V22.1 Ledger를 읽기 전용으로 검증했습니다.",
            "검증은 주문 체결 또는 Broker 전송 승인을 의미하지 않습니다.",
            "잔액·보유수량 변경, API, Network 및 Live Execution은 차단됩니다.",
        ],
        next_actions=[
            "V22.3 단계에서 V22.2 Verification Certificate를 "
...
def save_audit_integrity_verification_result(
    result: OfflinePaperVerifyCertAuditV222Result,
...
def load_audit_integrity_verification_result(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("V22.2 Verification Ledger Audit JSON은 object여야 합니다.")
    return payload
