...
from backtest.offline_paper_verify_cert_audit_v22_2 import (
    OfflinePaperVerifyCertAuditV222Result,
...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "offline_paper_verify_cert_audit_ledger_v22_3"
)
REQUIRED_AUDIT_LEDGER_TEXT = (
    "RECORD OFFLINE PAPER VERIFICATION CERTIFICATE AUDIT V22.3"
)
GENESIS_HASH = "0" * 64


@dataclass(frozen=True)
class OfflinePaperVerifyCertAuditLedgerV223Policy:
    required_source_version: str = "V22.2"
    required_source_status: str = "VERIFIED_IN_MEMORY"
    required_verification_status: str = "PASSED"
    required_confirmation_text: str = REQUIRED_AUDIT_LEDGER_TEXT
...
@dataclass(frozen=True)
class OfflinePaperVerifyCertAuditLedgerV223Entry:
    ledger_entry_id: str
    sequence: int
    recorded_at: str
    ...
    source_audit_verify_ledger_result_id: str
    audit_verify_ledger_snapshot_hash: str
    verified_entry_count: int
    latest_source_ledger_entry_id: str
    latest_source_ledger_entry_hash: str
    latest_source_verification_certificate_hash: str
    latest_source_audit_verify_ledger_snapshot_hash: str
...
@dataclass
class OfflinePaperVerifyCertAuditLedgerV223Result:
    version: str
    created_at: str
    ...
    latest_verification_certificate_id: str | None
    latest_verification_certificate_hash: str | None
    latest_source_audit_verify_ledger_result_id: str | None
    latest_audit_verify_ledger_snapshot_hash: str | None
    latest_source_verification_certificate_hash: str | None
    latest_source_audit_verify_ledger_snapshot_hash: str | None
    ...
    live_order_created: bool
    live_execution_authorized: bool
    ledger_policy: OfflinePaperVerifyCertAuditLedgerV223Policy
    entries: tuple[OfflinePaperVerifyCertAuditLedgerV223Entry, ...]
...
def validate_policy(
    policy: OfflinePaperVerifyCertAuditLedgerV223Policy,
) -> list[str]:
    if not isinstance(policy, OfflinePaperVerifyCertAuditLedgerV223Policy):
        return ["V22.3 Verification Certificate Ledger Policy 형식 오류입니다."]
    errors: list[str] = []
    expected = {
        "required_source_version": "V22.2",
        "required_source_status": "VERIFIED_IN_MEMORY",
        "required_verification_status": "PASSED",
        "required_confirmation_text": REQUIRED_AUDIT_LEDGER_TEXT,
    }
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V22.3 기준과 다릅니다.")
    if (
        ...
        errors.append("V22.3 Ledger 보관 한도는 양의 정수여야 합니다.")
    for name in (
        "require_operator",
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V22.3에서 True여야 합니다.")
    return errors


def normalize_entries(
    existing: Any,
) -> tuple[OfflinePaperVerifyCertAuditLedgerV223Entry, ...]:
    if existing is None:
        return ()
    if not isinstance(existing, (tuple, list)):
        raise TypeError("Existing V22.3 Ledger는 tuple/list여야 합니다.")
    if not all(
        isinstance(entry, OfflinePaperVerifyCertAuditLedgerV223Entry)
        for entry in existing
    ):
        raise TypeError("Existing V22.3 Ledger Entry 형식 오류입니다.")
    return tuple(existing)


def _safe_entry(entry: OfflinePaperVerifyCertAuditLedgerV223Entry) -> bool:
    return not any(
        (
...
def _safe_source(source: OfflinePaperVerifyCertAuditV222Result) -> bool:
    return not any(
        (
...
def verify_verification_certificate_audit_ledger_chain(
    entries: tuple[OfflinePaperVerifyCertAuditLedgerV223Entry, ...],
) -> tuple[bool, list[str]]:
    errors: list[str] = []
    ...
    for expected_sequence, entry in enumerate(entries, start=1):
        if entry.sequence != expected_sequence:
            errors.append(f"V22.3 Ledger Sequence {expected_sequence} 오류입니다.")
        ...
        if entry.verification_certificate_id in certificate_ids:
            errors.append(
                "중복 V22.2 Verification Certificate ID: "
                f"{entry.verification_certificate_id}"
            )
        ...
        try:
            ...
            if previous_time and recorded_at < previous_time:
                errors.append("V22.3 Ledger 기록 시간이 역순입니다.")
            previous_time = recorded_at
        except (TypeError, ValueError):
            errors.append("V22.3 Ledger 기록 시간이 올바르지 않습니다.")
        if entry.verification_status != "PASSED":
            errors.append("V22.3 Ledger Verification Status 오류입니다.")
        if not _safe_entry(entry):
            errors.append("V22.3 Ledger Entry 실행 안전장치 오류입니다.")
...
def _result(
    ...
    reasons: list[str],
    **checks: bool,
) -> OfflinePaperVerifyCertAuditLedgerV223Result:
    latest = entries[-1] if entries else None
    recorded = status == "RECORDED_IN_MEMORY"
    return OfflinePaperVerifyCertAuditLedgerV223Result(
        version="V22.3",
        created_at=now.isoformat(),
        ledger_result_id=str(uuid.uuid4()),
        result_status=status,
        result_status_label=(
            "Offline Paper Verification Certificate Ledger 기록 완료"
            if recorded
            else "Offline Paper Verification Certificate Ledger 차단"
            if status == "BLOCKED"
            else "Offline Paper Verification Certificate Ledger 실패"
        ),
        ...
        latest_verification_result_id=(
            latest.verification_result_id if latest else None
        ),
        latest_verification_certificate_id=(
            latest.verification_certificate_id if latest else None
        ),
        latest_verification_certificate_hash=(
            latest.verification_certificate_hash if latest else None
        ),
        latest_source_audit_verify_ledger_result_id=(
            latest.source_audit_verify_ledger_result_id if latest else None
        ),
        latest_audit_verify_ledger_snapshot_hash=(
            latest.audit_verify_ledger_snapshot_hash if latest else None
        ),
        latest_source_verification_certificate_hash=(
            latest.latest_source_verification_certificate_hash if latest else None
        ),
        latest_source_audit_verify_ledger_snapshot_hash=(
            latest.latest_source_audit_verify_ledger_snapshot_hash
            if latest
            else None
        ),
        ...
        reasons=reasons,
        warnings=[
            "V22.3은 V22.2 Verification Certificate를 In-Memory Ledger에 "
            "기록합니다.",
            "기록은 주문 체결 또는 Broker 전송 승인을 의미하지 않습니다.",
            "잔액·보유수량 변경, API, Network 및 Live Execution은 차단됩니다.",
        ],
        next_actions=[
            "V22.2 Verification Certificate Ledger Integrity Audit "
            "단계에서 V22.3 Ledger를 읽기 전용으로 검증합니다."
        ],
    )


def _source_linkage(
    source: OfflinePaperVerifyCertAuditV222Result,
) -> list[str]:
    certificate = source.certificate
    if certificate is None:
        return ["V22.2 Verification Certificate가 없습니다."]
    errors: list[str] = []
    for actual, expected, label in (
        (
            source.verification_certificate_id,
            certificate.verification_certificate_id,
            "V22.2 Verification Certificate ID",
        ),
        (
            source.verification_certificate_hash,
            certificate.certificate_hash,
            "V22.2 Verification Certificate Hash",
        ),
        (
            source.source_ledger_result_id,
            certificate.source_ledger_result_id,
            "V21.7 Source Ledger Result ID",
        ),
        (
            source.verified_entry_count,
        ...
        (
            ...
            "V21.7 Latest Ledger Entry ID",
        ),
        (
            source.latest_ledger_entry_hash,
            certificate.latest_ledger_entry_hash,
            "V21.7 Latest Ledger Entry Hash",
        ),
        (
            source.verification_certificate_ledger_snapshot_hash,
            certificate.verification_certificate_ledger_snapshot_hash,
            "V21.7 Verification Certificate Ledger Snapshot Hash",
        ),
        (
...
def record_offline_paper_verification_certificate_audit_v22_3(
    source: Any,
    operator: Any,
    confirmation_text: Any,
    existing: Any = None,
    policy: OfflinePaperVerifyCertAuditLedgerV223Policy | None = None,
    now: datetime | None = None,
) -> OfflinePaperVerifyCertAuditLedgerV223Result:
    policy = policy or OfflinePaperVerifyCertAuditLedgerV223Policy()
    ...
    if confirmation_text != getattr(policy, "required_confirmation_text", None):
        input_errors.append("V22.3 확인 문구가 일치하지 않습니다.")
    if policy_errors or input_errors:
        safe_policy = (
            policy
            if isinstance(policy, OfflinePaperVerifyCertAuditLedgerV223Policy)
            else OfflinePaperVerifyCertAuditLedgerV223Policy()
        )
        return _result(
            safe_policy,
            now,
    ...
    chain_valid, chain_errors = verify_verification_certificate_audit_ledger_chain(entries)
    if not chain_valid:
        return _result(
            policy,
            now,
    ...
    if not isinstance(source, OfflinePaperVerifyCertAuditV222Result):
        return _result(
            policy,
            now,
            "FAILED",
            entries,
            ["Source는 V22.2 Verification Ledger Audit Result여야 합니다."],
            policy=True,
            input=True,
            existing=True,
            chain=True,
    ...
    if not (
        ...
        and source.verification_status == policy.required_verification_status
        and source.all_checks_passed
        and source.verification_completed
        and not source.ledger_modified
        and certificate is not None
    ):
        source_errors.append("정상 V22.2 Integrity Verification Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V22.2 Source 실행 안전장치 오류입니다.")
    ...
    if certificate.verification_status != policy.required_verification_status:
        linkage_errors.append(
            "V22.2 Verification Status가 V22.3 기준과 다릅니다."
        )
    if linkage_errors:
        return _result(
            policy,
    ...
    if any(
        entry.verification_certificate_id == certificate.verification_certificate_id
        ...
        return _result(
            ...
            "BLOCKED",
            entries,
            ["중복 V22.2 Verification Certificate ID가 차단되었습니다."],
            policy=True,
            input=True,
            source=True,
            certificate=True,
    ...
    try:
        verified_at = datetime.fromisoformat(certificate.verified_at)
        if verified_at.tzinfo is None:
            raise ValueError
        if now < verified_at or (
        ...
        return _result(
            ...
            "BLOCKED",
            entries,
            ["역순 또는 잘못된 V22.3 Ledger 시간이 차단되었습니다."],
            policy=True,
            input=True,
            source=True,
            certificate=True,
    ...
    if source_before != canonical_json(source.to_dict()):
        return _result(
            ...
            "FAILED",
            entries,
            ["V22.3 기록 중 V22.2 Source가 변경되었습니다."],
            policy=True,
            input=True,
            source=True,
            certificate=True,
    ...
    payload = {
        ...
        "verification_result_id": source.verification_result_id,
        ...
        "source_audit_verify_ledger_result_id": (
            certificate.source_ledger_result_id
        ),
        "audit_verify_ledger_snapshot_hash": (
            certificate.verification_certificate_ledger_snapshot_hash
        ),
        "verified_entry_count": certificate.total_ledger_entry_count,
        "latest_source_ledger_entry_id": (
            certificate.latest_ledger_entry_id
        ),
        "latest_source_ledger_entry_hash": (
            certificate.latest_ledger_entry_hash
        ),
        "latest_source_verification_certificate_hash": (
            certificate.latest_verification_certificate_hash
        ...
        "latest_v21_1_order_hash": certificate.latest_v21_1_order_hash,
        "latest_v21_0_account_hash": certificate.latest_v21_0_account_hash,
    ...
    new_entry = OfflinePaperVerifyCertAuditLedgerV223Entry(
        **payload,
    ...
    if trimmed_count:
        ...
        rebuilt: list[OfflinePaperVerifyCertAuditLedgerV223Entry] = []
        previous_hash = GENESIS_HASH
    ...
    valid_chain, final_errors = verify_verification_certificate_audit_ledger_chain(combined)
    result = _result(
        policy,
        now,
        ...
        combined,
        [
            "V22.2 Verification Certificate "
            f"{certificate.verification_certificate_id}를 기록했습니다.",
            f"현재 V22.3 Ledger Entry는 {len(combined)}개입니다.",
        ]
        + final_errors,
        policy=True,
        input=True,
...
def save_verification_certificate_audit_ledger_result(
    result: OfflinePaperVerifyCertAuditLedgerV223Result,
    output_directory: Path = OUTPUT_DIRECTORY,
) -> tuple[Path, Path]:
    output_directory.mkdir(parents=True, exist_ok=True)
    stamp = result.created_at.replace(":", "").replace("-", "").replace("+", "_")
    report_path = output_directory / f"verification_certificate_ledger_{stamp}.json"
    latest_path = output_directory / "latest_verification_certificate_ledger.json"
...
def load_verification_certificate_audit_ledger_result(path: Path) -> dict[str, Any]:
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("V22.3 Verification Certificate Ledger JSON은 object여야 합니다.")
    return payload
