...
from backtest.sandbox_verification_final_report_v18_0 import (
...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "sandbox_final_report_archive_v18_1"
)
REQUIRED_ARCHIVE_TEXT = (
    "ARCHIVE IN MEMORY SANDBOX VERIFICATION FINAL REPORT V18.0"
)
GENESIS_HASH = "0" * 64


@dataclass(frozen=True)
class SandboxFinalReportArchiveV181Policy:
    required_source_version: str = "V18.0"
...
@dataclass(frozen=True)
class SandboxFinalReportArchiveV181Entry:
...
@dataclass
class SandboxFinalReportArchiveV181Result:
    version: str
    created_at: str
    ...
    live_execution_authorized: bool
    archive_policy: SandboxFinalReportArchiveV181Policy
    entries: tuple[SandboxFinalReportArchiveV181Entry, ...]
...
def validate_policy(
    policy: SandboxFinalReportArchiveV181Policy,
) -> list[str]:
    if not isinstance(policy, SandboxFinalReportArchiveV181Policy):
        return ["Sandbox Verification Final Report Archive Policy 형식 오류입니다."]
    errors: list[str] = []
    expected = {
        "required_source_version": "V18.0",
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V18.1 기준과 다릅니다.")
    ...
    for name in (
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V18.1에서 True여야 합니다.")
    return errors


def normalize_entries(
    existing: Any,
) -> tuple[SandboxFinalReportArchiveV181Entry, ...]:
    if existing is None:
        return ()
    if not isinstance(existing, (tuple, list)):
        raise TypeError("Existing Archive는 tuple/list여야 합니다.")
    if not all(
        isinstance(entry, SandboxFinalReportArchiveV181Entry)
...
def _safe_source(source: SandboxVerificationFinalReportV180Result) -> bool:
    return not any(
        (
...
def _result(
    ...
    reasons: list[str],
    **checks: bool,
) -> SandboxFinalReportArchiveV181Result:
    latest = entries[-1] if entries else None
    return SandboxFinalReportArchiveV181Result(
        version="V18.1",
        ...
        reasons=reasons,
        warnings=[
            "V18.1은 V18.0 Final Report를 In-Memory Archive에 기록만 합니다.",
...
def archive_sandbox_final_report_v18_1(
    source: Any,
    operator: str,
    confirmation_text: str,
    existing: Any = None,
    policy: SandboxFinalReportArchiveV181Policy | None = None,
    now: datetime | None = None,
) -> SandboxFinalReportArchiveV181Result:
    policy = policy or SandboxFinalReportArchiveV181Policy()
    ...
    if policy_errors or input_errors:
        safe_policy = (
            policy
            if isinstance(policy, SandboxFinalReportArchiveV181Policy)
            else SandboxFinalReportArchiveV181Policy()
        )
        return _result(
    ...
    if not isinstance(source, SandboxVerificationFinalReportV180Result):
        return _result(
            policy, now, "FAILED", entries,
            ["Source는 V18.0 Sandbox Verification Final Report Result여야 합니다."],
    ...
    if not (
        ...
        source_errors.append("정상 V18.0 Verification Final Report가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V18.0 Source 실행 안전장치 오류입니다.")
    ...
    if operator != report.operator:
        return _result(
            ...
            ["Operator가 V18.0 Final Report와 다릅니다."],
            policy=True, input=True, source=True, report_hash=True,
    ...
    payload = {
        ...
        "linked_final_report_hash": report.latest_final_report_hash,
    ...
    combined = entries + (
        SandboxFinalReportArchiveV181Entry(
    ...
    if trimmed_count:
        ...
        rebuilt: list[SandboxFinalReportArchiveV181Entry] = []
        previous_hash = GENESIS_HASH
        for sequence, item in enumerate(kept, start=1):
            ...
            item = SandboxFinalReportArchiveV181Entry(
    ...
    result = _result(
        ...
        combined,
        [
            f"V18.0 Final Report {report.final_report_id}를 보관했습니다.",
...
def save_archive_result(
    result: SandboxFinalReportArchiveV181Result,
    output_directory: Path = OUTPUT_DIRECTORY,
) -> tuple[Path, Path]:
    output_directory.mkdir(parents=True, exist_ok=True)
    stamp = result.created_at.replace(":", "").replace("-", "").replace("+", "_")
    report_path = output_directory / (
        f"sandbox_final_report_archive_v18_1_{stamp}.json"
    )
    latest_path = output_directory / (
        "latest_sandbox_final_report_archive_v18_1.json"
    )
