...
from backtest.sandbox_audit_ledger_integrity_verification import (
    SandboxAuditLedgerIntegrityResult,
...
class SandboxVerificationCertificateLedgerPolicy:
    required_source_version: str = "V15.4"
...
@dataclass(frozen=True)
class SandboxVerificationCertificateLedgerEntry:
    ledger_entry_id: str
    sequence: int
    recorded_at: str
...
@dataclass
class SandboxVerificationCertificateLedgerResult:
    version: str
    created_at: str
    ...
    live_execution_authorized: bool
    ledger_policy: SandboxVerificationCertificateLedgerPolicy
...
def validate_policy(
    policy: SandboxVerificationCertificateLedgerPolicy,
) -> list[str]:
    if not isinstance(policy, SandboxVerificationCertificateLedgerPolicy):
        return ["Verification Certificate Ledger Policy 형식 오류입니다."]
    errors: list[str] = []
    expected = {
        "required_source_version": "V15.4",
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V15.5 기준과 다릅니다.")
    ...
    for name in (
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V15.5에서 True여야 합니다.")
    return errors


def normalize_entries(
    existing: Any,
) -> tuple[SandboxVerificationCertificateLedgerEntry, ...]:
    if existing is None:
        return ()
    if not isinstance(existing, (tuple, list)):
        raise TypeError("Existing Verification Ledger는 tuple/list여야 합니다.")
    if not all(
        isinstance(x, SandboxVerificationCertificateLedgerEntry)
        for x in existing
    ):
...
def _safe_source(source: SandboxAuditLedgerIntegrityResult) -> bool:
    return not any(
        (
...
def _result(
    ...
    **checks: bool,
    ...
    return SandboxVerificationCertificateLedgerResult(
        version="V15.5",
        ...
        result_status_label=(
            ...
            else "Verification Ledger 차단"
            if status == "BLOCKED"
            else "Verification Ledger 실패"
        ),
        ...
        reasons=reasons,
        warnings=[
            "V15.5는 V15.4 Verification Certificate를 기록만 합니다.",
            "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
        ],
        next_actions=[
            "Verification Ledger Hash Chain을 확인합니다.",
...
def record_sandbox_verification_certificate(
    ...
    existing: Any = None,
    policy: SandboxVerificationCertificateLedgerPolicy | None = None,
    now: datetime | None = None,
) -> SandboxVerificationCertificateLedgerResult:
    policy = policy or SandboxVerificationCertificateLedgerPolicy()
    ...
    if not isinstance(source, SandboxAuditLedgerIntegrityResult):
        return _result(
            policy, now, "FAILED", entries,
            ["Source는 V15.4 Audit Ledger Integrity Result여야 합니다."],
    ...
    if not (
        ...
        source_errors.append("정상 V15.4 Verification Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V15.4 Source 실행 안전장치 오류입니다.")
    ...
    if any(
        x.verification_certificate_id == certificate.verification_certificate_id
        for x in entries
    ):
        return _result(
    ...
    payload = {
        ...
        "source_audit_ledger_result_id": certificate.source_ledger_result_id,
        "ledger_snapshot_hash": certificate.ledger_snapshot_hash,
        "verified_entry_count": certificate.total_ledger_entry_count,
        "latest_audit_ledger_entry_id": certificate.latest_ledger_entry_id,
        "latest_audit_ledger_entry_hash": certificate.latest_ledger_entry_hash,
    ...
    if trimmed_count:
        ...
        rebuilt: list[SandboxVerificationCertificateLedgerEntry] = []
        previous_hash = GENESIS_HASH
        for sequence, item in enumerate(kept, start=1):
            ...
            item = SandboxVerificationCertificateLedgerEntry(
...
def save_certificate_ledger_result(
    result: SandboxVerificationCertificateLedgerResult,
...
def load_certificate_ledger_result(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("Verification Ledger JSON 최상위 값은 object여야 합니다.")
    return payload
