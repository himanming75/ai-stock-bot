...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "sandbox_verification_audit_ledger_integrity_v18_8"
)
...
class SandboxVerificationAuditLedgerIntegrityV188Policy:
    required_source_version: str = "V18.7"
...
@dataclass(frozen=True)
class SandboxVerificationAuditLedgerFindingV188:
    sequence: int
    ledger_entry_id: str
...
@dataclass(frozen=True)
class SandboxVerificationAuditLedgerCertificateV188:
    ...
    findings: tuple[
        SandboxVerificationAuditLedgerFindingV188, ...
    ]
...
@dataclass
class SandboxVerificationAuditLedgerIntegrityV188Result:
    version: str
    created_at: str
    ...
    certificate: (
        SandboxVerificationAuditLedgerCertificateV188 | None
    )
...
def verify_certificate_audit_ledger_chain(
    ...
    for expected_sequence, entry in enumerate(entries, start=1):
        ...
        if any(not hasattr(entry, name) for name in required):
            errors.append(
                f"Entry {expected_sequence}의 V18.7 구조가 올바르지 않습니다."
            )
            continue
...
def validate_policy(
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V18.8 기준과 다릅니다.")
    for name in (
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V18.8에서 True여야 합니다.")
    return errors
...
def verify_verification_certificate(
    certificate: (
        SandboxVerificationAuditLedgerCertificateV188
    ),
) -> tuple[bool, list[str]]:
    if not isinstance(
        certificate,
...
def _empty_result(
    ...
    reasons: list[str],
    **checks: bool,
) -> SandboxVerificationAuditLedgerIntegrityV188Result:
    return SandboxVerificationAuditLedgerIntegrityV188Result(
        version="V18.8",
        ...
        reasons=reasons,
        warnings=[
            "V18.8는 V18.7 Certificate Audit Ledger를 읽기 전용으로 검증합니다.",
            "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
        ],
        next_actions=["V18.7 Audit Ledger Source와 입력을 확인합니다."],
    )


def verify_sandbox_verification_audit_ledger_v18_8(
    source: Any,
    operator: str,
    ...
    policy = (
        policy
        or SandboxVerificationAuditLedgerIntegrityV188Policy()
    )
    ...
    if policy_errors or input_errors:
        safe_policy = (
            ...
            else SandboxVerificationAuditLedgerIntegrityV188Policy()
        )
    ...
    if (
        ...
        return _empty_result(
            ...
            now,
            "FAILED",
            ["Source는 V18.7 Verification Archive Certificate Audit Ledger여야 합니다."],
            policy=True,
            input=True,
        )
    ...
    try:
        ...
        return _empty_result(
            ...
            now,
            "FAILED",
            ["V18.7 Source Entries 형식이 올바르지 않습니다."],
            policy=True,
            input=True,
        )
    ...
    if not (
        ...
        source_errors.append("정상 V18.7 Audit Ledger Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V18.7 Source 실행 안전장치 오류입니다.")
    if source_errors:
        return _empty_result(
            policy,
            now,
    ...
    if operator != latest.operator:
        return _empty_result(
            ...
            now,
            "BLOCKED",
            ["Operator가 V18.7 Latest Ledger Entry와 다릅니다."],
            policy=True,
            input=True,
            source=True,
            chain=True,
    ...
    return (
        SandboxVerificationAuditLedgerIntegrityV188Result(
            ...
            reasons=[
                f"V18.7 Certificate Audit Ledger Entry {len(entries)}개를 검증했습니다.",
                "Ledger Hash Chain, Source Linkage 및 Snapshot Hash가 유효합니다.",
            ],
            warnings=[
                "V18.8는 V18.7 Certificate Audit Ledger를 변경하지 않습니다.",
                "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
            ],
            next_actions=[
