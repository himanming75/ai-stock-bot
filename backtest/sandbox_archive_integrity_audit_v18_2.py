...
from backtest.sandbox_final_report_archive_v18_1 import (
    GENESIS_HASH,
    SandboxFinalReportArchiveV181Entry as SourceArchiveEntry,
    SandboxFinalReportArchiveV181Result as SourceArchiveResult,
...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "sandbox_archive_integrity_audit_v18_2"
)
REQUIRED_AUDIT_TEXT = (
    "AUDIT IN MEMORY SANDBOX FINAL REPORT ARCHIVE V18.1"
)


@dataclass(frozen=True)
class SandboxArchiveIntegrityAuditV182Policy:
    required_source_version: str = "V18.1"
...
@dataclass(frozen=True)
class SandboxArchiveIntegrityFindingV182:
    sequence: int
...
@dataclass(frozen=True)
class SandboxArchiveIntegrityCertificateV182:
    ...
    findings: tuple[
        SandboxArchiveIntegrityFindingV182, ...
    ]
...
@dataclass
class SandboxArchiveIntegrityAuditV182Result:
    version: str
    created_at: str
    audit_result_id: str
    result_status: str
    ...
    live_execution_authorized: bool
    audit_policy: (
        SandboxArchiveIntegrityAuditV182Policy
    )
    certificate: (
        SandboxArchiveIntegrityCertificateV182 | None
    )
...
def validate_policy(
    policy: SandboxArchiveIntegrityAuditV182Policy,
) -> list[str]:
    if not isinstance(
        policy,
        SandboxArchiveIntegrityAuditV182Policy,
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V18.2 기준과 다릅니다.")
    for name in (
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V18.2에서 True여야 합니다.")
    return errors
...
def verify_audit_certificate(
    certificate: (
        SandboxArchiveIntegrityCertificateV182
    ),
) -> tuple[bool, list[str]]:
    if not isinstance(
        certificate,
...
def _empty_result(
    ...
    reasons: list[str],
    **checks: bool,
) -> SandboxArchiveIntegrityAuditV182Result:
    return SandboxArchiveIntegrityAuditV182Result(
        version="V18.2",
        ...
        reasons=reasons,
        warnings=[
            "V18.2는 V18.1 Archive를 읽기 전용으로 감사합니다.",
            "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
        ],
        next_actions=["V18.1 Archive Source와 입력을 수동 확인합니다."],
    )


def audit_sandbox_archive_integrity_v18_2(
    source: Any,
    operator: str,
    confirmation_text: str,
    policy: (
        SandboxArchiveIntegrityAuditV182Policy | None
    ) = None,
    now: datetime | None = None,
) -> SandboxArchiveIntegrityAuditV182Result:
    policy = (
        policy
        or SandboxArchiveIntegrityAuditV182Policy()
    )
    ...
    if policy_errors or input_errors:
        safe_policy = (
            ...
            else SandboxArchiveIntegrityAuditV182Policy()
        )
    ...
    if not isinstance(
        ...
        return _empty_result(
            ...
            now,
            "FAILED",
            ["Source는 V18.1 Verification Final Report Archive여야 합니다."],
            policy=True,
            input=True,
        )
    ...
    if not (
        ...
        source_errors.append("정상 V18.1 Archive Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V18.1 Source 실행 안전장치 오류입니다.")
    entries = tuple(source.entries)
    if not entries:
        source_errors.append("V18.1 Archive Entry가 비어 있습니다.")
    if source_errors:
        return _empty_result(
            policy,
            now,
    ...
    if operator != latest.operator:
        return _empty_result(
            ...
            now,
            "BLOCKED",
            ["Operator가 V18.1 Latest Archive Entry와 다릅니다."],
            policy=True,
            input=True,
            source=True,
            chain=True,
...
def save_audit_result(
    result: SandboxArchiveIntegrityAuditV182Result,
