...
from backtest.sandbox_verification_audit_ledger import (
    GENESIS_HASH,
    SandboxVerificationAuditLedgerEntry,
    SandboxVerificationAuditLedgerResult,
    sha256_payload as ledger_sha256_payload,
    verify_verification_audit_ledger_chain,
)
...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "sandbox_verification_audit_ledger_integrity_verification"
)
...
class SandboxVerificationAuditLedgerIntegrityPolicy:
    required_source_version: str = "V15.7"
...
@dataclass(frozen=True)
class SandboxVerificationAuditLedgerFinding:
    sequence: int
    ledger_entry_id: str
...
@dataclass(frozen=True)
class SandboxVerificationAuditLedgerVerificationCertificate:
    ...
    findings: tuple[SandboxVerificationAuditLedgerFinding, ...]
...
@dataclass
class SandboxVerificationAuditLedgerIntegrityResult:
    version: str
    created_at: str
    ...
    live_execution_authorized: bool
    verification_policy: SandboxVerificationAuditLedgerIntegrityPolicy
    certificate: SandboxVerificationAuditLedgerVerificationCertificate | None
...
def ledger_snapshot_hash(
    entries: tuple[SandboxVerificationAuditLedgerEntry, ...],
) -> str:
    return sha256_payload({"entries": [entry.to_dict() for entry in entries]})


def validate_policy(
    policy: SandboxVerificationAuditLedgerIntegrityPolicy,
) -> list[str]:
    if not isinstance(policy, SandboxVerificationAuditLedgerIntegrityPolicy):
        return ["Verification Audit Ledger Integrity Policy 형식 오류입니다."]
    errors: list[str] = []
    expected = {
        "required_source_version": "V15.7",
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V15.8 기준과 다릅니다.")
    for name in (
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V15.8에서 True여야 합니다.")
    return errors


def _safe_entry(entry: SandboxVerificationAuditLedgerEntry) -> bool:
    return not any(
        (
...
def _safe_source(source: SandboxVerificationAuditLedgerResult) -> bool:
    return not any(
        (
...
def build_findings(
    entries: tuple[SandboxVerificationAuditLedgerEntry, ...],
) -> tuple[SandboxVerificationAuditLedgerFinding, ...]:
    findings: list[SandboxVerificationAuditLedgerFinding] = []
    ...
    for expected_sequence, entry in enumerate(entries, start=1):
        ...
        certificate_id_unique = entry.audit_certificate_id not in certificate_ids
        ...
        findings.append(
            SandboxVerificationAuditLedgerFinding(
...
def verify_verification_certificate(
    certificate: SandboxVerificationAuditLedgerVerificationCertificate,
) -> tuple[bool, list[str]]:
    if not isinstance(
        certificate, SandboxVerificationAuditLedgerVerificationCertificate
    ):
    ...
    if not all(
        ...
        errors.append("Verification Certificate 검사 결과가 올바르지 않습니다.")
...
def _empty_result(
    ...
    **checks: bool,
    ...
    return SandboxVerificationAuditLedgerIntegrityResult(
        version="V15.8",
        created_at=now.isoformat(),
        verification_result_id=str(uuid.uuid4()),
        result_status=status,
        result_status_label=(
            "Verification Audit Ledger Integrity Verification 차단"
            if status == "BLOCKED"
            else "Verification Audit Ledger Integrity Verification 실패"
        ),
        ...
        reasons=reasons,
        warnings=[
            "V15.8은 V15.7 Verification Audit Ledger를 읽기 전용으로 검증합니다.",
            "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
        ],
        next_actions=["V15.7 Verification Audit Ledger Source를 확인합니다."],
    )


def verify_sandbox_verification_audit_ledger_integrity(
    source: Any,
    operator: str,
    confirmation_text: str,
    policy: SandboxVerificationAuditLedgerIntegrityPolicy | None = None,
    now: datetime | None = None,
) -> SandboxVerificationAuditLedgerIntegrityResult:
    policy = policy or SandboxVerificationAuditLedgerIntegrityPolicy()
    ...
    if policy_errors or input_errors:
        safe_policy = (
            policy
            if isinstance(policy, SandboxVerificationAuditLedgerIntegrityPolicy)
            else SandboxVerificationAuditLedgerIntegrityPolicy()
        )
        return _empty_result(
            safe_policy, now, "BLOCKED", policy_errors + input_errors,
            policy=not policy_errors, input=not input_errors,
        )
    if not isinstance(source, SandboxVerificationAuditLedgerResult):
        return _empty_result(
            policy, now, "FAILED",
            ["Source는 V15.7 Verification Audit Ledger Result여야 합니다."],
    ...
    if not (
        ...
        source_errors.append("정상 V15.7 Verification Audit Ledger가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V15.7 Source 실행 안전장치가 올바르지 않습니다.")
    if source_errors:
        return _empty_result(
            policy, now, "FAILED", source_errors, policy=True, input=True
        )
    source_before = canonical_json(source.to_dict())
    chain_valid, chain_errors = verify_verification_audit_ledger_chain(entries)
    ...
    if operator != latest.operator:
        return _empty_result(
            policy, now, "BLOCKED",
            ["Operator가 V15.7 Latest Ledger Entry와 다릅니다."],
    ...
    if not unchanged:
        return _empty_result(
            policy, now, "FAILED",
            ["읽기 전용 Verification 중 Ledger가 변경되었습니다."],
            policy=True, input=True, source=True, chain=True, linkage=True,
    ...
    return SandboxVerificationAuditLedgerIntegrityResult(
        ...
        result_status_label=(
            "Sandbox Verification Audit Ledger Integrity Verification 완료"
        ),
        verification_status="PASSED",
        source_ledger_result_id=source.ledger_result_id,
        ...
        reasons=[
            f"V15.7 Verification Audit Ledger Entry {len(entries)}개를 검증했습니다.",
            "Ledger Hash Chain, Source Linkage 및 Snapshot Hash가 유효합니다.",
        ],
        warnings=[
            "V15.8은 V15.7 Verification Audit Ledger를 변경하지 않습니다.",
            "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
        ],
        next_actions=[
            "Ledger Snapshot Hash와 Verification Certificate Hash를 보관합니다.",
            "검증 결과를 실제 거래와 분리해 수동 검토합니다.",
        ],
    )


def save_verification_result(
    result: SandboxVerificationAuditLedgerIntegrityResult,
    output_directory: Path = OUTPUT_DIRECTORY,
) -> tuple[Path, Path]:
    output_directory.mkdir(parents=True, exist_ok=True)
    stamp = result.created_at.replace(":", "").replace("-", "").replace("+", "_")
    report = output_directory / f"verification_audit_ledger_verification_{stamp}.json"
    latest = output_directory / "latest_verification_audit_ledger_verification.json"
...
def load_verification_result(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("Verification JSON 최상위 값은 object여야 합니다.")
    return payload
