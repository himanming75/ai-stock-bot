...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "offline_paper_audit_verify_ledger_v21_9"
)
REQUIRED_VERIFY_LEDGER_TEXT = (
    "RECORD OFFLINE PAPER AUDIT INTEGRITY VERIFICATION V21.9"
)
GENESIS_HASH = "0" * 64


@dataclass(frozen=True)
class OfflinePaperAuditVerifyLedgerV219Policy:
    required_source_version: str = "V21.8"
    required_source_status: str = "VERIFIED_IN_MEMORY"
    required_verification_status: str = "PASSED"
    required_confirmation_text: str = REQUIRED_VERIFY_LEDGER_TEXT
...
@dataclass(frozen=True)
class OfflinePaperAuditVerifyLedgerV219Entry:
    ledger_entry_id: str
    sequence: int
    recorded_at: str
    ...
    source_audit_integrity_ledger_result_id: str
    audit_integrity_ledger_snapshot_hash: str
    verified_entry_count: int
    latest_audit_integrity_ledger_entry_id: str
    latest_audit_integrity_ledger_entry_hash: str
...
@dataclass
class OfflinePaperAuditVerifyLedgerV219Result:
    version: str
    created_at: str
    ...
    latest_source_audit_integrity_ledger_result_id: str | None
    latest_audit_integrity_ledger_snapshot_hash: str | None
    ...
    live_order_created: bool
    live_execution_authorized: bool
    ledger_policy: OfflinePaperAuditVerifyLedgerV219Policy
    entries: tuple[OfflinePaperAuditVerifyLedgerV219Entry, ...]
...
def validate_policy(
    policy: OfflinePaperAuditVerifyLedgerV219Policy,
) -> list[str]:
    if not isinstance(policy, OfflinePaperAuditVerifyLedgerV219Policy):
        return ["V21.9 Audit Integrity Ledger Policy 형식 오류입니다."]
    errors: list[str] = []
    expected = {
        "required_source_version": "V21.8",
        "required_source_status": "VERIFIED_IN_MEMORY",
        "required_verification_status": "PASSED",
        "required_confirmation_text": REQUIRED_VERIFY_LEDGER_TEXT,
    }
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V21.9 기준과 다릅니다.")
    if (
        ...
        errors.append("V21.9 Ledger 보관 한도는 양의 정수여야 합니다.")
    for name in (
        "require_operator",
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V21.9에서 True여야 합니다.")
    return errors


def normalize_entries(
    existing: Any,
) -> tuple[OfflinePaperAuditVerifyLedgerV219Entry, ...]:
    if existing is None:
        return ()
    if not isinstance(existing, (tuple, list)):
        raise TypeError("Existing V21.9 Ledger는 tuple/list여야 합니다.")
    if not all(
        isinstance(entry, OfflinePaperAuditVerifyLedgerV219Entry)
        for entry in existing
    ):
        raise TypeError("Existing V21.9 Ledger Entry 형식 오류입니다.")
    return tuple(existing)


def _safe_entry(entry: OfflinePaperAuditVerifyLedgerV219Entry) -> bool:
    return not any(
        (
...
def _safe_source(source: OfflinePaperAuditIntegrityVerifyV218Result) -> bool:
    return not any(
        (
...
def verify_audit_verify_ledger_chain(
    entries: tuple[OfflinePaperAuditVerifyLedgerV219Entry, ...],
) -> tuple[bool, list[str]]:
    errors: list[str] = []
    ...
    for expected_sequence, entry in enumerate(entries, start=1):
        if entry.sequence != expected_sequence:
            errors.append(f"V21.9 Ledger Sequence {expected_sequence} 오류입니다.")
        ...
        if entry.verification_certificate_id in certificate_ids:
            errors.append(
                "중복 V21.8 Verification Certificate ID: "
        ...
        try:
            ...
            if previous_time and recorded_at < previous_time:
                errors.append("V21.9 Ledger 기록 시간이 역순입니다.")
            previous_time = recorded_at
        except (TypeError, ValueError):
            errors.append("V21.9 Ledger 기록 시간이 올바르지 않습니다.")
        if entry.verification_status != "PASSED":
            errors.append("V21.9 Ledger Verification Status 오류입니다.")
        if not _safe_entry(entry):
            errors.append("V21.9 Ledger Entry 실행 안전장치 오류입니다.")
...
def _result(
    ...
    reasons: list[str],
    **checks: bool,
) -> OfflinePaperAuditVerifyLedgerV219Result:
    latest = entries[-1] if entries else None
    recorded = status == "RECORDED_IN_MEMORY"
    return OfflinePaperAuditVerifyLedgerV219Result(
        version="V21.9",
        created_at=now.isoformat(),
        ledger_result_id=str(uuid.uuid4()),
        result_status=status,
        result_status_label=(
            "Offline Paper Audit Integrity Verification Certificate 기록 완료"
            if recorded
            else "Offline Paper Audit Integrity Verification Ledger 차단"
            if status == "BLOCKED"
            else "Offline Paper Audit Integrity Verification Ledger 실패"
        ),
        ...
        latest_source_audit_integrity_ledger_result_id=(
            latest.source_audit_integrity_ledger_result_id if latest else None
        ),
        latest_audit_integrity_ledger_snapshot_hash=(
            latest.audit_integrity_ledger_snapshot_hash if latest else None
        ),
        latest_audit_certificate_hash=(
            latest.latest_audit_certificate_hash if latest else None
        ),
        latest_audit_certificate_ledger_snapshot_hash=(
            latest.latest_audit_certificate_ledger_snapshot_hash
            if latest
            else None
        ),
        ...
        reasons=reasons,
        warnings=[
            "V21.9은 V21.8 Verification Certificate를 In-Memory Ledger에 "
            "기록합니다.",
            "기록은 주문 체결 또는 Broker 전송 승인을 의미하지 않습니다.",
            "잔액·보유수량 변경, API, Network 및 Live Execution은 차단됩니다.",
        ],
        next_actions=[
            "V22.0 Audit Certificate Ledger Integrity Verification "
            "Certificate Ledger Integrity Audit 단계에서 V21.9 Ledger를 "
            "읽기 전용으로 검증합니다."
        ],
    )
...
def _source_linkage(
    source: OfflinePaperAuditIntegrityVerifyV218Result,
) -> list[str]:
    certificate = source.certificate
    if certificate is None:
        return ["V21.8 Verification Certificate가 없습니다."]
    errors: list[str] = []
    for actual, expected, label in (
        (
            source.verification_certificate_id,
            certificate.verification_certificate_id,
            "V21.8 Verification Certificate ID",
        ),
        (
            source.verification_certificate_hash,
            certificate.certificate_hash,
            "V21.8 Verification Certificate Hash",
        ),
        (
        ...
        (
            source.audit_integrity_ledger_snapshot_hash,
            certificate.audit_integrity_ledger_snapshot_hash,
            "V21.7 Audit Integrity Ledger Snapshot Hash",
        ),
        (
        ...
        (
            ...
            certificate.latest_audit_certificate_ledger_snapshot_hash,
...
def record_offline_paper_audit_verification_v21_9(
    source: Any,
    operator: Any,
    confirmation_text: Any,
    existing: Any = None,
    policy: OfflinePaperAuditVerifyLedgerV219Policy | None = None,
    now: datetime | None = None,
) -> OfflinePaperAuditVerifyLedgerV219Result:
    policy = policy or OfflinePaperAuditVerifyLedgerV219Policy()
    ...
    if confirmation_text != getattr(policy, "required_confirmation_text", None):
        input_errors.append("V21.9 확인 문구가 일치하지 않습니다.")
    if policy_errors or input_errors:
        safe_policy = (
            policy
            if isinstance(policy, OfflinePaperAuditVerifyLedgerV219Policy)
            else OfflinePaperAuditVerifyLedgerV219Policy()
        )
        return _result(
            safe_policy,
            now,
    ...
    chain_valid, chain_errors = verify_audit_verify_ledger_chain(entries)
    if not chain_valid:
        return _result(
            policy,
            now,
    ...
    if not isinstance(source, OfflinePaperAuditIntegrityVerifyV218Result):
        return _result(
            policy,
            now,
            "FAILED",
            entries,
            ["Source는 V21.8 Audit Integrity Verification Result여야 합니다."],
            policy=True,
            input=True,
            existing=True,
            chain=True,
    ...
    if not (
        ...
        source_errors.append("정상 V21.8 Integrity Verification Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V21.8 Source 실행 안전장치 오류입니다.")
    ...
    if certificate.verification_status != policy.required_verification_status:
        linkage_errors.append(
            "V21.8 Verification Status가 V21.9 기준과 다릅니다."
        )
    if linkage_errors:
        return _result(
            policy,
    ...
    if any(
        ...
        return _result(
            ...
            "BLOCKED",
            entries,
            ["중복 V21.8 Verification Certificate ID가 차단되었습니다."],
            policy=True,
            input=True,
            source=True,
            certificate=True,
    ...
    try:
        ...
        return _result(
            ...
            "BLOCKED",
            entries,
            ["역순 또는 잘못된 V21.9 Ledger 시간이 차단되었습니다."],
            policy=True,
            input=True,
            source=True,
            certificate=True,
    ...
    if source_before != canonical_json(source.to_dict()):
        return _result(
            ...
            "FAILED",
            entries,
            ["V21.9 기록 중 V21.8 Source가 변경되었습니다."],
            policy=True,
            input=True,
            source=True,
            certificate=True,
    ...
    payload = {
        ...
        "source_audit_integrity_ledger_result_id": (
            certificate.source_ledger_result_id
        ),
        "audit_integrity_ledger_snapshot_hash": (
            certificate.audit_integrity_ledger_snapshot_hash
        ),
        "verified_entry_count": certificate.total_ledger_entry_count,
        "latest_audit_integrity_ledger_entry_id": (
            certificate.latest_ledger_entry_id
        ),
        "latest_audit_integrity_ledger_entry_hash": (
            certificate.latest_ledger_entry_hash
        ),
        "latest_audit_certificate_hash": (
            certificate.latest_audit_certificate_hash
    ...
    new_entry = OfflinePaperAuditVerifyLedgerV219Entry(
        **payload,
    ...
    if trimmed_count:
        ...
        rebuilt: list[OfflinePaperAuditVerifyLedgerV219Entry] = []
        previous_hash = GENESIS_HASH
    ...
    valid_chain, final_errors = verify_audit_verify_ledger_chain(combined)
    result = _result(
        policy,
        now,
        ...
        combined,
        [
            "V21.8 Verification Certificate "
            f"{certificate.verification_certificate_id}를 기록했습니다.",
            f"현재 V21.9 Ledger Entry는 {len(combined)}개입니다.",
        ]
        + final_errors,
        policy=True,
        input=True,
...
def save_audit_verification_ledger_result(
    result: OfflinePaperAuditVerifyLedgerV219Result,
    output_directory: Path = OUTPUT_DIRECTORY,
) -> tuple[Path, Path]:
    output_directory.mkdir(parents=True, exist_ok=True)
    stamp = result.created_at.replace(":", "").replace("-", "").replace("+", "_")
    report_path = output_directory / f"audit_verify_ledger_{stamp}.json"
...
def load_audit_verification_ledger_result(path: Path) -> dict[str, Any]:
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("V21.9 Audit Integrity Ledger JSON은 object여야 합니다.")
    return payload
