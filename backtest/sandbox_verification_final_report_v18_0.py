...
from backtest.sandbox_verification_certificate_ledger_v17_9 import (
    SandboxVerificationFinalReportArchiveCertificateAuditFinalReportArchiveIntegrityAuditLedgerIntegrityVerificationCertificateLedgerIntegrityAuditLedgerIntegrityVerificationCertificateLedgerEntry as SourceCertificateLedgerEntry,
    SandboxVerificationFinalReportArchiveCertificateAuditFinalReportArchiveIntegrityAuditLedgerIntegrityVerificationCertificateLedgerIntegrityAuditLedgerIntegrityVerificationCertificateLedgerResult as SourceCertificateLedgerResult,
...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "sandbox_verification_final_report_v18_0"
)
REQUIRED_FINAL_REPORT_TEXT = (
    "GENERATE IN MEMORY SANDBOX VERIFICATION FINAL REPORT V18.0"
)


@dataclass(frozen=True)
class SandboxVerificationFinalReportV180Policy:
    required_source_version: str = "V17.9"
...
@dataclass(frozen=True)
class SandboxVerificationCertificateSummaryV180:
    sequence: int
    ledger_entry_id: str
    recorded_at: str
...
@dataclass(frozen=True)
class SandboxVerificationFinalReportV180:
    final_report_id: str
    finalized_at: str
    report_status: str
    ...
    certificate_summaries: tuple[
        SandboxVerificationCertificateSummaryV180,
        ...,
    ]
    report_hash: str
...
@dataclass
class SandboxVerificationFinalReportV180Result:
    version: str
    created_at: str
    ...
    live_execution_authorized: bool
    report_policy: SandboxVerificationFinalReportV180Policy
    report: SandboxVerificationFinalReportV180 | None
...
def validate_policy(
    policy: SandboxVerificationFinalReportV180Policy,
) -> list[str]:
    if not isinstance(policy, SandboxVerificationFinalReportV180Policy):
        return ["Sandbox Verification Final Report Policy 형식 오류입니다."]
    errors: list[str] = []
    expected = {
        "required_source_version": "V17.9",
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V18.0 기준과 다릅니다.")
    for name in (
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V18.0에서 True여야 합니다.")
    return errors


def verify_final_report(
    report: SandboxVerificationFinalReportV180,
) -> tuple[bool, list[str]]:
    if not isinstance(report, SandboxVerificationFinalReportV180):
...
def _empty_result(
    ...
    reasons: list[str],
    **checks: bool,
) -> SandboxVerificationFinalReportV180Result:
    return SandboxVerificationFinalReportV180Result(
        version="V18.0",
        ...
        reasons=reasons,
        warnings=[
            "V18.0은 In-Memory Sandbox Verification 최종 보고서만 생성합니다.",
            "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
        ],
        next_actions=["입력과 V17.9 Certificate Ledger를 수동 확인합니다."],
    )


def generate_sandbox_verification_final_report_v18_0(
    source: Any,
    operator: str,
    confirmation_text: str,
    policy: SandboxVerificationFinalReportV180Policy | None = None,
    now: datetime | None = None,
) -> SandboxVerificationFinalReportV180Result:
    policy = policy or SandboxVerificationFinalReportV180Policy()
    ...
    if policy_errors or input_errors:
        safe_policy = (
            policy
            if isinstance(policy, SandboxVerificationFinalReportV180Policy)
            else SandboxVerificationFinalReportV180Policy()
        )
    ...
    if not isinstance(
        ...
        return _empty_result(
            policy, now, "FAILED",
            ["Source는 V17.9 Verification Certificate Ledger Result여야 합니다."],
    ...
    if not (
        ...
        source_errors.append("정상 V17.9 Certificate Ledger Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V17.9 Source 실행 안전장치 오류입니다.")
    if source_errors:
        return _empty_result(
    ...
    if len(operators) != 1 or operator not in operators:
        validation_errors.append("Operator가 V17.9 Certificate Ledger와 다릅니다.")
    try:
    ...
    return SandboxVerificationFinalReportV180Result(
        ...
        report=report,
        reasons=[
            f"V17.9 Certificate Ledger Entry {len(entries)}개를 검증했습니다.",
            "Certificate Ledger Hash Chain과 Source Linkage가 유효합니다.",
            "최종 상태는 SANDBOX_VERIFICATION_COMPLETE입니다.",
        ],
        warnings=[
            "V18.0 Final Report는 실행 또는 주문 권한을 부여하지 않습니다.",
            "V17.9 Certificate Ledger는 변경되지 않았습니다.",
            "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
        ],
        next_actions=[
...
def save_final_report_result(
    result: SandboxVerificationFinalReportV180Result,
    output_directory: Path = OUTPUT_DIRECTORY,
) -> tuple[Path, Path]:
    output_directory.mkdir(parents=True, exist_ok=True)
    stamp = result.created_at.replace(":", "").replace("-", "").replace("+", "_")
    report_path = output_directory / (
        f"sandbox_verification_final_report_v18_0_{stamp}.json"
    )
    latest_path = output_directory / (
        "latest_sandbox_verification_final_report_v18_0.json"
    )
