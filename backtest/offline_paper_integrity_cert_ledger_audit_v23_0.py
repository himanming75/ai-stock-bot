...
from backtest.offline_paper_integrity_cert_ledger_v22_9 import (
    GENESIS_HASH,
    OfflinePaperIntegrityCertLedgerV229Entry,
...
OUTPUT_DIRECTORY = (
    ...
    / "offline_paper_integrity_cert_ledger_audit_v23_0"
)
CONFIRMATION_TEXT = "AUDIT OFFLINE PAPER INTEGRITY CERTIFICATE LEDGER V23.0"


@dataclass(frozen=True)
class OfflinePaperIntegrityCertLedgerAuditV230Policy:
    required_source_version: str = "V22.9"
...
@dataclass(frozen=True)
class OfflinePaperIntegrityCertLedgerAuditFindingV230:
    sequence: int
    ledger_entry_id: str
    sequence_valid: bool
...
@dataclass(frozen=True)
class OfflinePaperIntegrityCertLedgerAuditCertificateV230:
    ...
    latest_source_audit_certificate_ledger_snapshot_hash: str
    ...
    findings: tuple[OfflinePaperIntegrityCertLedgerAuditFindingV230, ...]
...
@dataclass
class OfflinePaperIntegrityCertLedgerAuditV230Result:
    version: str
    created_at: str
    audit_result_id: str
    result_status: str
    ...
    order_submitted: bool
    live_execution_authorized: bool
    audit_policy: OfflinePaperIntegrityCertLedgerAuditV230Policy
    certificate: OfflinePaperIntegrityCertLedgerAuditCertificateV230 | None
...
def validate_policy(policy: OfflinePaperIntegrityCertLedgerAuditV230Policy) -> list[str]:
    if not isinstance(policy, OfflinePaperIntegrityCertLedgerAuditV230Policy):
        return ["V23.0 Policy 형식 오류입니다."]
    errors: list[str] = []
    if policy.required_source_version != "V22.9":
        errors.append("V23.0 Source Version은 V22.9이어야 합니다.")
    if policy.required_source_status != "RECORDED_IN_MEMORY":
        errors.append("V23.0 Source Status 정책 오류입니다.")
    if policy.required_confirmation_text != CONFIRMATION_TEXT:
        errors.append("V23.0 확인 문구 정책 오류입니다.")
    for name, value in policy.to_dict().items():
        if name.startswith(("require_", "read_only_")) or name.endswith("_disabled"):
            if value is not True:
                errors.append(f"{name}는 V23.0에서 True여야 합니다.")
    return errors
...
def _source_linkage_valid(entry: OfflinePaperIntegrityCertLedgerV229Entry) -> bool:
    hashes = (
        entry.source_integrity_certificate_hash,
        entry.source_audit_certificate_ledger_snapshot_hash,
        entry.latest_source_audit_certificate_hash,
    )
    return bool(
        entry.source_integrity_result_id
        and entry.source_integrity_certificate_id
        and entry.source_audit_certificate_ledger_result_id
...
def create_findings(
...
) -> tuple[OfflinePaperIntegrityCertLedgerAuditFindingV230, ...]:
    findings = []
    ...
    for expected, entry in enumerate(entries, start=1):
        ...
        unique = entry.source_integrity_certificate_id not in certificate_ids
        checks = (
        ...
        findings.append(
            OfflinePaperIntegrityCertLedgerAuditFindingV230(
...
def verify_integrity_certificate(
    ...
    if not isinstance(certificate, OfflinePaperIntegrityCertLedgerAuditCertificateV230):
        return False, ["V23.0 Integrity Certificate 형식 오류입니다."]
    expected = sha256_payload(certificate.payload_without_hash())
    valid = certificate.certificate_hash == expected
    return valid, [] if valid else ["V23.0 Integrity Certificate Hash 오류입니다."]


def _result(
    policy: OfflinePaperIntegrityCertLedgerAuditV230Policy,
    now: datetime,
    source: OfflinePaperIntegrityCertLedgerV229Result | None,
    errors: list[str],
    certificate: OfflinePaperIntegrityCertLedgerAuditCertificateV230 | None = None,
) -> OfflinePaperIntegrityCertLedgerAuditV230Result:
    ...
    return OfflinePaperIntegrityCertLedgerAuditV230Result(
        version="V23.0", created_at=now.isoformat(), audit_result_id=str(uuid.uuid4()),
        ...
        certificate=certificate, reasons=errors or [
            "V22.9 Integrity Certificate Ledger가 읽기 전용으로 감사되었습니다.",
            "외부 API·Network·Broker 주문·Live Execution은 차단되었습니다.",
        ],
    )


def audit_offline_paper_integrity_certificate_ledger_v23_0(
    source: OfflinePaperIntegrityCertLedgerV229Result,
    *,
    operator: str,
    confirmation_text: str,
    policy: OfflinePaperIntegrityCertLedgerAuditV230Policy | None = None,
    now: datetime | None = None,
) -> OfflinePaperIntegrityCertLedgerAuditV230Result:
    selected = policy or OfflinePaperIntegrityCertLedgerAuditV230Policy()
    current = (now or datetime.now(timezone.utc)).astimezone(timezone.utc)
    errors = validate_policy(selected)
    if not isinstance(operator, str) or not operator.strip():
        errors.append("V23.0 Operator가 필요합니다.")
    if confirmation_text != CONFIRMATION_TEXT:
        errors.append("V23.0 확인 문구가 일치하지 않습니다.")
    if not isinstance(source, OfflinePaperIntegrityCertLedgerV229Result):
        errors.append("Source는 V22.9 Ledger Result여야 합니다.")
        return _result(selected, current, None, errors)
    before = source.to_dict()
    if source.version != selected.required_source_version:
        errors.append("V22.9 Source Version 오류입니다.")
    if source.result_status != selected.required_source_status or not source.all_checks_passed:
        errors.append("V22.9 Source Status 오류입니다.")
    if not source.entries:
        errors.append("V22.9 Ledger가 비어 있습니다.")
    ...
    payload = {
        ...
        "latest_source_audit_certificate_hash": latest.latest_source_audit_certificate_hash,
        "latest_source_audit_certificate_ledger_snapshot_hash": (
            latest.source_audit_certificate_ledger_snapshot_hash
        ),
    ...
    certificate = OfflinePaperIntegrityCertLedgerAuditCertificateV230(
        **payload,
...
def save_result(
    result: OfflinePaperIntegrityCertLedgerAuditV230Result,
...
def load_result(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("V23.0 Audit JSON은 object여야 합니다.")
    return payload
