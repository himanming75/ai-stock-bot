...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "sandbox_verification_audit_ledger_v18_7"
)
REQUIRED_LEDGER_TEXT = (
    "RECORD IN MEMORY SANDBOX VERIFICATION AUDIT LEDGER V18.7"
)
GENESIS_HASH = "0" * 64


@dataclass(frozen=True)
class SandboxVerificationAuditLedgerV187Policy:
    required_source_version: str = "V18.6"
...
@dataclass(frozen=True)
class SandboxVerificationAuditLedgerV187Entry:
    ledger_entry_id: str
    sequence: int
    recorded_at: str
...
@dataclass
class SandboxVerificationAuditLedgerV187Result:
    version: str
    created_at: str
    ...
    live_execution_authorized: bool
    ledger_policy: (
        SandboxVerificationAuditLedgerV187Policy
    )
    entries: tuple[
        SandboxVerificationAuditLedgerV187Entry, ...
    ]
...
def verify_audit_certificate(
    ...
    if (
        ...
        return False, ["V18.6 Audit Certificate 구조 오류입니다."]
    errors: list[str] = []
...
def validate_policy(
    policy: SandboxVerificationAuditLedgerV187Policy,
) -> list[str]:
    if not isinstance(
        policy,
        SandboxVerificationAuditLedgerV187Policy,
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V18.7 기준과 다릅니다.")
    ...
    for name in (
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V18.7에서 True여야 합니다.")
    return errors
...
def verify_sandbox_verification_audit_ledger_v18_7(
    entries: tuple[
        SandboxVerificationAuditLedgerV187Entry, ...
    ],
) -> tuple[bool, list[str]]:
...
def _result(
    ...
    reasons: list[str],
    **checks: bool,
) -> SandboxVerificationAuditLedgerV187Result:
    latest = entries[-1] if entries else None
    return SandboxVerificationAuditLedgerV187Result(
        version="V18.7",
        ...
        reasons=reasons,
        warnings=[
            "V18.7은 V18.6 Audit Certificate를 메모리에 기록만 합니다.",
...
def record_sandbox_verification_audit_v18_7(
    source: Any,
    operator: str,
    ...
    policy = (
        policy
        or SandboxVerificationAuditLedgerV187Policy()
    )
    ...
    if policy_errors or input_errors:
        safe_policy = (
            ...
            else SandboxVerificationAuditLedgerV187Policy()
        )
        return _result(
    ...
    chain_valid, chain_errors = verify_sandbox_verification_audit_ledger_v18_7(
        entries
    )
    if not chain_valid:
        return _result(
    ...
    if any(
        ...
        return _result(
            ...
            ["Source는 V18.6 Certificate Ledger Integrity Audit여야 합니다."],
    ...
    if not (
        ...
        source_errors.append("정상 V18.6 Certificate Audit Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V18.6 Source 실행 안전장치 오류입니다.")
    ...
    valid, errors = verify_sandbox_verification_audit_ledger_v18_7(combined)
    result = _result(
...
def save_sandbox_verification_audit_ledger_v18_7(
    result: SandboxVerificationAuditLedgerV187Result,
...
def load_sandbox_verification_audit_ledger_v18_7(path: Path) -> dict[str, Any]:
