...
from backtest.sandbox_verification_audit_certificate_ledger import (
...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "sandbox_verification_final_report"
)
REQUIRED_FINAL_REPORT_TEXT = (
    "GENERATE IN MEMORY SANDBOX VERIFICATION FINAL REPORT"
)


@dataclass(frozen=True)
class SandboxVerificationFinalReportPolicy:
    required_source_version: str = "V15.9"
...
@dataclass(frozen=True)
class SandboxVerificationCertificateSummary:
    sequence: int
    ledger_entry_id: str
    recorded_at: str
...
@dataclass(frozen=True)
class SandboxVerificationFinalReport:
    final_report_id: str
    finalized_at: str
    report_status: str
    ...
    certificate_summaries: tuple[SandboxVerificationCertificateSummary, ...]
    report_hash: str
...
@dataclass
class SandboxVerificationFinalReportResult:
    version: str
    created_at: str
    ...
    live_execution_authorized: bool
    report_policy: SandboxVerificationFinalReportPolicy
    report: SandboxVerificationFinalReport | None
...
def validate_policy(
    policy: SandboxVerificationFinalReportPolicy,
) -> list[str]:
    if not isinstance(policy, SandboxVerificationFinalReportPolicy):
        return ["Sandbox Verification Final Report Policy 형식 오류입니다."]
    errors: list[str] = []
    expected = {
        "required_source_version": "V15.9",
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V16.0 기준과 다릅니다.")
    for name in (
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V16.0에서 True여야 합니다.")
    return errors


def verify_final_report(
    report: SandboxVerificationFinalReport,
) -> tuple[bool, list[str]]:
    if not isinstance(report, SandboxVerificationFinalReport):
...
def _safe_source(
    source: SandboxVerificationAuditCertificateLedgerResult,
) -> bool:
    return not any(
        (
...
def _empty_result(
    ...
    reasons: list[str],
    **checks: bool,
) -> SandboxVerificationFinalReportResult:
    return SandboxVerificationFinalReportResult(
        version="V16.0",
        ...
        reasons=reasons,
        warnings=[
            "V16.0은 In-Memory Sandbox Verification 최종 보고서만 생성합니다.",
            "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
        ],
        next_actions=["입력과 V15.9 Certificate Ledger를 수동 확인합니다."],
    )


def generate_sandbox_verification_final_report(
    source: Any,
    operator: str,
    confirmation_text: str,
    policy: SandboxVerificationFinalReportPolicy | None = None,
    now: datetime | None = None,
) -> SandboxVerificationFinalReportResult:
    policy = policy or SandboxVerificationFinalReportPolicy()
    ...
    if policy_errors or input_errors:
        safe_policy = (
            policy
            if isinstance(policy, SandboxVerificationFinalReportPolicy)
            else SandboxVerificationFinalReportPolicy()
        )
        return _empty_result(
            safe_policy, now, "BLOCKED", policy_errors + input_errors,
            policy=not policy_errors, input=not input_errors,
        )
    if not isinstance(source, SandboxVerificationAuditCertificateLedgerResult):
        return _empty_result(
            policy, now, "FAILED",
            ["Source는 V15.9 Verification Certificate Ledger Result여야 합니다."],
    ...
    if not (
        ...
        source_errors.append("정상 V15.9 Certificate Ledger Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V15.9 Source 실행 안전장치 오류입니다.")
    if source_errors:
        return _empty_result(
    ...
    if len(operators) != 1 or operator not in operators:
        validation_errors.append("Operator가 V15.9 Certificate Ledger와 다릅니다.")
    try:
    ...
    summaries = tuple(
        SandboxVerificationCertificateSummary(
    ...
    return SandboxVerificationFinalReportResult(
        ...
        result_status_label="Sandbox Verification Final Report 생성 완료",
        final_verification_status="SANDBOX_VERIFICATION_COMPLETE",
        operator=operator,
        source_ledger_result_id=source.ledger_result_id,
        source_latest_entry_id=latest.ledger_entry_id,
        ...
        report=report,
        reasons=[
            f"V15.9 Certificate Ledger Entry {len(entries)}개를 검증했습니다.",
            "Certificate Ledger Hash Chain과 Source Linkage가 유효합니다.",
            "최종 상태는 SANDBOX_VERIFICATION_COMPLETE입니다.",
        ],
        warnings=[
            "V16.0 Final Report는 실행 또는 주문 권한을 부여하지 않습니다.",
            "V15.9 Certificate Ledger는 변경되지 않았습니다.",
            "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
        ],
        next_actions=[
...
def save_final_report_result(
    result: SandboxVerificationFinalReportResult,
    output_directory: Path = OUTPUT_DIRECTORY,
) -> tuple[Path, Path]:
    output_directory.mkdir(parents=True, exist_ok=True)
    stamp = result.created_at.replace(":", "").replace("-", "").replace("+", "_")
    report_path = output_directory / f"sandbox_verification_final_report_{stamp}.json"
    latest_path = output_directory / "latest_sandbox_verification_final_report.json"
