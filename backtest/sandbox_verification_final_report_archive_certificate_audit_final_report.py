...
from backtest.sandbox_verification_final_report_archive_certificate_audit_certificate_ledger import (
...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "sandbox_verification_final_report_archive_certificate_audit_final_report"
)
REQUIRED_FINAL_REPORT_TEXT = (
    "GENERATE IN MEMORY SANDBOX VERIFICATION FINAL REPORT ARCHIVE CERTIFICATE AUDIT FINAL REPORT"
)


@dataclass(frozen=True)
class SandboxVerificationFinalReportArchiveCertificateAuditFinalReportPolicy:
    required_source_version: str = "V16.9"
...
@dataclass(frozen=True)
class SandboxVerificationFinalReportArchiveCertificateAuditCertificateSummary:
    sequence: int
    ledger_entry_id: str
    recorded_at: str
...
@dataclass(frozen=True)
class SandboxVerificationFinalReportArchiveCertificateAuditFinalReport:
    final_report_id: str
    finalized_at: str
    report_status: str
    ...
    certificate_summaries: tuple[
        SandboxVerificationFinalReportArchiveCertificateAuditCertificateSummary,
        ...,
    ]
    report_hash: str
...
@dataclass
class SandboxVerificationFinalReportArchiveCertificateAuditFinalReportResult:
    version: str
    created_at: str
    ...
    live_execution_authorized: bool
    report_policy: SandboxVerificationFinalReportArchiveCertificateAuditFinalReportPolicy
    report: SandboxVerificationFinalReportArchiveCertificateAuditFinalReport | None
...
def validate_policy(
    policy: SandboxVerificationFinalReportArchiveCertificateAuditFinalReportPolicy,
) -> list[str]:
    if not isinstance(policy, SandboxVerificationFinalReportArchiveCertificateAuditFinalReportPolicy):
        return ["Sandbox Verification Final Report Policy 형식 오류입니다."]
    errors: list[str] = []
    expected = {
        "required_source_version": "V16.9",
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V17.0 기준과 다릅니다.")
    for name in (
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V17.0에서 True여야 합니다.")
    return errors


def verify_final_report(
    report: SandboxVerificationFinalReportArchiveCertificateAuditFinalReport,
) -> tuple[bool, list[str]]:
    if not isinstance(report, SandboxVerificationFinalReportArchiveCertificateAuditFinalReport):
...
def _empty_result(
    ...
    **checks: bool,
    ...
    return SandboxVerificationFinalReportArchiveCertificateAuditFinalReportResult(
        version="V17.0",
        ...
        reasons=reasons,
        warnings=[
            "V17.0은 In-Memory Sandbox Verification 최종 보고서만 생성합니다.",
            "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
        ],
        next_actions=["입력과 V16.9 Certificate Ledger를 수동 확인합니다."],
    )


def generate_sandbox_verification_final_report_archive_certificate_audit_final_report(
    source: Any,
    operator: str,
    confirmation_text: str,
    policy: SandboxVerificationFinalReportArchiveCertificateAuditFinalReportPolicy | None = None,
    now: datetime | None = None,
) -> SandboxVerificationFinalReportArchiveCertificateAuditFinalReportResult:
    policy = policy or SandboxVerificationFinalReportArchiveCertificateAuditFinalReportPolicy()
    ...
    if policy_errors or input_errors:
        safe_policy = (
            policy
            if isinstance(policy, SandboxVerificationFinalReportArchiveCertificateAuditFinalReportPolicy)
            else SandboxVerificationFinalReportArchiveCertificateAuditFinalReportPolicy()
        )
    ...
    if not isinstance(
        ...
        return _empty_result(
            policy, now, "FAILED",
            ["Source는 V16.9 Verification Certificate Ledger Result여야 합니다."],
    ...
    if not (
        ...
        source_errors.append("정상 V16.9 Certificate Ledger Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V16.9 Source 실행 안전장치 오류입니다.")
    if source_errors:
        return _empty_result(
    ...
    if len(operators) != 1 or operator not in operators:
        validation_errors.append("Operator가 V16.9 Certificate Ledger와 다릅니다.")
    try:
    ...
    summaries = tuple(
        SandboxVerificationFinalReportArchiveCertificateAuditCertificateSummary(
            ...
            source_certificate_audit_ledger_result_id=(
                entry.source_certificate_audit_ledger_result_id
            ),
            latest_certificate_audit_ledger_entry_id=(
                entry.latest_certificate_audit_ledger_entry_id
            ),
            latest_certificate_audit_ledger_entry_hash=(
                entry.latest_certificate_audit_ledger_entry_hash
            ),
    ...
    report_payload = {
        ...
        "latest_previous_final_report_hash": latest.latest_final_report_hash,
        "latest_certificate_ledger_snapshot_hash": (
            latest.latest_certificate_ledger_snapshot_hash
        ),
    ...
    return SandboxVerificationFinalReportArchiveCertificateAuditFinalReportResult(
        ...
        report=report,
        reasons=[
            f"V16.9 Certificate Ledger Entry {len(entries)}개를 검증했습니다.",
            "Certificate Ledger Hash Chain과 Source Linkage가 유효합니다.",
            "최종 상태는 SANDBOX_VERIFICATION_COMPLETE입니다.",
        ],
        warnings=[
            "V17.0 Final Report는 실행 또는 주문 권한을 부여하지 않습니다.",
            "V16.9 Certificate Ledger는 변경되지 않았습니다.",
            "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
        ],
        next_actions=[
...
def save_final_report_result(
    result: SandboxVerificationFinalReportArchiveCertificateAuditFinalReportResult,
    output_directory: Path = OUTPUT_DIRECTORY,
) -> tuple[Path, Path]:
    output_directory.mkdir(parents=True, exist_ok=True)
    stamp = result.created_at.replace(":", "").replace("-", "").replace("+", "_")
    report_path = output_directory / f"sandbox_verification_final_report_archive_certificate_audit_final_report_{stamp}.json"
    latest_path = output_directory / "latest_sandbox_verification_final_report_archive_certificate_audit_final_report.json"
