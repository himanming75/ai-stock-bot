...
from backtest.sandbox_verification_ledger_integrity_audit import (
...
OUTPUT_DIRECTORY = (
    ...
    / "output"
    / "trading_engine"
    / "sandbox_verification_audit_ledger"
)
REQUIRED_LEDGER_TEXT = (
    "RECORD IN MEMORY SANDBOX VERIFICATION AUDIT"
)
GENESIS_HASH = "0" * 64


@dataclass(frozen=True)
class SandboxVerificationAuditLedgerPolicy:
    required_source_version: str = "V15.6"
...
@dataclass(frozen=True)
class SandboxVerificationAuditLedgerEntry:
    ledger_entry_id: str
    sequence: int
    recorded_at: str
...
@dataclass
class SandboxVerificationAuditLedgerResult:
    version: str
    created_at: str
    ...
    live_execution_authorized: bool
    ledger_policy: SandboxVerificationAuditLedgerPolicy
...
def validate_policy(policy: SandboxVerificationAuditLedgerPolicy) -> list[str]:
    if not isinstance(policy, SandboxVerificationAuditLedgerPolicy):
        return ["Verification Audit Ledger Policy 형식 오류입니다."]
    errors: list[str] = []
    expected = {
        "required_source_version": "V15.6",
    ...
    for name, value in expected.items():
        if getattr(policy, name) != value:
            errors.append(f"{name} 값이 V15.7 기준과 다릅니다.")
    if policy.maximum_ledger_entries <= 0:
        errors.append("Verification Audit Ledger 보관 한도는 0보다 커야 합니다.")
    for name in (
        "require_operator",
        ...
        if getattr(policy, name) is not True:
            errors.append(f"{name}는 V15.7에서 True여야 합니다.")
    return errors


def normalize_entries(
    existing: Any,
) -> tuple[SandboxVerificationAuditLedgerEntry, ...]:
    if existing is None:
        return ()
    if not isinstance(existing, (tuple, list)):
        raise TypeError("Existing Verification Audit Ledger는 tuple/list여야 합니다.")
    if not all(
        isinstance(entry, SandboxVerificationAuditLedgerEntry)
        for entry in existing
    ):
        raise TypeError("Existing Verification Audit Ledger Entry 형식 오류입니다.")
    return tuple(existing)


def verify_verification_audit_ledger_chain(
    entries: tuple[SandboxVerificationAuditLedgerEntry, ...],
) -> tuple[bool, list[str]]:
    errors: list[str] = []
    ...
    for expected_sequence, entry in enumerate(entries, start=1):
        ...
        try:
            ...
            if previous_time and recorded_at < previous_time:
                errors.append("Verification Audit Ledger 시간이 역순입니다.")
            previous_time = recorded_at
        except (TypeError, ValueError):
            errors.append("Verification Audit Ledger 시간이 올바르지 않습니다.")
        if entry.audit_status != "PASSED":
            errors.append("Verification Audit Status가 PASSED가 아닙니다.")
        if any(
            (
            ...
            errors.append("Verification Audit Ledger Entry 안전장치 오류입니다.")
        previous_hash = entry.entry_hash
    return not errors, errors


def _safe_source(
    source: SandboxVerificationLedgerIntegrityAuditResult,
) -> bool:
    return not any(
        (
...
def _result(
    ...
    **checks: bool,
    ...
    return SandboxVerificationAuditLedgerResult(
        version="V15.7",
        created_at=now.isoformat(),
        ledger_result_id=str(uuid.uuid4()),
        result_status=status,
        result_status_label=(
            "Sandbox Verification Audit Ledger 기록 완료"
            if status == "RECORDED_IN_MEMORY"
            else "Verification Audit Ledger 차단"
            if status == "BLOCKED"
            else "Verification Audit Ledger 실패"
        ),
        ...
        reasons=reasons,
        warnings=[
            "V15.7은 V15.6 Audit Certificate를 메모리에 기록만 합니다.",
            "Broker API, 계좌, Network, 실제 주문 및 Live Execution은 차단됩니다.",
        ],
        next_actions=[
            "Verification Audit Ledger Hash Chain을 확인합니다.",
            "Audit Certificate와 Snapshot Hash 연결을 수동 검토합니다.",
        ],
    )


def record_sandbox_verification_audit(
    source: Any,
    operator: str,
    confirmation_text: str,
    existing: Any = None,
    policy: SandboxVerificationAuditLedgerPolicy | None = None,
    now: datetime | None = None,
) -> SandboxVerificationAuditLedgerResult:
    policy = policy or SandboxVerificationAuditLedgerPolicy()
    ...
    if policy_errors or input_errors:
        safe_policy = (
            policy
            if isinstance(policy, SandboxVerificationAuditLedgerPolicy)
            else SandboxVerificationAuditLedgerPolicy()
        )
        return _result(
    ...
    if not isinstance(source, SandboxVerificationLedgerIntegrityAuditResult):
        return _result(
            policy, now, "FAILED", entries,
            ["Source는 V15.6 Verification Ledger Integrity Audit Result여야 합니다."],
    ...
    if not (
        ...
        source_errors.append("정상 V15.6 Verification Audit Source가 아닙니다.")
    if not _safe_source(source):
        source_errors.append("V15.6 Source 실행 안전장치 오류입니다.")
    ...
    try:
        if now < datetime.fromisoformat(certificate.audited_at) or (
            ...
            return _result(
                policy, now, "BLOCKED", entries,
                ["역순 Verification Audit Ledger 시간이 차단되었습니다."],
                policy=True, input=True, source=True, certificate=True,
    ...
    if trimmed_count:
        ...
        rebuilt: list[SandboxVerificationAuditLedgerEntry] = []
        previous_hash = GENESIS_HASH
        for sequence, item in enumerate(kept, start=1):
            ...
            item = SandboxVerificationAuditLedgerEntry(
                **item_payload, entry_hash=sha256_payload(item_payload)
            )
            rebuilt.append(item)
            previous_hash = item.entry_hash
        combined = tuple(rebuilt)
    valid, errors = verify_verification_audit_ledger_chain(combined)
    result = _result(
        policy, now, "RECORDED_IN_MEMORY" if valid else "FAILED",
        combined,
        [
            f"Verification Audit Certificate "
            f"{certificate.audit_certificate_id}를 기록했습니다.",
            f"현재 Verification Audit Ledger Entry는 {len(combined)}개입니다.",
        ] + errors,
        policy=True, input=True, source=True, certificate=True,
...
def save_verification_audit_ledger_result(
    result: SandboxVerificationAuditLedgerResult,
    output_directory: Path = OUTPUT_DIRECTORY,
) -> tuple[Path, Path]:
    output_directory.mkdir(parents=True, exist_ok=True)
    stamp = result.created_at.replace(":", "").replace("-", "").replace("+", "_")
    report = output_directory / f"verification_audit_ledger_{stamp}.json"
    latest = output_directory / "latest_verification_audit_ledger.json"
...
def load_verification_audit_ledger_result(path: Path) -> dict[str, Any]:
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("Verification Audit Ledger JSON은 object여야 합니다.")
    return payload
