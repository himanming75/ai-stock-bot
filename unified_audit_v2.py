...
def build(root):
    ...
    for x in validation_closed:
        events.append({
            ...
            "time":x.get("exit_time") or x.get("exit_time_utc") or x.get("generated_at_utc") or "",
            "symbol":x.get("symbol",""),
    ...
    for x in exits:
        events.append({
            ...
            "time":x.get("submitted_at_utc") or "",
            "symbol":x.get("symbol",""),
            "side":"SELL",
            "quantity":x.get("qty"),
            "realized_pl":None,
    ...
    report={
        "stage":"PAPER_UNIFIED_TRADE_AUDIT_DASHBOARD_V2",
        ...
        "reliability_health":reliability.get("health",{}),
        "watchdog":watchdog,
        "broker_snapshot":{
            "market_open":broker.get("market_open"),
            "position_count":broker.get("position_count",0),
            "position_symbols":broker.get("position_symbols",[]),
            "open_order_count":broker.get("open_order_count",0),
            "open_order_symbols":broker.get("open_order_symbols",[]),
        },
        "recent_timeline":events[:100],
    }
    out=rt/"paper_unified_audit_v2"
    out.mkdir(parents=True,exist_ok=True)
    (out/"latest_unified_trade_audit.json").write_text(
