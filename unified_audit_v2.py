...
def build(root):
    ...
    observer=read_json(rt/"paper_validation_transition_observer/latest_transition_observer.json")
    analytics=main_report(root)
    etrade_live=read_json(rt/"etrade_live_readiness_stage1/latest_etrade_live_readiness.json")
    baseline=analytics.get("validation_baseline",{})
    ...
    progress=dict(validation.get("progress",{}))
    progress["closed_trades"]=len(validation_closed)
    progress["closed_trade_target"]=300
    progress["closed_trade_target_progress_pct"]=round(min(100.0,len(validation_closed)/300*100),2)
    if observer:
        op=observer.get("validation_progress",{})
        progress["trading_days"]=op.get("trading_days",progress.get("trading_days",0))
        progress["trading_day_target"]=10
        progress["trading_day_target_progress_pct"]=op.get("trading_day_progress_pct",0)
    ...
    report={"stage":"PAPER_UNIFIED_TRADE_AUDIT_DASHBOARD_V2","status":"PASS","mode":"READ_ONLY",
            ...
            "validation_transition":observer,
            ...
            "ai_outcome_metrics":ai_link.get("metrics",{}),"ai_linked_trades":ai_link.get("rows",[]),
            "live_readiness":analytics.get("live_readiness",{}),"etrade_live_readiness":etrade_live,
