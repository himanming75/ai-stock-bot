...
from test_sandbox_verification_final_report_archive_certificate_audit_final_report import (
    create_source as create_final_source,
)
from test_sandbox_verification_final_report_archive_certificate_audit_final_report import finalize

from backtest.sandbox_verification_final_report_archive_certificate_audit_final_report_archive import (
    SandboxVerificationFinalReportArchiveCertificateAuditFinalReportArchivePolicy,
    archive_sandbox_verification_final_report_archive_certificate_audit_final_report,
    load_archive_result,
    save_archive_result,
...
def archive(
    ...
    text: str = "ARCHIVE IN MEMORY SANDBOX VERIFICATION FINAL REPORT ARCHIVE CERTIFICATE AUDIT FINAL REPORT",
    existing: Any = None,
    policy: Any = None,
    now: datetime = NOW,
) -> Any:
...
def main() -> None:
    ...
    require(
        first.entries[0].previous_final_report_hash
        == source.latest_previous_final_report_hash,
        "Previous Final Report Hash 보존 실패",
    )
    require(
        first.entries[0].source_certificate_ledger_snapshot_hash
    ...
    require(source.to_dict() == source_before, "V17.0 Source 변경 감지")
    require_safe(first)
    ...
    unsafe_policy = SandboxVerificationFinalReportArchiveCertificateAuditFinalReportArchivePolicy(
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        require(payload["version"] == "V17.1", "저장 Version 오류")
    ...
    checks = {
        "Version is V17.1": second.version == "V17.1",
        ...
        "V17.0 final report source passed": second.source_checks_passed,
        "V17.0 final report hash passed": second.report_hash_checks_passed,
        ...
        "Result save and load passed": payload["version"] == "V17.1",
    ...
    print("=" * 108)
    print(
        "AI STOCK BOT V17.1 SANDBOX VERIFICATION FINAL REPORT ARCHIVE "
        "CERTIFICATE AUDIT FINAL REPORT ARCHIVE TEST"
    )
    print("=" * 108)
    print("V17.1 VALIDATION CHECKS")
    print("-" * 108)
    for name, passed in checks.items():
        print(f"{name:<74}: {passed}")
    print("=" * 108)
    require(checks["All checks passed"], "V17.1 Validation Check 실패")
    print()
    print(
        "V17.1 sandbox verification final report archive certificate audit "
        "final report archive test completed successfully."
    )
    print(
        "V17.0 Final Report Hash, Verification Ledger Snapshot Hash, 이전 "
        "Final Report Hash와 Certificate Ledger Snapshot Hash 보존, "
