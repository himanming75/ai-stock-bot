...
from test_sandbox_verification_final_report_archive_audit_ledger_integrity_verification import (
...
from backtest.sandbox_verification_final_report_archive_verification_certificate_ledger import (
    ...
    record_sandbox_verification_final_report_archive_certificate,
    save_certificate_ledger_result,
    verify_certificate_ledger_chain,
)


NOW = datetime(2026, 7, 28, 19, 50, tzinfo=timezone.utc)
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        require(payload["version"] == "V16.5", "저장 Version 오류")
    ...
    checks = {
        "Version is V16.5": second.version == "V16.5",
        ...
        "V16.4 verification source passed": second.source_checks_passed,
        ...
        "Result save and load passed": payload["version"] == "V16.5",
    ...
    print("=" * 108)
    print(
        "AI STOCK BOT V16.5 SANDBOX VERIFICATION FINAL REPORT ARCHIVE "
        "VERIFICATION CERTIFICATE LEDGER TEST"
    )
    print("=" * 108)
    print("V16.5 VALIDATION CHECKS")
    print("-" * 108)
    for name, passed in checks.items():
        print(f"{name:<74}: {passed}")
    print("=" * 108)
    require(checks["All checks passed"], "V16.5 Validation Check 실패")
    print()
    print(
        "V16.5 sandbox verification final report archive verification "
        "certificate ledger test completed successfully."
    )
    print(
        "V16.4 Certificate Hash, Final Report Hash, Certificate Ledger "
