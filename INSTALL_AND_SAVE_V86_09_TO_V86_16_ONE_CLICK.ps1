...
Write-Host "=== V86.09-V86.16 ONE-CLICK INSTALL ==="
...
git add `
  ...
  tools/install_check_v86_09_to_v86_16.py `
  tools/verify_indicator_engine_v86_09_to_v86_16.py `
  RUN_V86_09_TO_V86_16_INDICATOR_ENGINE.ps1 `
  RUN_V86_09_TO_V86_16_TEST_AND_VERIFY.ps1 `
  INSTALL_AND_SAVE_V86_09_TO_V86_16_ONE_CLICK.ps1 `
  release/v86_09_to_v86_16 `
  V86_09_TO_V86_16_MANIFEST.json `
  GIT_COMMIT_V86_09_TO_V86_16.txt

$Staged = git diff --cached --name-only
if ($Staged) {
    git commit -m "V86.09-V86.16 indicator engine integrated"
    if ($LASTEXITCODE -ne 0) { throw "COMMIT FAILED" }
}
else {
    Write-Host "No new V86.09-V86.16 changes to commit."
}
...
git log -1 --oneline
Write-Host "V86.09-V86.16 ONE-CLICK COMPLETE"
