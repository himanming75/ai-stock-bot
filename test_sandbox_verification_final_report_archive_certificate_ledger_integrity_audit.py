...
from test_sandbox_verification_final_report_archive_verification_certificate_ledger import (
...
from backtest.sandbox_verification_final_report_archive_certificate_ledger_integrity_audit import (
    ...
    audit_sandbox_verification_final_report_archive_certificate_ledger,
    load_audit_result,
    save_audit_result,
...
def create_source() -> Any:
    ...
    return record(
        ...
        now=datetime(2026, 7, 28, 19, 51, tzinfo=timezone.utc),
    )
...
def main() -> None:
    ...
    require(source.to_dict() == source_before, "V16.5 Source 변경")
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        require(payload["version"] == "V16.6", "저장 Version 오류")
    ...
    checks = {
        "Version is V16.6": result.version == "V16.6",
        ...
        "V16.5 certificate ledger source passed": result.source_checks_passed,
        ...
        "Result save and load passed": payload["version"] == "V16.6",
    ...
    print("=" * 108)
    print(
        "AI STOCK BOT V16.6 SANDBOX VERIFICATION FINAL REPORT ARCHIVE "
        "CERTIFICATE LEDGER INTEGRITY AUDIT TEST"
    )
    print("=" * 108)
    print("V16.6 VALIDATION CHECKS")
    print("-" * 108)
    for name, passed in checks.items():
        print(f"{name:<74}: {passed}")
    print("=" * 108)
    require(checks["All checks passed"], "V16.6 Validation Check 실패")
    print()
    print(
        "V16.6 sandbox verification final report archive certificate "
        "ledger integrity audit test completed successfully."
    )
    print(
        "V16.5 Ledger Hash Chain, Entry Findings, Snapshot Hash, 읽기 전용 "
