...
from test_sandbox_verification_final_report_v19_0 import (
    create_source as create_final_source,
)
from test_sandbox_verification_final_report_v19_0 import finalize

from backtest.sandbox_final_report_archive_v19_1 import (
    SandboxFinalReportArchiveV191Policy,
    archive_sandbox_final_report_v19_1,
    load_archive_result,
    save_archive_result,
...
NOW = datetime(2026, 7, 29, 7, 30, tzinfo=timezone.utc)
...
def main() -> None:
    ...
    require(
        first.entries[0].carried_final_report_hash
        == source.latest_final_report_hash,
        "Carried Final Report Hash 보존 실패",
    )
    require(
        first.entries[0].linked_final_report_hash
    ...
    require(
        ...
        "Latest Certificate Ledger Snapshot Hash 보존 실패",
    )
    require(
        first.entries[0].archived_certificate_ledger_snapshot_hash
    ...
    require(source.to_dict() == source_before, "V19.0 Source 변경 감지")
    require_safe(first)
    ...
    unsafe_policy = SandboxFinalReportArchiveV191Policy(
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        require(payload["version"] == "V19.1", "저장 Version 오류")
    ...
    checks = {
        "Version is V19.1": second.version == "V19.1",
        ...
        "V19.0 final report source passed": second.source_checks_passed,
        "V19.0 final report hash passed": second.report_hash_checks_passed,
        ...
        "Latest certificate ledger snapshot hash was preserved": (
            first.entries[0].latest_certificate_ledger_snapshot_hash
            == source.latest_certificate_ledger_snapshot_hash
        ),
        ...
        "Result save and load passed": payload["version"] == "V19.1",
    ...
    print("=" * 108)
    print(
        "AI STOCK BOT V19.1 SANDBOX FINAL REPORT ARCHIVE TEST"
    )
    print("=" * 108)
    print("V19.1 VALIDATION CHECKS")
    print("-" * 108)
    for name, passed in checks.items():
        print(f"{name:<74}: {passed}")
    print("=" * 108)
    require(checks["All checks passed"], "V19.1 Validation Check 실패")
    print()
    print(
        "V19.1 sandbox final report archive test completed successfully."
    )
    print(
        "V19.0 Final Report Hash와 모든 연결 Snapshot Hash 보존, Archive "
        "Sequence, SHA-256 Hash Chain 및 중복·역순·변조 차단이 "
        "검증되었습니다."
    )
    print(
