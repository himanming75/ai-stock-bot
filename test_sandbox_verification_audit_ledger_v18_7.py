...
from test_sandbox_verification_certificate_ledger_audit_v18_6 import (
...
from backtest.sandbox_verification_audit_ledger_v18_7 import (
    SandboxVerificationAuditLedgerV187Policy,
    load_sandbox_verification_audit_ledger_v18_7,
    record_sandbox_verification_audit_v18_7,
    save_sandbox_verification_audit_ledger_v18_7,
    verify_sandbox_verification_audit_ledger_v18_7,
)


NOW = datetime(2026, 7, 29, 5, 40, tzinfo=timezone.utc)
...
def main() -> None:
    ...
    require(
        ...
        "V18.6 Audit Certificate Hash 보존 실패",
    )
    require(
    ...
    valid_chain, chain_errors = verify_sandbox_verification_audit_ledger_v18_7(
        second.entries
    )
    ...
    changed_valid, changed_errors = verify_sandbox_verification_audit_ledger_v18_7(
        changed_entries
    )
    ...
    with tempfile.TemporaryDirectory() as directory:
        report_path, latest_path = save_sandbox_verification_audit_ledger_v18_7(
            second, Path(directory)
        )
        require(report_path.exists() and latest_path.exists(), "저장 실패")
        payload = load_sandbox_verification_audit_ledger_v18_7(latest_path)
        require(payload["version"] == "V18.7", "저장 Version 오류")
    ...
    checks = {
        "Version is V18.7": second.version == "V18.7",
        ...
        "V18.6 audit source passed": second.source_checks_passed,
        ...
        "V18.6 audit certificate hash was preserved": (
            first.entries[0].audit_certificate_hash
            == first_source.audit_certificate_hash
        ),
        ...
        "Result save and load passed": payload["version"] == "V18.7",
    ...
    print("=" * 108)
    print(
        "AI STOCK BOT V18.7 SANDBOX VERIFICATION AUDIT LEDGER TEST"
    )
    print("=" * 108)
    print("V18.7 VALIDATION CHECKS")
    print("-" * 108)
    for name, passed in checks.items():
        print(f"{name:<74}: {passed}")
    print("=" * 108)
    require(checks["All checks passed"], "V18.7 Validation Check 실패")
    print()
    print(
        "V18.7 sandbox verification audit ledger "
        "test completed successfully."
    )
    print(
        "V18.6 Audit Certificate Hash와 연결 Hash 보존, Ledger Sequence "
