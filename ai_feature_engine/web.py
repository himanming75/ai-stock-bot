...
def serve(*, report_path: Path, certification_path: Path, host="127.0.0.1", port=8772):
    Handler.report_path = report_path
    Handler.certification_path = certification_path
    server = ThreadingHTTPServer((host, port), Handler)
    print(f"AI Feature & Signal Dashboard / AI 특징 및 신호 대시보드: http://{host}:{port}")
    print("Signal Candidates Only / 신호 후보 전용")
