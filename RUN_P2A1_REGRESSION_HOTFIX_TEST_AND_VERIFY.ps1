...
try {
    ...
    Write-Host "=== P2A1 REGRESSION COMPATIBILITY TEST ==="
    & $Python -m unittest tools.test_p2_actual_paper_execution -v
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

    Write-Host "=== P2A SAFETY TEST ==="
    ...
    Write-Host "=== P2A1 SIGNATURE VERIFY ==="
    & $Python (Join-Path $Root "tools\verify_p2a1_hotfix.py")
    exit $LASTEXITCODE
}
finally {
