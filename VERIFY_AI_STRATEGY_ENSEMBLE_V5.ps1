...
& $Python -m unittest `
  tools.test_ai_strategy_ensemble_v5 `
  -v

if($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

$Text = Get-Content .\ai_strategy_ensemble_v5\service.py -Raw
$ForbiddenCalls = @(
  '\.\s*submit_order\s*\(',
  '\.\s*cancel_order\s*\(',
  '\.\s*replace_order\s*\(',
...
Write-Host "ENSEMBLE OBSERVATION ONLY: PASS"
Write-Host "AUTOMATIC WEIGHT CHANGES: OFF"
