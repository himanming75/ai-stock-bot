...
from test_sandbox_verification_final_report_archive_certificate_audit_final_report_archive import archive
from test_sandbox_verification_final_report_archive_certificate_audit_final_report_archive import (
    create_source as create_final_report_archive_source,
)
...
from backtest.sandbox_verification_final_report_archive_certificate_audit_final_report_archive_integrity_audit import (
    ...
    audit_sandbox_verification_final_report_archive_certificate_audit_final_report_archive_integrity,
    load_audit_result,
    save_audit_result,
...
def create_source() -> Any:
    first = archive(create_final_report_archive_source())
    return archive(
        create_final_report_archive_source(),
        existing=first.entries,
        now=datetime(2026, 7, 28, 23, 1, tzinfo=timezone.utc),
    )
...
def main() -> None:
    ...
    require(source.to_dict() == source_before, "V17.1 Source 변경")
    require(
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        require(payload["version"] == "V17.2", "저장 Version 오류")
        require(
    ...
    checks = {
        "Version is V17.2": result.version == "V17.2",
        ...
        "V17.1 archive source passed": result.source_checks_passed,
        ...
        "Result save and load passed": payload["version"] == "V17.2",
    ...
    print("=" * 108)
    print(
        "AI STOCK BOT V17.2 SANDBOX VERIFICATION FINAL REPORT "
        "ARCHIVE CERTIFICATE AUDIT FINAL REPORT ARCHIVE "
        "INTEGRITY AUDIT TEST"
    )
    print("=" * 108)
    print("V17.2 VALIDATION CHECKS")
    print("-" * 108)
    for name, passed in checks.items():
        print(f"{name:<74}: {passed}")
    print("=" * 108)
    require(checks["All checks passed"], "V17.2 Validation Check 실패")
    print()
    print(
        "V17.2 sandbox verification final report archive integrity "
        "audit test completed successfully."
    )
    print(
        "V17.1 Archive Chain, Entry Findings, 연결 Hash, Snapshot Hash, "
        "읽기 전용 감사 및 Audit Certificate Hash가 검증되었습니다."
    )
    print(
