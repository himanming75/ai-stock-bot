...
from test_sandbox_archive_audit_ledger_v18_3 import (
...
from backtest.sandbox_archive_audit_ledger_integrity_v18_4 import (
    ...
    verify_sandbox_archive_audit_ledger_v18_4,
    verify_verification_certificate,
)


NOW = datetime(2026, 7, 29, 4, 40, tzinfo=timezone.utc)
...
def create_source() -> Any:
    ...
    return record(
        ...
        now=datetime(2026, 7, 29, 4, 21, tzinfo=timezone.utc),
    )
...
def main() -> None:
    ...
    require(source.to_dict() == source_before, "V18.3 Source 변경")
    require(
        all(
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        require(payload["version"] == "V18.4", "저장 Version 오류")
        require(
    ...
    checks = {
        "Version is V18.4": result.version == "V18.4",
        ...
        "V18.3 audit ledger source passed": result.source_checks_passed,
        ...
        "Result save and load passed": payload["version"] == "V18.4",
    ...
    print("=" * 108)
    print(
        "AI STOCK BOT V18.4 SANDBOX ARCHIVE AUDIT LEDGER "
        "INTEGRITY VERIFICATION TEST"
    )
    print("=" * 108)
    print("V18.4 VALIDATION CHECKS")
    print("-" * 108)
    for name, passed in checks.items():
        print(f"{name:<74}: {passed}")
    print("=" * 108)
    require(checks["All checks passed"], "V18.4 Validation Check 실패")
    print()
    print(
        "V18.4 sandbox archive audit ledger integrity verification "
        "test completed successfully."
    )
    print(
        "V18.3 Ledger Chain, Entry Findings, 연결 Hash, Snapshot Hash, "
