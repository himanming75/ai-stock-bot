...
from test_sandbox_verification_audit_certificate_ledger import (
    create_source as create_certificate_source,
)
from test_sandbox_verification_audit_certificate_ledger import record

from backtest.sandbox_verification_final_report import (
    SandboxVerificationFinalReportPolicy,
    generate_sandbox_verification_final_report,
...
def create_source() -> Any:
    ...
    return record(
        ...
        now=datetime(2026, 7, 28, 17, 11, tzinfo=timezone.utc),
    )
...
def finalize(
    ...
    text: str = "GENERATE IN MEMORY SANDBOX VERIFICATION FINAL REPORT",
    policy: Any = None,
    now: datetime = NOW,
) -> Any:
    return silent(
...
def main() -> None:
    ...
    require(source.to_dict() == source_before, "V15.9 Source 변경 감지")
    ...
    unsafe_policy = SandboxVerificationFinalReportPolicy(
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        require(payload["version"] == "V16.0", "저장 Version 오류")
        require(
    ...
    checks = {
        "Version is V16.0": result.version == "V16.0",
        "Default policy is valid": result.policy_checks_passed,
        "Policy is immutable": (
            SandboxVerificationFinalReportPolicy.__dataclass_params__.frozen
        ),
        "Invalid policies are blocked": not unsafe.all_checks_passed,
        "V15.9 certificate ledger source passed": result.source_checks_passed,
        ...
        "Result save and load passed": payload["version"] == "V16.0",
    ...
    print("=" * 108)
    print("AI STOCK BOT V16.0 SANDBOX VERIFICATION FINAL REPORT TEST")
    print("=" * 108)
    print("V16.0 VALIDATION CHECKS")
    print("-" * 108)
    for name, passed in checks.items():
        print(f"{name:<74}: {passed}")
    print("=" * 108)
    require(checks["All checks passed"], "V16.0 Validation Check 실패")
    print()
    print(
        "V16.0 sandbox verification final report test "
        "completed successfully."
    )
    print(
        "V15.9 Certificate Ledger Chain, Snapshot Hash, 읽기 전용 Finalization "
        "및 Final Report Hash가 검증되었습니다."
    )
    print(
