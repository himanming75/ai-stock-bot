...
from backtest.offline_paper_audit_verify_cert_ledger_audit_v22_6 import (
    CONFIRMATION_TEXT,
    OfflinePaperAuditVerifyCertLedgerAuditV226Policy,
    audit_offline_paper_audit_verification_certificate_ledger_v22_6,
    load_result,
    save_result,
    verify_audit_certificate,
)
from test_offline_paper_audit_verify_cert_ledger_v22_5 import (
    NOW as V22_5_NOW,
    create_source as create_v22_4_source,
    record as record_v22_5,
)
...
def create_source():
    first = record_v22_5(
        create_v22_4_source(V22_5_NOW),
        now=V22_5_NOW + timedelta(minutes=1),
    )
    return record_v22_5(
        create_v22_4_source(V22_5_NOW + timedelta(minutes=2)),
        entries=first.entries,
        now=V22_5_NOW + timedelta(minutes=3),
    )


def audit(source, operator="paper-auditor", text=CONFIRMATION_TEXT, policy=None, now=NOW):
    return audit_offline_paper_audit_verification_certificate_ledger_v22_6(
...
def source_has_no_external_calls():
    path = Path(__file__).resolve().parent / "backtest" / (
        "offline_paper_audit_verify_cert_ledger_audit_v22_6.py"
    )
...
def main():
    ...
    result = audit(source)
    require(result.all_checks_passed, "V22.6 정상 감사 실패")
    require(source.to_dict() == before, "V22.5 Source 변경")
    valid_certificate, certificate_errors = verify_audit_certificate(result.certificate)
    require(valid_certificate and not certificate_errors, "V22.6 Certificate Hash 실패")
    ...
    wrong_policy = audit(
        source,
        policy=replace(policy, broker_api_disabled=False),
    )
    ...
    tampered_source.entries = (replace(source.entries[0], operator="attacker"), source.entries[1])
    tampered_ledger = audit(tampered_source)

    duplicate_source = copy.deepcopy(source)
    duplicate_second = rehash(replace(
        source.entries[1],
        source_verification_certificate_id=source.entries[0].source_verification_certificate_id,
    ))
    ...
    duplicate_source.ledger_snapshot_hash = sha256_payload(
        [entry.to_dict() for entry in duplicate_source.entries]
    )
    ...
    unsafe_source.ledger_snapshot_hash = sha256_payload(
        [entry.to_dict() for entry in unsafe_source.entries]
    )
    unsafe = audit(unsafe_source)

    tampered_certificate = replace(result.certificate, audit_status="FAILED")
    tampered_valid, _ = verify_audit_certificate(tampered_certificate)
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        save_load = (
            loaded["version"] == "V22.6"
            and latest_loaded["audit_certificate_hash"] == result.audit_certificate_hash
        )

    checks = {
        "Version is V22.6": result.version == "V22.6",
        ...
        "V22.5 ledger source passed": result.source_checks_passed,
        ...
        "V22.5 ledger remained unchanged": source.to_dict() == before,
        "Audit certificate hash passed": valid_certificate,
    ...
    print("=" * 78)
    print("AI STOCK BOT V22.6 OFFLINE PAPER AUDIT VERIFICATION CERTIFICATE LEDGER AUDIT TEST")
    print("=" * 78)
    print("V22.6 VALIDATION CHECKS")
    print("-" * 78)
    for label, passed in checks.items():
        print(f"{label:<54} : {passed}")
    print("=" * 78)
    require(all(checks.values()), "V22.6 Validation Check 실패")
    print()
    print("V22.6 offline paper audit verification certificate ledger audit test completed successfully.")
    print("V22.5 Ledger Chain, Snapshot Hash, Entry Findings 및 Audit Certificate Hash가 검증되었습니다.")
