...
from backtest.offline_paper_audit_cert_ledger_v22_7 import (
    CONFIRMATION_TEXT,
    OfflinePaperAuditCertLedgerV227Policy,
    load_result,
    record_offline_paper_audit_certificate_v22_7,
    save_result,
    sha256_payload,
    verify_ledger_chain,
)
from test_offline_paper_audit_verify_cert_ledger_audit_v22_6 import (
    NOW as V22_6_NOW,
    audit as audit_v22_6,
    create_source as create_v22_5_source,
)
...
def create_source(now=V22_6_NOW):
    return audit_v22_6(create_v22_5_source(), now=now)


def record(source, existing=None, operator="paper-auditor", text=CONFIRMATION_TEXT, policy=None, now=NOW):
    with contextlib.redirect_stdout(io.StringIO()):
        return record_offline_paper_audit_certificate_v22_7(
...
def source_has_no_external_calls():
    path = Path(__file__).resolve().parent / "backtest" / "offline_paper_audit_cert_ledger_v22_7.py"
...
def main():
    ...
    second_source = create_source(V22_6_NOW + timedelta(minutes=5))
    ...
    require(valid_chain and not chain_errors, "V22.7 Hash Chain 실패")
    ...
    broken_linkage.audit_certificate_hash = "f" * 64
    ...
    tampered_existing = record(
        create_source(V22_6_NOW + timedelta(minutes=10)),
        existing=changed,
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        save_load = loaded["version"] == "V22.7" and len(latest_loaded["entries"]) == 2

    checks = {
        "Version is V22.7": second.version == "V22.7",
        ...
        "Audit certificate hash was preserved": first.entries[0].source_audit_certificate_hash == first_source.audit_certificate_hash,
        "Source ledger snapshot hash was preserved": first.entries[0].source_verification_ledger_snapshot_hash == first_source.source_ledger_snapshot_hash,
    ...
    print("=" * 82)
    print("AI STOCK BOT V22.7 OFFLINE PAPER AUDIT CERTIFICATE LEDGER TEST")
    print("=" * 82)
    print("V22.7 VALIDATION CHECKS")
    print("-" * 82)
    for label, passed in checks.items():
        print(f"{label:<58} : {passed}")
    print("=" * 82)
    require(all(checks.values()), "V22.7 Validation Check 실패")
    print()
    print("V22.7 offline paper audit certificate ledger test completed successfully.")
    print("V22.6 Audit Certificate Hash, Source Ledger Snapshot Hash 및 SHA-256 Ledger Chain이 검증되었습니다.")
