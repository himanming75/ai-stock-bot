...
from backtest.offline_paper_verify_cert_audit_v22_2 import (
    OfflinePaperVerifyCertAuditV222Policy,
    ledger_snapshot_hash,
    load_audit_integrity_verification_result,
    save_audit_integrity_verification_result,
    verify_offline_paper_verification_certificate_ledger_v22_2,
    verify_verification_certificate,
)
from test_offline_paper_verify_cert_ledger_v22_1 import (
    NOW as V22_1_NOW,
    create_source as create_v22_0_source,
    record as record_v22_1,
)
NOW = datetime(2026, 7, 29, 14, 0, tzinfo=timezone.utc)
CONFIRMATION = "AUDIT OFFLINE PAPER VERIFICATION CERTIFICATE LEDGER V22.2"
...
def create_source() -> Any:
    first_source = create_v22_0_source()
    first = record_v22_1(first_source)
    second_source = create_v22_0_source(V22_1_NOW + timedelta(minutes=1))
    return record_v22_1(
        second_source,
        existing=first.entries,
        now=V22_1_NOW + timedelta(minutes=1),
    )
...
def source_has_no_external_execution_calls() -> bool:
    source_path = (
        ...
        / "backtest"
        / "offline_paper_verify_cert_audit_v22_2.py"
    )
...
def main() -> None:
    ...
    require(result.version == "V22.2", "Version 오류")
    ...
    require(
        ...
        "V22.0 Audit Verification Ledger Snapshot Hash 보존 실패",
    )
    require(
    ...
    require(
        ...
        "V21.9 Source Audit Verification Ledger Snapshot Hash 보존 실패",
    )
    require(
    ...
    require(
        ...
        "V22.2 Verification Certificate Hash 실패",
    )
    require_safe(result)
    ...
    try:
        ...
        immutable = True
    require(immutable, "V22.2 Policy가 immutable이 아닙니다.")
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        require(loaded["version"] == "V22.2", "저장 Version 오류")
        require(
    ...
    checks = {
        "Version is V22.2": result.version == "V22.2",
        ...
        "V22.1 ledger source passed": result.source_checks_passed,
        ...
        "V22.1 ledger remained unchanged": result.ledger_unchanged_checks_passed,
        ...
        "Result save and load passed": loaded["version"] == "V22.2",
    ...
    print("=" * 78)
    print("AI STOCK BOT V22.2 OFFLINE PAPER VERIFICATION LEDGER INTEGRITY AUDIT TEST")
    print("=" * 78)
    print("V22.2 VALIDATION CHECKS")
    print("-" * 78)
    for label, passed in checks.items():
        print(f"{label:<{width}} : {passed}")
    print("=" * 78)
    require(all(checks.values()), "V22.2 표시 검증 중 실패가 있습니다.")
    print()
    print("V22.2 offline paper verification ledger integrity audit test completed successfully.")
    print(
        "V22.1 Ledger Chain, Entry Findings, Snapshot Hash, 읽기 전용 검증 및 "
