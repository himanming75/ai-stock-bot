...
from backtest.sandbox_verification_final_report_archive_certificate_audit_final_report_archive_integrity_audit_ledger_integrity_verification_certificate_ledger_integrity_audit import (
    GENESIS_HASH,
    ...
    audit_sandbox_verification_final_report_archive_certificate_audit_final_report_archive_integrity_audit_ledger_integrity_verification_certificate_ledger,
    load_audit_result,
    save_audit_result,
...
NOW = datetime(2026, 7, 29, 0, 50, tzinfo=timezone.utc)
...
def create_entry(
    ...
    payload = {
        ...
        "latest_previous_final_report_hash": chr(ord(marker) + 5) * 64,
...
def create_source() -> Any:
    ...
    second = create_entry(
        2,
        datetime(2026, 7, 29, 0, 31, tzinfo=timezone.utc),
        first.entry_hash,
        "2",
    )
    ...
    return OptimizedLedgerSource(
        ...
        result_status_label="Optimized V17.5 test fixture",
...
def main() -> None:
    ...
    require(source.to_dict() == source_before, "V17.5 Source 변경")
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        require(payload["version"] == "V17.6", "저장 Version 오류")
    ...
    checks = {
        "Version is V17.6": result.version == "V17.6",
        ...
        "V17.5 certificate ledger source passed": result.source_checks_passed,
        ...
        "Result save and load passed": payload["version"] == "V17.6",
    ...
    print("=" * 108)
    print(
        "AI STOCK BOT V17.6 SANDBOX VERIFICATION FINAL REPORT ARCHIVE "
        "CERTIFICATE AUDIT FINAL REPORT ARCHIVE INTEGRITY AUDIT LEDGER "
        "INTEGRITY VERIFICATION CERTIFICATE LEDGER INTEGRITY AUDIT TEST"
    )
    print("=" * 108)
    print("V17.6 VALIDATION CHECKS")
    print("-" * 108)
    for name, passed in checks.items():
        print(f"{name:<74}: {passed}")
    print("=" * 108)
    require(checks["All checks passed"], "V17.6 Validation Check 실패")
    print()
    print(
        "V17.6 sandbox verification certificate ledger integrity audit "
        "test completed successfully."
    )
    print(
        "V17.5 Ledger Hash Chain, 연결 Hash, Entry Findings, 읽기 전용 "
