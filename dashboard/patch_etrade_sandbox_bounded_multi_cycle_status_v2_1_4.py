...
def main():
    ...
    if '"bounded_multi_cycle_v2_1_4": bounded_multi' in text:
        print("V2.1.4 STATUS ALREADY PRESENT")
        return 0
    ...
    if marker not in text:
        raise RuntimeError("V2.1.4 STATUS MARKER NOT FOUND")
    text=text.replace(marker,repl,1)

    marker2='        "contracts":{\n'
    repl2='        "bounded_multi_cycle_v2_1_4": bounded_multi,\n        "contracts":{\n'
    if marker2 not in text:
        raise RuntimeError("V2.1.4 STATUS RETURN MARKER NOT FOUND")
    text=text.replace(marker2,repl2,1)

    target.write_text(text,encoding="utf-8")
    print("V2.1.4 STATUS INTEGRATION: PASS")
    return 0
