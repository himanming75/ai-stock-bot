...
def main():
    ...
    if '"ai_signal_decision_v2_1_5": ai_signal_decision' in text:
        print("V2.1.5 STATUS ALREADY PRESENT")
        return 0
    ...
    if marker not in text:
        raise RuntimeError("V2.1.5 STATUS MARKER NOT FOUND")
    text=text.replace(marker,repl,1)

    marker2='        "contracts":{\n'
    repl2='        "ai_signal_decision_v2_1_5": ai_signal_decision,\n        "contracts":{\n'
    if marker2 not in text:
        raise RuntimeError("V2.1.5 STATUS RETURN MARKER NOT FOUND")
    text=text.replace(marker2,repl2,1)

    target.write_text(text,encoding="utf-8")
    print("V2.1.5 STATUS INTEGRATION: PASS")
    return 0
