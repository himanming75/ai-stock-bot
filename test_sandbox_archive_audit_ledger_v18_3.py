...
from test_sandbox_archive_integrity_audit_v18_2 import (
    audit,
)
from test_sandbox_archive_integrity_audit_v18_2 import (
...
from backtest.sandbox_archive_audit_ledger_v18_3 import (
    SandboxArchiveAuditLedgerV183Policy,
    load_audit_ledger_result,
    record_sandbox_archive_audit_v18_3,
    save_audit_ledger_result,
    verify_audit_ledger_chain,
)


NOW = datetime(2026, 7, 29, 4, 20, tzinfo=timezone.utc)
...
def record(
    ...
    text: str = (
        "RECORD IN MEMORY SANDBOX ARCHIVE INTEGRITY AUDIT V18.2"
    ),
    existing: Any = None,
    policy: Any = None,
    now: datetime = NOW,
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        require(payload["version"] == "V18.3", "저장 Version 오류")
    ...
    checks = {
        "Version is V18.3": second.version == "V18.3",
        ...
        "V18.2 audit source passed": second.source_checks_passed,
        ...
        "Result save and load passed": payload["version"] == "V18.3",
    ...
    print("=" * 108)
    print(
        "AI STOCK BOT V18.3 SANDBOX ARCHIVE AUDIT LEDGER TEST"
    )
    print("=" * 108)
    print("V18.3 VALIDATION CHECKS")
    print("-" * 108)
    for name, passed in checks.items():
        print(f"{name:<74}: {passed}")
    print("=" * 108)
    require(checks["All checks passed"], "V18.3 Validation Check 실패")
    print()
    print(
        "V18.3 sandbox archive audit ledger test completed successfully."
    )
    print(
        "V18.2 Audit Certificate Hash, Final Report Hash, 연결 Hash, "
