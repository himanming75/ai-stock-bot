...
from .canonical_paper_gate_semantics_v2_1_19_1 import (
...
def _extract_reward_risk(signal):
    for key in (
        "source_reward_risk",
        "reward_risk",
...
def qualify_evidence_row_v2_1_17(row):
    ...
    for idx,signal in enumerate(signals,1):
        ...
        try:
            quantity=_as_decimal(
                signal.get("quantity"),
                f"signal {idx} quantity",
            )
            if quantity<=0:
                reasons.append(
                    f"SIGNAL_{idx}_NONPOSITIVE_QUANTITY"
                )
        except ValueError:
            quantity=Decimal("0")
        ...
        try:
            ...
            if confidence>Decimal("1"):
                reasons.append(
                    f"SIGNAL_{idx}_CONFIDENCE_ABOVE_1"
                )
        except ValueError:
            confidence=None
            reasons.append(
                f"SIGNAL_{idx}_INVALID_CONFIDENCE"
            )

        rr_raw=_extract_reward_risk(signal)
        if rr_raw is None:
            reward_risk=None
            reasons.append(
                f"SIGNAL_{idx}_CANONICAL_REWARD_RISK_MISSING"
            )
        else:
            try:
                reward_risk=_as_decimal(
                    rr_raw,
                    f"signal {idx} reward_risk",
                )
                ...
                reward_risk=None
                reasons.append(
                    f"SIGNAL_{idx}_INVALID_REWARD_RISK"
                )
        ...
        if signal_key in seen_signal_keys:
            reasons.append(
                f"SIGNAL_{idx}_DUPLICATE_SIGNAL"
            )
        ...
        normalized_signals.append({
            ...
            "source_reward_risk":(
                None if reward_risk is None else str(reward_risk)
            ),
        })
    ...
    return {
        ...
        "generic_etrade_bridge_min_confidence":str(
            GENERIC_ETRADE_BRIDGE_MIN_CONFIDENCE
        ),
        "canonical_min_confidence":str(
            CANONICAL_PAPER_MIN_CONFIDENCE
        ),
        "canonical_min_reward_risk":str(
            CANONICAL_PAPER_MIN_REWARD_RISK
        ),
...
class EvidenceQualificationSandboxReadinessGateV2117:
    ...
    def _existing_keys(self):
        ...
        for line in self.qualification_ledger.read_text(
            ...
            if key and semantic=="CORRECTED_V2_1_19_1":
                keys.add(key)
        return keys
    ...
    def evaluate(self):
        ...
        source_rows=ready_rows=not_ready_rows=0
        new_rows=duplicate_rows=0
        latest=None
        ...
        with self.source_ledger.open(
            ...
            for line in src:
                ...
                result=qualify_evidence_row_v2_1_17(
                    evidence
                )
        ...
        return {
            ...
            "latest_qualification":(
                None
                if latest is None
                else str(self.latest_qualification)
            ),
            "manual_review_required":True,
            "automatic_sandbox_execution_allowed":False,
            "etrade_oauth_started":False,
