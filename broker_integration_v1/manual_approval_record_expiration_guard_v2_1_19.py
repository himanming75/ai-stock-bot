...
class ApprovalPolicyV2119:
    ...
    def validate(self):
        if self.expires_minutes<1:
            raise ValueError(
                "expires_minutes must be >= 1"
            )
        if self.expires_minutes>120:
            raise ValueError(
                "expires_minutes must be <= 120"
            )
        return self
...
class ManualApprovalRecordExpirationGuardV2119:
    def __init__(
        ...
        self.approval_ledger=(
            self.runtime_dir/"approval_ledger.jsonl"
        )
        self.latest_approval=(
            self.runtime_dir/"latest_approval.json"
        )
    ...
    def _validate_packet(packet,evidence_key):
        ...
        if packet.get("packet_status")!="AWAITING_MANUAL_REVIEW":
            reasons.append(
                "PACKET_NOT_AWAITING_MANUAL_REVIEW"
            )
        ...
        if packet.get("canonical_min_confidence")!="0.75":
            reasons.append(
                "CANONICAL_CONFIDENCE_SEMANTICS_NOT_CORRECTED"
            )
        if packet.get("canonical_min_reward_risk")!="1.0":
            reasons.append(
                "CANONICAL_REWARD_RISK_SEMANTICS_NOT_CORRECTED"
            )
        if packet.get("canonical_paper_gate_semantics")!="CORRECTED_V2_1_19_1":
            reasons.append(
                "CANONICAL_PAPER_SEMANTIC_MARKER_MISSING"
            )
        ...
        if packet.get("manual_approval_recorded") is not False:
            reasons.append(
                "PACKET_ALREADY_MARKED_APPROVED"
            )
        if packet.get(
            "automatic_sandbox_execution_allowed"
        ) is not False:
            reasons.append(
                "AUTO_SANDBOX_EXECUTION_NOT_LOCKED"
            )
        if int(
            packet.get("broker_orders_submitted") or 0
        )!=0:
            reasons.append(
                "PACKET_BROKER_ORDER_COUNT_NOT_ZERO"
            )
        if packet.get(
            "production_order_submission"
        ) is not False:
            reasons.append(
                "PACKET_PROD_NOT_LOCKED"
            )
        if packet.get("live_trading") is not False:
            reasons.append(
                "PACKET_LIVE_NOT_LOCKED"
            )

        signals=list(packet.get("signals") or [])
        if int(
            packet.get("eligible_signal_count") or 0
        )!=len(signals):
            reasons.append(
                "PACKET_SIGNAL_COUNT_MISMATCH"
            )
        if not signals:
        ...
        for idx,signal in enumerate(signals,1):
            try:
                ...
                confidence=None
                reasons.append(
                    f"SIGNAL_{idx}_CONFIDENCE_MISSING"
                )
            try:
                rr=float(
                    signal.get("source_reward_risk")
                )
            except Exception:
                rr=None
                reasons.append(
                    f"SIGNAL_{idx}_REWARD_RISK_MISSING"
                )

            if confidence is not None and confidence<0.75:
                reasons.append(
                    f"SIGNAL_{idx}_CONFIDENCE_BELOW_CANONICAL_PAPER"
                )
            if rr is not None and rr<1.0:
                reasons.append(
                    f"SIGNAL_{idx}_REWARD_RISK_BELOW_CANONICAL_PAPER"
                )
    ...
    def approve(
        ...
        if approval_phrase!=APPROVAL_PHRASE:
            return {
                "status":"NOT_APPROVED",
                "reason":
                    "EXPLICIT_APPROVAL_PHRASE_REQUIRED",
                "required_phrase":
                    APPROVAL_PHRASE,
        ...
        if existing is not None:
            return {
                "status":
                    "NOT_APPROVED_DUPLICATE",
                "reason":
                    "APPROVAL_ALREADY_RECORDED_FOR_EVIDENCE",
                "evidence_key":evidence_key,
        ...
        if not packet_path.exists():
            return {
                "status":
                    "WAITING_FOR_V2_1_18_REVIEW_PACKET",
                "evidence_key":evidence_key,
        ...
        if now.tzinfo is None:
            raise ValueError(
                "now_fn must return timezone-aware datetime"
            )
        now=now.astimezone(timezone.utc)
        expires=now+timedelta(
            minutes=self.policy.expires_minutes
        )
        fingerprint=canonical_packet_fingerprint(
            packet
        )
        ...
        record={
            ...
            "evidence_key":evidence_key,
            "approved_by":
                str(approved_by or "LOCAL_USER"),
            "approved_at_utc":now.isoformat(),
            "expires_at_utc":expires.isoformat(),
            "expires_minutes":
                self.policy.expires_minutes,
            "packet_path":str(packet_path),
        ...
        with self.approval_ledger.open(
            "a",encoding="utf-8"
        ) as f:
            f.write(
                json.dumps(
        ...
        return {
            "status":
                "PASS_MANUAL_APPROVAL_RECORDED",
            **record,
            "approval_ledger":
                str(self.approval_ledger),
            "latest_approval":
                str(self.latest_approval),
        }
    ...
    def validate_approval(
        ...
        if now.tzinfo is None:
            raise ValueError(
                "now timestamp must be timezone-aware"
            )
        now=now.astimezone(timezone.utc)
        expires=parse_utc(
            row["expires_at_utc"]
        ...
        if row.get(
            ...
            reasons.append(
                "LEGACY_APPROVAL_SEMANTICS_BLOCKED"
            )
        if now>=expires:
        ...
        if int(row.get("usage_count") or 0)!=0:
            reasons.append(
                "APPROVAL_USAGE_COUNT_NOT_ZERO"
            )
        if row.get("one_time_use") is not True:
            reasons.append(
                "ONE_TIME_USE_FLAG_NOT_TRUE"
            )
        if row.get(
            "automatic_sandbox_execution_allowed"
        ) is not False:
            reasons.append(
                "AUTO_EXECUTION_NOT_LOCKED"
            )

        packet_path=Path(
            row.get("packet_path") or ""
        )
        if not packet_path.exists():
            reasons.append(
            ...
            packet_reasons=self._validate_packet(
                packet,
                evidence_key,
            )
            reasons.extend(
                [
                    "PACKET_REVALIDATION_"+x
                    for x in packet_reasons
                ]
            )
            current=canonical_packet_fingerprint(
                packet
            )
            if current!=row.get(
                "packet_fingerprint_sha256"
        ...
        return {
            ...
            "reasons":reasons,
            "approval_consumed":
                row.get("approval_consumed"),
            "usage_count":
                row.get("usage_count"),
            "one_time_use":
                row.get("one_time_use"),
            "automatic_sandbox_execution_allowed":False,
            "etrade_oauth_started":False,
            "sandbox_preview_sent":False,
            "sandbox_place_sent":False,
