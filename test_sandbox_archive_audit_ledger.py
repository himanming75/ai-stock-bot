...
from test_sandbox_archive_integrity_audit import audit
from test_sandbox_archive_integrity_audit import (
    create_source as create_archive_source,
)

from backtest.sandbox_archive_audit_ledger import (
    SandboxArchiveAuditLedgerPolicy,
    load_audit_ledger_result,
    record_sandbox_archive_audit,
...
def record(
    ...
    text: str = "RECORD IN MEMORY SANDBOX ARCHIVE AUDIT",
    existing: Any = None,
    policy: Any = None,
    now: datetime = NOW,
) -> Any:
...
def main() -> None:
    ...
    valid_chain, chain_errors = verify_audit_ledger_chain(second.entries)
    ...
    unsafe_policy = SandboxArchiveAuditLedgerPolicy(
    ...
    changed_entry = replace(
        second.entries[0], archive_snapshot_hash="e" * 64
    )
    changed_entries = (changed_entry, second.entries[1])
    changed_valid, changed_errors = verify_audit_ledger_chain(changed_entries)
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        require(payload["version"] == "V15.3", "저장 Version 오류")
    ...
    checks = {
        "Version is V15.3": second.version == "V15.3",
        "Default policy is valid": second.policy_checks_passed,
        "Policy is immutable": (
            SandboxArchiveAuditLedgerPolicy.__dataclass_params__.frozen
        ),
        "Invalid policies are blocked": not unsafe.all_checks_passed,
        "V15.2 audit source passed": second.source_checks_passed,
        ...
        "Result save and load passed": payload["version"] == "V15.3",
    ...
    print("=" * 100)
    print("AI STOCK BOT V15.3 SANDBOX ARCHIVE AUDIT LEDGER TEST")
    print("=" * 100)
    print("V15.3 VALIDATION CHECKS")
    print("-" * 100)
    for name, passed in checks.items():
        print(f"{name:<66}: {passed}")
    print("=" * 100)
    require(checks["All checks passed"], "V15.3 Validation Check 실패")
    print()
    print("V15.3 sandbox archive audit ledger test completed successfully.")
    print(
        "V15.2 Certificate Hash 보존, Ledger Sequence, SHA-256 Hash Chain 및 "
