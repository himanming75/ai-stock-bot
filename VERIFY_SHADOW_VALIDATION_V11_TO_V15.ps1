...
$Text = Get-Content .\shadow_validation_v11_v15\service.py -Raw
$ForbiddenCalls = @(
  '\.\s*submit_order\s*\(',
  '\.\s*cancel_order\s*\(',
  '\.\s*replace_order\s*\(',
...
Write-Host "FALSE SIGNAL DETECTOR: OBSERVATION ONLY"
Write-Host "TRADE REPLAY: AUDIT ONLY"
Write-Host "LIVE READINESS: ADVISORY ONLY"
