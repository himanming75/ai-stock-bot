...
from test_sandbox_verification_certificate_ledger import (
    create_source as create_verification_source,
)
from test_sandbox_verification_certificate_ledger import record

from backtest.sandbox_verification_ledger_integrity_audit import (
    SandboxVerificationLedgerIntegrityAuditPolicy,
    audit_sandbox_verification_ledger_integrity,
    load_audit_result,
    save_audit_result,
...
NOW = datetime(2026, 7, 28, 16, 10, tzinfo=timezone.utc)
...
def create_source() -> Any:
    first = record(create_verification_source())
    return record(
        create_verification_source(),
        existing=first.entries,
        now=datetime(2026, 7, 28, 15, 51, tzinfo=timezone.utc),
    )
...
def audit(
    ...
    text: str = "AUDIT IN MEMORY SANDBOX VERIFICATION LEDGER INTEGRITY",
    policy: Any = None,
    now: datetime = NOW,
) -> Any:
    return silent(
...
def main() -> None:
    ...
    require(source.to_dict() == source_before, "V15.5 Source 변경 감지")
    ...
    require(certificate_valid and not certificate_errors, "Certificate Hash 실패")
    require_safe(result)
    ...
    unsafe_policy = SandboxVerificationLedgerIntegrityAuditPolicy(
    ...
    backward = audit(source, now=datetime(2026, 7, 28, 15, 50, tzinfo=timezone.utc))
    ...
    changed_certificate = replace(
        result.certificate, verification_ledger_snapshot_hash="a" * 64
    )
    ...
    with tempfile.TemporaryDirectory() as directory:
        report_path, latest_path = save_audit_result(result, Path(directory))
        require(report_path.exists() and latest_path.exists(), "저장 실패")
        payload = load_audit_result(latest_path)
        require(payload["version"] == "V15.6", "저장 Version 오류")
    ...
    checks = {
        "Version is V15.6": result.version == "V15.6",
        ...
        "V15.5 verification ledger source passed": result.source_checks_passed,
        "Verification ledger hash chain passed": (
        ...
        "Verification ledger remained unchanged": (
        ...
        "Result save and load passed": payload["version"] == "V15.6",
    ...
    print("=" * 104)
    print("AI STOCK BOT V15.6 SANDBOX VERIFICATION LEDGER INTEGRITY AUDIT TEST")
    print("=" * 104)
    print("V15.6 VALIDATION CHECKS")
    print("-" * 104)
    for name, passed in checks.items():
        print(f"{name:<70}: {passed}")
    print("=" * 104)
    require(checks["All checks passed"], "V15.6 Validation Check 실패")
    print()
    print(
        "V15.6 sandbox verification ledger integrity audit test "
        "completed successfully."
    )
    print(
        "V15.5 Ledger Hash Chain, Entry Findings, Snapshot Hash, 읽기 전용 "
